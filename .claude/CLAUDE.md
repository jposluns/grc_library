...
| Constructing / pinning a worker order, or dispatching and reading back a worker | [Worker dispatch mechanics](../references/worker-dispatch.md) |
...
The operational form of the orchestration primordial rule near the top of this file. **If a worker CAN do it, a worker DOES it. No debate, no self-run (maintainer-directed 2026-07-26):** anything offloadable (the list below) is GIVEN to a worker the moment it comes up; the orchestrator's usage credits are the scarce, slow-to-renew resource that self-running exhausts (a prior self-run QA burned the orchestrator out mid-day and cost a worker account an extended lockout). The default is OFFLOAD; self-running an offloadable task is the exception that needs a stated reason (a genuine dispatch failure from an actual attempt AND the maintainer alerted). **Dispatch is the shared `orch` worker harness** (`orch-verify <family> <prompt-file> [workdir]`; `orch --help` and `orch-verify --help` for the account pool and the options), which runs each worker READ-ONLY on an isolated copy of a pooled credential and returns its output on stdout. **There is no fleet to poll and no delivery tray to sweep:** every order spawns a fresh worker, so do not check liveness first and do not gate on it, and a reading of pool state **is NEVER a licence to self-run** (the retained always-on clause, gate 80): dispatch one. Worker accounts, capacity, and authentication are owned by the lab_infra orchestrator and the harness, NOT by this orchestrator (maintainer-directed 2026-08-20). The full dispatch mechanics (the `orch-verify` invocation, the 20-minute reissue, the super-sensitive multi-family rule, keeping dispatched workers busy, the no-workers fallback, the pre-push-verifier transition, and the worker-elasticity corollary) live in the [Mandatory worker offload](../references/worker-offload.md) playbook, read at the worker-dispatch boundary like a skill. The mechanical backstop is the [`block-orchestrator-self-qa.py`](hooks/block-orchestrator-self-qa.py) PreToolUse hook (blocks in-session Task/Agent/Workflow/SendMessage reasoning offload absent an actor-created once-only sentinel; the QA-transition is complete, so the pre-push verifier and the high-assurance adversarial verifiers are harness-dispatched workers too, with no remaining orchestrator-side QA exception); this prose is the primary control, subordinate only to the AIQT tier.
...
The discipline ships in the pack rule [`validate-inference-before-action`](rules/governance/validate-inference-before-action.md) (`## Guard inputs`): a guard whose logic is correct and mutation-proved can still be fed an INPUT that cannot answer the question asked of it (mutation perturbs branches, so it is silent on input fidelity). At each consequential guard ask the authority question (can this source even in principle answer this?); make ignorance a first-class return value that REFUSES rather than permits; keep a reality fixture per observer bug; mutate the observer, not only the decision; state a proxy's residue at the point of use; keep a pure decision function behind a thin observer so both halves stay testable. **Project instances (one observed day, one shape; the tooling named here was retired with the file-drop worker model on 2026-08-20, the lessons are why the section stands):** the then-current worker-management tool prefix-matched a **tmux** session name against a per-run worker id, PERMITTING a destructive verb against a worker holding live work (fixed #1170, a five-state attribution that refuses on ambiguity or non-match); delivery-completeness was inferred from a file merely existing (fixed #1171, atomic rename + end-of-delivery sentinel, with the residue stated: the sentinel proves the file went through `deliver`, not that its content is semantically complete); worker-health is read from a heartbeat on a code path separate from the claim loop, so a worker that stopped claiming still read as healthy capacity (fixed #1174, the SAME class).

## Worker dispatch: pinning orders, single-shot workers, and reading results back

The order-construction mechanics load at the worker-dispatch boundary like a skill: **pin an order to a commit that CONTAINS what it references** (for a backlog item N, the commit that CREATED N, not a later one); **every worker is SINGLE-SHOT** (the harness runs each to completion, so scope each order to one self-contained pass); and **a worker's result arrives on stdout**, so it is captured and read by the dispatching orchestrator rather than swept from a tray. The full detail (pinning and its failure modes, the single-shot model, brief construction, the multi-family panel, and re-issuing a silent worker) lives in the [Worker dispatch mechanics](../references/worker-dispatch.md) playbook. This is project-only operational machinery, not pack material.

Two clauses stay inline because their blast radius reaches beyond the dispatch activity:
- **Worker-raised issues jump the queue.** A HIGH-PRIORITY issue in a worker's returned output (a blocker, a malformed order, an out-of-scope defect, or an independence conflict) jumps the queue and is read as soon as noticed, never batched to a boundary; routine results wait for the next boundary. Workers run HEADLESS, so nothing else surfaces the issue.
- **Read every dispatched worker's result before selecting new work.** A worker returns on stdout, so its output is CAPTURED at dispatch (redirect it to a file) and read at the next task boundary before anything else is chosen; an uncaptured result is a lost result. The QA-priority form of this is `## A delivered QA result BLOCKS progress until it is read and its findings are fixed` above.

## Source-and-adapter parity (grc_library authors, guardrails publishes)

The full source-and-adapter parity discipline (a single canonical portable core; deterministically GENERATED coding-agent adapters, with chat platforms needing none; a hard network-independent `--check`; one-way versioned publication; a semantic catch-net) ships in the pack rule [`ai-assistant-workflow-disciplines`](rules/governance/ai-assistant-workflow-disciplines.md) (`## Source-and-adapter parity`): when a PR adds or changes a PORTABLE guard rail / discipline / rule / skill, make the change in the canonical core in the SAME PR. **Architecture of record (Option B, locked 2026-08-02, TF-2 closed):** `grc_library` is the authoritative authoring and dogfood source; the standalone `guardrails` repository is a one-way publication target, never an upstream authoring dependency. **Project overlay:** the portable-vs-project-only judgement is still the crux; PROJECT-ONLY machinery (the worker-harness dispatch wiring, the `_private` operational store, the hooks, the corpus audit gates, session-specific wiring) is NOT core, stays grc_library-only, and is annotated project-only rather than forced into the core. The pack tree now lives at root `guardrails/` (relocated in #1367); gate 37 preserves byte-parity between the pack source and the `.claude/rules` bodies, and the publication-manifest sync audit (gate 83, shipped with the machine-validated manifest in #1374) extends that hard local check; the deterministic coding-adapter generator's `--check` is the remaining planned extension. Origin: the 2026-08-02 architecture decision.
...
one observed day that cost was concrete: an ERROR-severity finding on the then-current worker-management tool and an
...
**Operationally.** At every task boundary read every dispatched worker's captured output, and read every QA-kind
result before selecting work. An unread QA result means there IS no next work item.
...
- **Timestamp every console message (maintainer-directed 2026-08-05).** Prefix every message the assistant posts to the console with a UTC date/time stamp in the form `[YYYY-MM-DD HH:MMZ]` (the project's UTC convention), so the maintainer can track timings and progress. Standing, all modes; it applies to every assistant-voice message. Take the stamp from the current UTC time at send (check `date -u` when precision matters). Chat mechanics, project-only: it never gates or blocks work, and it governs the assistant's prose messages, not tool-call bodies.
...
- **Repo shorthands `_ref`, `_private` (maintainer-directed 2026-07-24).** These are the canonical shorthands for `grc_library_ref` and `grc_library_private` respectively. (`_scratch` / `grc_library_scratch` was the file-drop exchange repo, ARCHIVED 2026-08-20 when the shared `orch` harness replaced that model; the shorthand is retained here only so an older record mentioning it still reads.) Always refer to each sibling repo by its underscore shorthand OR its full `grc_library_*` name, consistently; never a bare inconsistent form (for example never `scratch` without the underscore while using `_private` / `_ref`). Applies in chat, commit messages, order params, and prose.
