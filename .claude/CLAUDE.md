...
1. **Feature branch only, never `main`** (hook [`block-branch-to-main-edit.py`](hooks/block-branch-to-main-edit.py) for Edit/Write, and the git-native pre-commit check [`check-commit-on-main.py`](../tools/check-commit-on-main.py), installed by [`install-git-hooks.sh`](../tools/install-git-hooks.sh), which refuses the COMMIT from any tool or worktree); confirm `tools/run_all_audits.sh` after each commit, not only before push.
