...
6. **After merge, list the next five planned PRs** from [`TODO.md`](../TODO.md) and the private `P-TODO.md` (whichever list holds the next work) in chat, and write them to `grc_library_private/.working/next-prs.txt` in THIS PR (first line `1) ...; 2) ...; 3) ...`, roughly 120 chars; further queue on a `# then:` line). Behavioural.
