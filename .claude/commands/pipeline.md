...
3. **Refine the tool's rough output** (the tool is a deterministic backbone; polish the semantic parts):
   - **Checkbox, THREE states** (per the canonical `pipeline-view-format`): the tool marks every open bullet `[ ]` by default, so cross-check each id. A closed item (in `DONE.md`) is `[x] PR####` (flag stale done-bullets for rotation). An open item whose WORKERS have completed all pre-work possible (a verified/ready seed or an orch-verify result returned, verified, and recorded against the id, e.g. a READY row in the seed-verification log) is `[w]`. An open item with no worker pre-work yet stays `[ ]`. Blocked comes ONLY from a GRANTED `[BLOCKED]` tag, never assistant-asserted.
