Invoke the `pr-retrospective` skill defined in this project's pack at [`guardrails/skills/pr-retrospective/SKILL.md`](../../guardrails/skills/pr-retrospective/SKILL.md). Slash-command entry point is `/retro`; the underlying skill name remains `pr-retrospective` (the descriptive name documents the workflow's purpose, the slash command is the ergonomic verb). Execute the retrospective (a pre-merge finalizing step, run after `/validate-pr` and before merge) per the six-step process the skill encodes:
