...
list passes, and the guard claims NO dialect completeness. That residual class is
ACCEPTED, not solved: the r4 codex verifier demonstrated it with "I will come back after
CI completes", and adding that one phrasing (beside "check back") does not close the
class. Pattern-adds are whack-a-mole; the behavioural rule is the real control and this
hook is only the nudge. Underscore spellings of the compound targets
(``dispatch_and_await``, ``offload_and_await``) DO now match (r4 codex B1). The trigger is the WORD, not a verified absence of productive
...
WAITING_PATTERNS = [
    ...
    r"\bwaiting\b",
    r"dispatch[-_]and[-_]await",  # the dressed-up substitutes the actor actually used
    r"offload[-_]and[-_]await",
...
_EXEMPT_PATTERNS = [
    ...
    r"\basync[ /-]?await\b",
    r"\bawait\b(?=\()",                            # await(...), NO space: prose "await (x)" is a yield
    r"\bawait\s+[A-Za-z_]\w*[._][\w.]*\(",         # await worker.result(); a DOTTED/UNDERSCORED call only
...
def last_assistant_text(transcript_path: str) -> str:
    ...
    for line in reversed(lines):
        ...
        try:
            ...
            return ""     # a parse fault fails OPEN: never fall through to an OLDER row
...
def _consume_escape() -> bool:
    ...
    The stat is ``Path.stat()``, NOT ``Path.is_file()``: ``is_file()`` swallows OSError
    and returns False, so the fail-open branch above was unreachable dead code on a real
    path and the guard actually failed CLOSED on a stat fault (found by the r4 claude
    verifier, F2). ``stat()`` propagates, so the documented behaviour is the real one."""
    try:
        st = ESCAPE_FILE.stat()
    except FileNotFoundError:
        return False          # no sentinel: the block stands
    except OSError:
        return True           # cannot stat: fail OPEN, never block on a filesystem fault
    if not stat_mod.S_ISREG(st.st_mode):
        return False          # a directory or device at that path is not the sentinel
    try:
        ESCAPE_FILE.unlink()
    except OSError:
        pass
...
def _self_test() -> int:
    ...
    class T(unittest.TestCase):
        ...
        # ---- r4 dual-family regression set. Every concrete input BOTH families gave as
        # a live false negative, pinned end-to-end. Sources: claude F1, codex B1/B2/B3/B4.
        def test_r4_false_negatives_now_block(self):
            for phrase in (
                # claude F1 / codex B2: the await-paren exemption swallowed prose
                "I await (1) the CI result and (2) the r4 delivery.",
                "I await (for now) the CI result.",
                # codex B2 adjacent: the qualified-call exemption swallowed prose
                "I await confirmation(for now).",
                # codex B1: underscore spellings of the compound targets
                "I will dispatch_and_await from the worker.",
                "I will offload_and_await from the worker.",
                # codex B3: a first-person yield paraphrase beside "check back"
                "I will come back after CI completes.",
            ):
                self.assertIsNotNone(decide(False, phrase), phrase)

        def test_r4_regression_set_is_a_sole_trigger_each(self):
            """No co-trigger may mask a regression in the pattern under test."""
            for phrase in (
                "I await (1) the CI result and (2) the r4 delivery.",
                "I await confirmation(for now).",
                "I will dispatch_and_await from the worker.",
                ...
                self.assertEqual(len(all_triggers(_sanitize(phrase))), 1, phrase)

        def test_r4_fixes_do_not_break_the_real_code_exemptions(self):
            """The B2 narrowing must still allow genuine await-call prose."""
            for phrase in (
                "The helper calls await(fut) before returning.",
                "The helper calls await worker.result() before returning.",
                "The helper calls await worker_result() before returning.",
                "The function uses async/await.",
                "Renamed wait_for_completion.",
            ):
                self.assertIsNone(decide(False, phrase), phrase)

        def test_b4_parse_fault_does_not_fall_through_to_an_older_row(self):
            """codex B4: a malformed NEWER row must fail open, not surface an older yield."""
            with tempfile.NamedTemporaryFile("w", suffix=".jsonl", delete=False) as f:
                f.write(json.dumps(assistant_row("Waiting on CI.")) + "\n")
                f.write("{broken\n")
                path = f.name
            try:
                self.assertEqual(last_assistant_text(path), "")
        ...
        def test_b4_recursion_fault_does_not_fall_through_either(self):
            with tempfile.NamedTemporaryFile("w", suffix=".jsonl", delete=False) as f:
                f.write(json.dumps(assistant_row("Waiting on CI.")) + "\n")
                f.write("[" * 100000 + "0" + "]" * 100000 + "\n")
                path = f.name
            try:
                self.assertEqual(last_assistant_text(path), "")
                self.assertEqual(run_main({"transcript_path": path}), 0)
        ...
        def test_f2_reality_fixture_absent_sentinel_on_a_REAL_path(self):
            """claude F2: the fail-open branch must be reachable on a real Path.

            is_file() swallows OSError, so the old code failed CLOSED on a stat fault and
            the documented fail-open was dead. This pins the real-Path behaviours."""
            with tempfile.TemporaryDirectory() as d:
                absent = Path(d) / ".allow-waiting-yield"
                with escape_at(absent):
                    self.assertFalse(_consume_escape())      # absent: the block stands
                present = Path(d) / ".present"
                present.write_text("")
                with escape_at(present):
                    self.assertTrue(_consume_escape())       # present: allow and consume
                    self.assertFalse(present.exists())       # consumed exactly once
                a_dir = Path(d) / "adir"
                a_dir.mkdir()
                with escape_at(a_dir):
                    self.assertFalse(_consume_escape())      # a directory is not the sentinel
