...
2. **PR close-out checklist.** Before pushing any PR, confirm every paired bookkeeping
   ...
   - **Detailed-mirror current-week sweep** (the changelog-restructure current-week model;
     ...
     resolved via `lint_common.resolve_working`; Option C, closed in #2067)
     ...
     RETIRED; the LIVE detailed mirror now lives in the operational store (resolved via
     `resolve_working`), its completed weeks are rolled up in the root CHANGELOG AND swept to the
