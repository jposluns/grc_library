...
| SCA | TVM-06 | RV.1.1, RV.2.2, PW.4.1, PW.4.4, PS.3.2 | A.8.8 |
| Container scan | I&S-04 | PW.4.4, RV.1.1 | A.8.8 |
| IaC scan | CCC-06 | PW.7.2 | A.8.9 |
| Artefact signing | CCC-04 | PS.2.1 | A.8.27 |
| Manual approval | CCC-01 to 03 | N/A | A.8.32 |
| Branch protection | CCC-04 | PS.1.1 | A.8.32 |

**SLSA relationship.** SLSA v1.2 has two tracks, each with its own levels: a Build track (L0 to L3) describing the trustworthiness of the build platform and the provenance it produces, and a Source track (L1 to L4) describing the trustworthiness of the process that produced a source revision. Two of the gates above relate to it. Artefact signing relates to the Build track, but a Build level is a property of the build PLATFORM and the provenance it generates (Build L2 needs a hosted build platform that generates and signs provenance; L3 additionally needs a hardened, tamper-resistant platform), not of a signing step in isolation. Branch protection relates to the Source track: restricting who can update a protected branch and preventing history rewrites support its access-and-history requirement (Source L2 and above), and required review before merge supports its code-review requirement (Source L4); a Source level is likewise a property of the source control system and its continuous enforcement, not of the gate alone. The source-analysis and scan gates (secret scanning, SAST, SCA, container and IaC scan) confer no Build level; where the source control system enforces them as required checks before merge, they are the kind of organizational technical control the Source track counts from Source L3, again as a property of that system's continuous enforcement rather than of the gate alone. Do not read a per-gate SLSA level from this table.
