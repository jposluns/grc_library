...
- **An archive-coverage gate (planned, defense-in-depth).** A second check is planned to assert that every rolled-up root period vouches for an existing archive file spanning its range, closing the mirror-only silent-pass direction the parity gate's dynamic floor leaves open. It would carry an exemption list for accepted history-only ranges whose detail predates the archive convention or is fragmented in version-control history (not lost, but not cleanly reconstructable). The coupled roll-up tool above is the PRIMARY guard (it makes a root-only roll-up or mirror-only sweep impossible via the sanctioned path); this gate is an additional mechanical backstop.
