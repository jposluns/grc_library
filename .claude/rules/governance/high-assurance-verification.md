...
3. **Independent adversarial verification.** Dispatch separate verifier workers, blind to each other and to the research workers' reasoning, each with an adversarial brief to *refute*, not to confirm. The two complementary lenses:
   ...
   The verifiers are independent because a verifier that sees the research worker's rationale inherits its blind spots. Each verdict must quote the ground-truth source. When more than one failure mode is plausible, give each verifier a distinct lens rather than running identical ones; diversity of lens catches what redundancy cannot. The independent verifiers ARE drawn from DIFFERENT model families (two or more, for example one Claude-family, one GPT/Codex-family, and a Gemini-family where a project has one), never multiple instances of a single family: different model families have systematically different blind spots, so a cross-family panel is a stronger form of the independence this stage requires than same-family instances, which share training-induced blind spots, and each additional independent family widens that coverage further. Family-diversity COMPOSES with the lens-diversity above rather than replacing it: assign the distinct lenses across different families (for example the false-negative lens to one family, the false-positive lens to another), so each verdict carries both a distinct lens and an independent family; a third available family adds a further independent verifier, taking a further lens where one applies or a cross-family check on an existing lens. Where a single refute-brief fits the change (one dominant failure mode), run that one brief to EACH available family and reconcile; a finding present in only one family's delivery is triaged on its own merits, and agreement between families is corroboration worth recording. The verifiers are SEPARATE-family WORKERS (dispatched as distinct worker sessions), not same-session subagents, which would share the orchestrator's own model family and defeat the cross-family independence. Run to as many independent families as have tokens; the graduated floor on token or tooling unavailability drops the panel to the families that can run, keeping the cross-family minimum of two wherever two are available.
...
The register is surfaced at session resume alongside the project's other configured standing records, such as a pending-decisions queue and any semantic-audit history, so a future session sees the open sensitive items before it starts queued work and does not lose track of an item mid-harness across a session boundary. An item is removed from the active register (or marked `verified` and retained for the record) only when its harness is complete; the assistant does not silently drop a `pending` item, exactly as it does not drop a pending decision.
...
| --- | --- | --- | --- | --- |
| Independent verification before a high-stakes change | N/A | GRC-05, A&A-04 | A.5.35, A.5.36 | V15.1, V16.2 |
| Adversarial review proportional to risk | N/A | GRC-04, GRC-05 | A.5.4, A.5.35 | V15.1 |
| Deterministic, repeatable apply (no ad-hoc hand edit) | N/A | CCC-01 to 04 | A.8.32 | V15 |
| Findings and verifications traceable to a record | N/A | LOG-02, LOG-04, LOG-10 | A.8.15, A.5.36 | V16.2, V16.4 |
