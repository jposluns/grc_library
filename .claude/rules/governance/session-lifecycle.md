...
- **If the QA is outstanding, WAIT for it.** Where a genuine attempt to delegate the QA fails (a dispatch that genuinely fails, no worker taking it up after a real attempt, or an independence conflict a real attempt surfaces), run it directly: on this one conflict the mandatory-QA obligation outranks the delegate-by-default preference, never the reverse. The self-run is licensed by a FAILED ATTEMPT, never by a pre-emptive reading that no worker will serve it. Requesting more worker capacity is the right parallel move, but the request must not become the reason the QA never ran.
...
every path and identifier the order tells the worker to read. Queue enough to fill the available worker capacity
...
| --- | --- | --- | --- | --- |
| Durable state records and auditable handoff | N/A | LOG-02, LOG-04, LOG-10 | A.8.15, A.5.36 | V16.2, V16.4 |
| Explicit authorization boundaries per mode | N/A | IAM-15, GRC-04 | A.5.15, A.5.18 | V8.2 |
| Evidence-gated lifecycle decisions | N/A | GRC-05 | A.5.36 | V16.2 |
| Concurrency control on shared state | N/A | CCC-01 to 03 | A.5.4, A.8.32 | V15.4 |
