...
2. **Lower quality or correctness.** The instruction would introduce a defect, skip a verification the work depends on, or produce a worse result than the requestor would accept if they saw it. (When the conflict is specifically the optimization-dimension tradeoff among the AIQT tier, Progress, Speed, and Cost, `project-integrity` governs; this rule covers the broader case of any net-negative instruction, including ones with no quality dimension.)
...
- **`project-integrity`** fixes the optimization-dimension priority (the AIQT tier > Progress > Speed > Cost) and says escalate when a *constraint* forces a compromise on that tier. This rule is broader on the trigger (any net-negative effect, including efficiency and productivity, not only quality) and specific on the source (a requestor *instruction* that is itself counterproductive). Where they overlap (a maintainer instruction that would lower quality), both point the same way: surface and confirm.
...
| --- | --- | --- | --- | --- |
| Surface adverse effects before acting on an instruction | N/A | GRC-04, GRC-05 | A.5.4, A.5.36 | V15.1 |
| Confirm-before-destructive-action discipline | N/A | GRC-04 | A.5.4, A.5.18 | V15.1 |
| Scope-bounded authorization (an override covers the case it was given for) | N/A | IAM-15 | A.5.15, A.5.18 | V8.2 |
| Decisions traceable to a confirmation step | N/A | LOG-02, GRC-04 | A.8.15, A.5.4 | V16.2 |
