...
**Status: ACTIVE (umbrella 4.1, compile PR-35).** This directory is the structure of a standalone,
...
- **PR-36 (next):** g18 (intra-doc-refs), same Pattern A `scan` shape (`REF_PATTERNS` + `EXEMPT_FILES` wrapper config). Then the SAFETY `scan` set g21-24. EXCLUDE gate 51 (grc-only `.working/`-store coupled).
