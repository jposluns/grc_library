...
New normative content uses `must` and `must not` for absolute obligations and prohibitions.
Reserve `shall` and `shall not` for two narrow cases: a direct quotation of an external standard
whose normative text uses that verb, and legacy content not yet harmonized (converted to `must`
opportunistically as documents are revised). Verb consistency within a document outranks
