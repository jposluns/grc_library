...
_ASVS_NOT_ID_BEFORE = re.compile(
    ...
    r"(?-i:(?:\s+(?:(?:for|of|and|the|on|in)\s+)?(?!ASVS\b)[A-Z][A-Za-z]{0,19}){0,4})"
...
_CWE_TOKEN = re.compile(r"(?:(?<![\w-])|(?<=\d-)|(?<=\d--))CWE-(\d+)(?![\w]|-\d|\.\d)", re.IGNORECASE)
...
def _check_asvs(raw: str, lineno: int, rel: str, header: list[str] | None,
    ...
    for m in _ASVS_TOKEN.finditer(raw):
        ...
        if m.group(2) == "0":  # a zero middle number is a version, never an ASVS identifier
            continue
        excl = _ASVS_NOT_ID_BEFORE.search(_plain(raw[:m.start()]))
        if excl and not _ASVS_WORD.search(excl.group(0)):  # an exclusion never spans ASVS itself
            continue
        tok = m.group(0)
