...
- **upstream** and **verified**: the publisher evidence for the historical fact (an `https://` URL), and the past UTC date it was checked (a TOML date, `YYYY-MM-DD`).

Every value is a single line of printable ASCII with no leading or trailing space.
