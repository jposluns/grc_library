...
Phase 1 velocity bundle: five medium README polish findings shipped as one PR; all edits localise to [`README.md`](../README.md). FR-4 — the "Framework alignment model" section's one-sentence acronym chain expanded into a first-use-explained bulleted list (ISO, NIST, COBIT, CSA CCM, STAR, OWASP, ASVS, SAMM, MITRE ATLAS, MITRE ATT&CK) with a soft pointer to [`governance/register-glossary.md`](../governance/register-glossary.md). FR-5 — the paradoxical "approximately 300+ documents ... automated counts in a future release" line replaced with a pointer to the already-machine-generated [`taxonomy.yml`](../taxonomy.yml) and [`docs/portal.md`](../docs/portal.md); sector-subdirectory paths linked. FR-6 — the standalone CalVer paragraph removed from the front matter; the version-policy reference folded into the "Specification and authoring files" table row for [`specification-master-project.md`](../specification-master-project.md) (its normative home). FR-7 — the "Where to go next, by intent" bulleted list reformulated as a table with an explicit audience-signal lead clause naming [`docs/portal.md`](../docs/portal.md) as the canonical front door per PR #165 (FR-56). FR-8 — both version-bearing rows demoted to the bottom of the metadata block and given inline parenthetical explanations distinguishing CalVer (library-wide) from semantic per-document versioning (this file); Pass-1's ⚠️ modification note ("the original line numbers have moved on; structural concern remains") acted on by the demote-and-explain edit. Rec-6 from the fitness review (the original FR-1 through FR-8 bundle) is now fully closed; FR-1, FR-2, FR-3 shipped earlier as severity-warranted individual PRs (#155, #156, #147). First Phase 1 bundle authored under the research-assistant discipline; two corrections to the worker draft were applied at apply-time (FR-3 PR cross-reference; current library/README version numbers). Per-doc README `1.9.23 → 1.9.24`; library `2026.06.152 → 2026.06.153`. Backlog 82 → 77 open.

### PR #171 — CLAUDE.md PR-activity-subscription + 60s-timer discipline (2026-06-21)

Not previously in TODO; surfaced during the PR #170 session as a paired discipline needed alongside the version-bump rule (the 60-second fallback timer for `mcp__github__subscribe_pr_activity` calls). Added a `## PR activity subscription discipline` section to [`.claude/CLAUDE.md`](../.claude/CLAUDE.md) codifying that every subscription call arms a paired 60-second background timer (subscriptions reliably deliver failure events but not all success transitions; the timer is the catch-net). PR workflow step 3 updated to reference the new section. The discipline operationalizes the webhook-subscriptions guidance in the pack rules `action-before-explanation-of-inaction.md` and `evidence-grounded-completion.md` (API polling and webhook subscriptions section) by specifying the cadence and the re-arm pattern that pack rules leave open. Also rotated PR #170 into [`.working/DONE.md`](DONE.md) as a non-FR workflow change. `Changelog: skip` per the `.claude/` and `.working/` directory exemptions.

### PR #170 — CLAUDE.md three-surface version-bump discipline (2026-06-21)

Not previously in TODO; surfaced during the post-merge debrief of PR #169 as a recurring pattern across multi-commit PRs (most recently PR #169's gate 40 catch on a body-change commit missing its per-doc Version bump). Added a `## Version-bump discipline` section to [`.claude/CLAUDE.md`](../.claude/CLAUDE.md) codifying (1) per-document `Version` bumps in the same commit that changes the document's body, (2) library CalVer bumps once per PR in the last commit before push, and (3) README `Version` field bumps in the same commit as the CalVer bump. Plus a three-question commit-time operationalization and a pointer to the post-commit [`tools/run_all_audits.sh`](../tools/run_all_audits.sh) discipline (already in PR workflow step 1) as the catch-net. `Changelog: skip` per the `.claude/` directory exemption (AI-assistant guidance, not user-visible content).

### PR #169 — FR-26 (medium) + FR-27 (medium) + FR-28 (medium): access-control procedure decision-path gaps (2026-06-21)

Phase 1 velocity bundle. Three medium-tier access-control polish findings closed, all in [`security/procedure-access-control.md`](../security/procedure-access-control.md). FR-26 — §1.2 3-business-day approval SLA gains a tiered escalation ladder (manager at +2 days, CISO at +2 more days, auto-decline at +2 beyond that, all logged and reviewed quarterly) following the 1-2-3-ceiling pattern from PR #152 (FR-19 CAPA §6.3.1) and PR #157 (FR-16 exception renewal §3.5). FR-27 — §3.2 access-review "appropriate" replaced with four bounded acceptance criteria (RBAC-catalogue role-profile match, least-privilege envelope, current business justification, PIM-role membership for privileged access), with explicit consequence (access failing any criterion is revoked under the next access review). FR-28 — §1.4 emergency-access verbal approval gains trigger conditions (declared incident response active per security/procedure-security-incident-response.md, or material business/safety harm from portal delay), retains the 24-hour formalisation clock, and adds §1.4.2 revocation consequence (Identity Team must revoke at 24 hours if not formalised). Opportunistic Canadian-English spelling rolled in (`formalized` → `formalised`). The "may" permission verb is preserved per FR-44 §6.1 rule 3; revocation uses "must revoke" rather than "may not" per FR-45 / PR #150. Per-doc `1.0.0 → 1.1.1` (two-step: 1.0.0 → 1.1.0 with the body changes; then 1.1.0 → 1.1.1 as the post-§3.2-rephrase recency bump caught by CI gate 40).
