...
### H-01 EU AI Act Article 18 retention under-specification closed (2026-07-25, PR #1153)

The codex deep assessment's only HIGH finding. Two retention rows allowed `Model decommission + 5 years` with "EU AI Act" as the stated legal basis, where the Act requires a provider to keep the Article 11 documentation set for 10 years from the system being placed on the market. Both rows now state the statutory period as a FLOOR (the earlier draft would have shortened retention for a long-lived model), the deeper defect is disclosed (nothing in the Act anchors retention to decommission, so the 5-year figure is the organization's own and citing the Act for it was the real error), and both reach the Act's SECOND 10-year clock for general-purpose models (Annex XI, Article 54(3)(b)), which the original finding did not mention. Run under the full high-assurance harness because the maintainer authorized merging it without human review.

### §1.24 Sweep 120 F2: the AI-jurisdiction coverage surfaces now name all seven annexes (2026-07-25, PR #1152)

Closed TODO 1.24, the eight surfaces that understated the library's own AI-jurisdiction coverage after the California ADMT and Singapore annexes shipped: the public For-AI page positively told readers those two were NOT covered, the glossary routed California ADMT to the annex #1141 de-designated, the coverage-gaps register graded a struck gap open and claimed NYC was uncited, the decision tree routed adopters to two of seven annexes, and the portal generator hardcoded a two-annex list. Three further defects surfaced by enumerating rather than checking only the cited lines: a third stale coverage-gaps row, a preamble understating by six rather than five, and a TODO 2.21 deferral that was wrong on arrival (the Australia annex predated the deferral text by three days). Also widened the web link manifest so gate 75 reaches 369 targets instead of 321, covering all 15 governance rules and all 24 skills, which were previously linked by the site and gated by nothing.
...
### §3.105 CCPA ADMT coverage de-duplicated (consolidated into the AI annex) (2026-07-24, PR #1141)

Consolidated the CCPA ADMT limb into a single canonical home per the maintainer decision. Migrated the one privacy-annex-only detail (the section-7001 significant-decision enumerated categories) into `ai/jurisdictions/annex-ai-us-california.md` first (no content lost), then narrowed the `privacy/jurisdictions/annex-privacy-united-states.md` ADMT bullet to a brief privacy-context summary plus a pointer to the AI annex, mirroring the pattern the privacy annex already uses for the Colorado AI statute; the privacy annex keeps its CCPA risk-assessment and cybersecurity-audit slices (privacy-domain). Both annexes' cross-reference/overlap notes updated to record the consolidation as done. Offloaded edit-plan research, orchestrator-verified against the live files; skeptical-verifier pre-push (authorial content removal). Both docs Version+Date bumped, taxonomy/scorecard regenerated.

### §1.22.3 initial working-tree sweep to _private (part a) (2026-07-24, PR #1140)

Ran the initial destructive .working-to-_private sweep as a dedicated cleanup-PR (data-safe emit-to-_private, verify-archived, then prune; git history retains everything). At the maintainer's decided cutoff (keep current + 1 prior week), all dated records and the ~383 roll-up rows fall inside that window and stay in-repo, so only the 2 completed one-off dirs (pack-hygiene-acceptance, pack-hygiene-fragments) swept to the grc_library_private archive; the 3 TODO references and the .working/README.md inventory row to the swept acceptance file were delinked. §1.22.3 stays open: (b) the DONE.md/pending-decisions destructive-sweep enablement is a deferred tool-build (per verify-1223-draft, it needs a DONE effective_floor + a tightened resolved-and-aged predicate + self-tests), and (c) the session-boundary dedicated-cleanup-PR policy codification remains.

### §3.56a guard 2: gate-name-citation inventory guard (gate 77); part (a) complete (2026-07-24, PR #1139)

Built the last §3.56a pack-hygiene mechanization as gate 77 (`tools/lint-gate-citation-inventory.py`): it parses the spec §6 inventory into a number-to-name map and flags a `gate N (<name>)` / `the <name> gate (N)` citation (in a wiring section, command stub, or CLAUDE.md) whose name shares no significant token with §6's canonical name for that number, catching a silent renumber. FP-safe by scanning only the citation surfaces (not the spec's own role-gloss prose), matching name-shaped parentheticals only, and comparing by token overlap; count-free so it does not trip gate 39. Wired across the four surfaces with a 9-case self-test + regression class, ships green guard-first (zero live drift). This completes §3.56a part (a), all 3 mechanizations built (the provenance-register-as-gate-41 surface, gate 76 skill-body linter, gate 77 citation guard); §3.56a stays open for parts (b) to (e).
...
### Two decided P3 pending-decisions closed: 3.19 + 3.32 (2026-07-23, PR #1103)
Acted on two maintainer decisions batch-answered at the Task-1-complete boundary. §3.19 (cross-repo worker-provenance link convention): chose plain backticked `repo:path` text over the fragile, un-gate-checkable cross-repo markdown link; the convention is recorded in the detailed-CHANGELOG header, zero live instances remained to convert (the current-week mirror had swept the old ones and current practice already used plain text), and the stale "per TODO 3.19" pointer in `tools/preflight-changelog.py` was reworded to the convention. §3.32 (count-vs-enumeration advisory): confirmed CLOSE; the built-then-census-vetoed advisory (63 candidates, zero true positives, not mechanically FP-separable) stays discarded and the corpus rests on the recount discipline clause. Both deleted from TODO; pending-decisions entries resolved.

### Task-1 pack reconciliation complete: backlog item 3.104 CLOSED (2026-07-23, PR #1102)
Closed item 3.104 (Task-1: bring the published `dev-security/claude-rules` pack to parity with adopted practice). Its parts: (a) GAP-1 the repeated-failure circuit-breaker shipped as a `## The repeated-failure circuit-breaker` clause in `validate-inference-before-action` (both trees); (b) GAP-2 shipped as the 14th rule `decision-classification-before-enacting` in #1101; (c) F1 genericized the `change-tracking` terse-entry `.working/` example (F2, the `adopt` SKILL owner-handle, was examined and DECLINED as a deliberate concrete-name example in the section titled the parent's instantiation, per surface-counterproductive-instructions); (d) the portable pack-parity-coupling discipline added as a `## Pack-parity coupling` clause in `ai-assistant-workflow-disciplines` (both trees). The FUTURE pack-doc tail (currency-cadence pack-doc dep 1.14; change-impact-map pack-doc already in 1.18's scope) was migrated to those P1 items and is covered standing by the new coupling convention. Pack `1.63.0` to `1.63.1` (patch).

### Fourteenth governance pack rule `decision-classification-before-enacting` (Task-1 GAP-2) (2026-07-23, PR #1101)
Added the fourteenth pack rule and wired it across every enumeration and web surface plus the byte-identical local mirror: authorized work about to NOT happen is classified ACT / ASK / BLOCKED (from a closed, externally-observable blocker set) and written to the decision log before enacting, so un-instrumented internal state can never justify a hold. This is the GAP-2 portion of Task-1 pack reconciliation (item 3.104). Not a TODO closure: 3.104 stays open for GAP-1 (the repeated-failure circuit-breaker), the F1/F2 genericization, and the portable pack-parity-coupling clause, which ship as a second PR.

### P2 umbrella series A (2.25) + B (2.26) + P4.31 + Task-1 (3.104) authored (2026-07-23, PR #1100)
Authored the two maintainer-confirmed Priority-2 umbrella series into the backlog (recording only; nothing executed): Series A (2.25, governance traceability and coverage) consolidating the existing 2.24/2.15/2.22 into children 2.25.1-2.25.3 via redirect stubs plus new children 2.25.4 (AI assurance) and 2.25.5 (maturity model); Series B (2.26, OSCAL) with five gated children (adopt-decision through profiles). Plus the P4.31 pack-publication item and the Task-1 pack-reconciliation item 3.104 (the enumeration found zero P1 pack inaccuracies, so Task 1 is P3-only: GAP-1 + GAP-2 + F1/F2 genericization + the pack-coupling clause). Counters bumped (P2 to 2.27, P3 to 3.105, P4 to 4.32); the four live §2.15/§2.22 citers swept to 2.25.2/2.25.3. Not a TODO closure (stubs close with their targets); maintainer-directed authoring per the 2026-07-23 planning prompt.
...
### Anti-false-completeness guardrail (4 layers, PRs #1088-#1090; maintainer-directed 2026-07-23, not previously in TODO)
The corrective control for a confirmed failure: a false "every remaining backlog item is blocked" claim, made from a partial review, used to justify stopping an unattended run (the maintainer named lying as the worst failure mode). Layer 1 (#1088): advisory tool `tools/audit-backlog-actionability.py` enumerates every open TODO item and flags the no-blocker-token ones as PRESUMED-ACTIONABLE (92 items / 43 blocker-signal / 49 presumed-actionable, live). Layer 2 (#1089): the `block-unjustified-decision.py` hook now REFUSES a hold decision-log entry justified by a set-completeness / exhaustion claim unless it embeds a fresh full-audit token matching the live TODO count (FP-safe, gates on the BLOCKED classification; item-count parity-tested against the tool). Layers 3-4 (#1090): `evidence-grounded-completion` (both trees) gains the set-completeness-is-completion-class and asymmetric-skepticism principles plus an anti-pattern, and CLAUDE.md gains the "backlog-status characterization is the audit tool's output" convention (a persistent blocked record goes in `_private`, never the public tree). Prose alone had been violated, so the fix is mechanical (tool + hook) plus the sharpened rule.

### TODO §3.99: parity-gate exclusion-allow-list + D1-D8 delta-gate additive hardening (2026-07-23, PR #1087)
Closed §3.99 by EXTENDING gate 35 (`tools/lint-audit-gate-parity.py`) additively (no new gate, no gate-count bump): `verify_exclusion_and_delta_guards` cross-checks each of the three exclusion allow-lists against a positive signal (a setup step runs no gate script; a delta-gate step's script is not a §6 gate; a non-gate pre-commit hook carries no `--check`, the discriminator that separates the regen hook from the `--check` §6 gates), and confirms the 8 PR-only D1-D8 delta gates map cross-surface between the workflow and the PR-time runner with contiguous numbering. So a real gate mistakenly added to an exclusion set, or a mis-wired delta gate, now FAILS rather than being masked. FP-free today (0 findings on the live config; gate 35 still "parity confirmed for 73 gates"); +2 tests; §6 gate-35 narrative clause added; offloaded draft (worker-a) + skeptical verifier.

### TODO §3.22: D7 handoff-snapshot version-token check (rotation; resolved by #1075/#1076) (2026-07-23, PR #1085)
Rotated §3.22 to DONE: the D7-defeated-by-#746 defect it described (D7 located version tokens on the token-less `Current truth` line, so it validated 0 tokens and passed trivially) was RESOLVED by the #1075/#1076 D7 work that closed the same defect keyed as §3.89/§3.101. `tools/check-handoff-snapshot-on-pr.py` now locates the dedicated `Version snapshot (D7 validates these tokens)` sub-line and FAILS non-vacuously on a token-less line (`validate_snapshot_tokens`); the `HandoffSnapshotOnPrTests` fixtures cover the stale-token and token-less cases on the new marker. §3.22's "protected part" (a CLAUDE.md D7 convention note) was moot: no such note exists in CLAUDE.md. Also reworded a recycled-`§3.22` lineage comment in `tests/test_linters.py` to cite the stable PR #649, and marked `_private` deferred-protected-changes item 8 landed.
...
### TODO §3.71: corpus-wide "GRC Manager" -> "GRC Programme Manager" rename (2026-07-23, PR #1082)
Renamed all 60 non-canonical "GRC Manager" occurrences to the canonical "GRC Programme Manager" (the `register-role-authority.md` canonical; no "GRC Manager" role exists) across 5 corpus docs (procedure-audit-planning 25, procedure-capa 30 incl. 2 section headings, policy-encryption-and-key-management 2, procedure-supplier-due-diligence 1, procedure-supplier-audit 2), each Version+Date co-bumped, taxonomy/scorecard regenerated. Applied deterministically (ordered plural->title-case->lowercase `str.replace`, count-verified exactly 60, zero residual corpus-wide) from an offloaded census (worker-a); heading-anchor check clean (the 2 renamed capa headings break no `#anchor` link, section refs use numbers); the distinct `Compliance Manager` / `Trade Compliance Manager` roles correctly untouched; skeptical verifier SHIP, per-touch reference-breadth recorded.

### TODO §3.103: widen the decision-log deferral-trigger keyword set (2026-07-23, PR #1081)
Widened, in EXACT parity, the deferral/hold keyword set that gates the forbidden-internal-state-justification check in BOTH `.claude/hooks/block-unjustified-decision.py` and the mirrored `grc_library_private/tools/validate.py`, adding the common synonyms (hold off, postpone, punt, back-burner, sit on, leave for later, do it later, push to, park it) beyond the original five (blocked/defer/wind-down/skip), plus a hook self-test for a synonym-phrased deferral carrying a forbidden phrase (10 hook tests pass). Also refreshed the CLAUDE.md Decision-discipline pointer (removed the now-stale "TODO §3.103" forward reference and the old 5-marker enumeration). Cross-repo, orchestrator-only (workers cannot touch `_private`); the `_private` half pushed directly (parity is the point).
...
### TODO RB-7: AI-defense-matrix reference acquisition + corpus use/cite (2026-07-22, PRs #1057-#1064)
Closed the maintainer-directed RB-7 track: the four aidefensematrix.com-surfaced AI-security frameworks the maintainer acquired (OWASP AI Exchange, SANS Critical AI Security Guidelines, Google SAIF, Cyber Defense Matrix) were ingested into `grc_library_ref` and their corpus use/cite applied across seven PRs (#1057 Brazil ANPD adequacy currency, #1058 SAIF "execution" overreach fix, #1059 Canada GC/CCCS companions, #1060 high-assurance OWASP-AIX+SANS full-column AI-security integration, #1061 LatAm transfer-mechanism citations, #1062 NYDFS financial-services citations, #1063 held-breadth NIST IR 8596 draft-watch see-also), with the §3.70 pack crypto parity tighten (#1064) landed alongside. Two egress-gated residuals remain on the `_private` queue (OWASP Agentic authoritative source; Colombia RNBD Decreto 886/2014). The `_ref` discard-candidate Wiz "Securing AI Agents 101" was deleted (`_ref` PR #100).
