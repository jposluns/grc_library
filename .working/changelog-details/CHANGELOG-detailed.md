...
## 2026-07-28, Library Version 2026.07.712, PR #1221

Fixes the highest-severity adoptability finding from the deep-assessment (TODO 3.177, dual-family): four surfaces invite adopters to delete `.working/`, but 9 gates read files inside it (7 requiring it) and 23 deliverable-corpus documents hard-link into it, so a clean delete fails 8 of 78 gates.
...
- **Caveat on the four delete-invitation surfaces** (the README, the root CHANGELOG, the project CLAUDE.md, and the .working README, twice): "several audit gates read files inside `.working/`, so a clean adopter delete fails 8 of 78 gates; keep it or skip those gates (restoring bare stub files does NOT fully recover, some gates need valid field content)". Also fixed the incidental dead design-decisions link in the .working README (relocated to the private store).

- **Spec prose + regression fixture (verify-round-1 completeness).** The one-way-dependency rule in [`specification-project-governance-separation.md`](../../governance/specification-project-governance-separation.md) (section 4 and section 7.3) and the gate-53 description in [`specification-audit-programme.md`](../../governance/specification-audit-programme.md) section 5 now record the `.working/` widening; a `test_corpus_to_working_link_flagged` case was added to `DirectionalDependencyTests` in [`tests/test_linters.py`](../../tests/test_linters.py) so the widened branch has coverage. The pre-push skeptical verify (round 1) also corrected the caveat count to `8 of 78` and made the remediation truthful (a bare stub does not recover the header-content gates).
...
- **Round-2 dual-family skeptical verify (codex + claude, both refute-briefed, pinned to the pre-fix diff).** Codex delivered first (authoritative) with DO-NOT-SHIP on three findings, all dispositioned in-window: (1) error, the TODO/DONE rotation for 3.177 was missed (the PR fixes it but left it open with stale facts), FIXED by rotating 3.177 to DONE and splitting the [`TODO.md`](../../TODO.md)-delete residual to 3.185; (2) warning, gate 53 misses reference-style `[text][ref]` links (theoretical, zero live cases), ROUTED to 3.186; (3) nit, this mirror's caveat quote still said `restore stubs`, FIXED. Codex upheld the `8 of 78` count (seven linters plus gate 36). The claude lens cross-references on the pre-fix SHA.
