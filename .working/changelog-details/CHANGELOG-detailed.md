...
- **Gate 60 guardrail-cadence** ([`tools/lint-guardrail-cadence.py`](../../tools/lint-guardrail-cadence.py)): MAINTAINER-RATIFIED narrowing of the documented fail-closed stance. The DEFAULT history lookup routes through `resolve_working` and no-ops on absent; the docstring is amended so an UNREADABLE (present-but-broken) history still fails closed while an ABSENT history is a skip (an adopter clone never owed the review). The stance change is recorded in the docstring because no test covers the absent-history case.
