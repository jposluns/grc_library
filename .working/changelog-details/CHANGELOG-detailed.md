...
- **[`.claude/settings.json`](../../.claude/settings.json)** statusLine repointed to [[`tools/statusline-next-pr.py`](../../tools/statusline-next-pr.py)](../../tools/statusline-next-pr.py) (one line changed), fixing a latent parse bug where the old inline regex mis-parsed the `1)`-form first queue line.
...
### Verification
- All 78 audit gates pass; [`tools/run-linter-regression.py`](../../tools/run-linter-regression.py) rc=0; every rewired tool/hook self-tests green (`pr-closeout` 47/47, `audit-cross-repo-references` 5/5, `statusline-next-pr` 4/4, 2 new helper tests).
