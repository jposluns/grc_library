...
### Changed
- [`dev-security/claude-rules/skills/deep-assessment/SKILL.md`](../../dev-security/claude-rules/skills/deep-assessment/SKILL.md): the terminal state changes from "explicit maintainer sign-off" to the QA-activity completion standard the skill's own component instruments already use (every finding validated and fixed-or-routed). Edited the frontmatter description, the "structurally this is the trust-recovery suite" inheritance paragraph (now inherits TWO conventions, not three, dropping sign-off-as-terminal-state with the reason: the sign-off exists in the reactive trust-recovery tier to declare confidence restored after a lapse, which the proactive assessment has none of), the phase-8 heading + body ("Record, surface, and route to a terminal disposition"), the parallel-execution note, the Red Flags (self-invocation is still forbidden; self-termination on a completed zero-finding run is now correct), the Verification list, the Common-Rationalizations "zero findings" row, and the See-Also trust-recovery note.
- [`dev-security/claude-rules/governance/ai-assistant-workflow-disciplines.md`](../../dev-security/claude-rules/governance/ai-assistant-workflow-disciplines.md) and its byte-identical `.claude/rules/governance/` copy (gate 37): the QA-activity-completion-standard clause "The heaviest tiers (a whole-project assessment, a trust-recovery suite) add one condition: sign-off" now scopes the sign-off to the REACTIVE trust-recovery tier only; the proactive whole-project assessment terminates on the five completion conditions like any other QA activity.
- [`.claude/CLAUDE.md`](../../.claude/CLAUDE.md): the `## Whole-project deep assessment` section (the proactive-counterpart paragraph inherits routing + apply-time verification but NOT the sign-off terminal state; the "NOT cadenced" paragraph now terminates on the completion standard) and the `## QA-activity completion standard` section (the added-condition now applies to `/trust-recovery` only, not `/deep-assessment`).
- [`.working/deep-assessment/register.md`](../deep-assessment/register.md): the header/conventions record the new terminal-state model (a run closes on the completion standard, not a sign-off; the `Sign-off` column is retained as a historical record for r1-r5 and a completion-disposition record going forward) and the phase-8 key is "record and route". Fixed r5's stale Sign-off cell (it read "HOLDS for explicit maintainer sign-off", contradicting its own Status cell = signed off 2026-07-18, [`pending-decisions.md`](../pending-decisions.md), and DONE #1022 where r5's Medium-1 SEF-02->SEF-07 was applied) and appended superseding markers to r1/r3's stale opening-snapshot prose (which said "holds for sign-off" while the table rows record both signed-off).
- [`.claude/commands/deep-assessment.md`](../../.claude/commands/deep-assessment.md): the PAIRED slash-command entry point (gate 44 step-parity with the skill), edited to mirror the skill: the frontmatter inheritance clause (routing convention, not the sign-off terminal state), the step-8 record-and-route wording, the step-8 phase description, and the report-back line (completion disposition, not a sign-off request). Step identifiers 1-8 unchanged, so gate 44 parity holds.
- [`dev-security/claude-rules/README.md`](../../dev-security/claude-rules/README.md): the pack directory-tree one-line description of the skill changed from "sign-off-terminated" to "completion-standard-terminated (no separate sign-off)".
- [`governance/specification-audit-programme.md`](../../governance/specification-audit-programme.md): section 5 (the functional gate-category view) category-5 "Programme and index integrity" gate list gained gate 78 (backlog item-number permanence), which was in section 6 but absent from every section-5 category (a low coherence gap surfaced by the loop-break Sweep 126 as NOTE 4 and verified real at source). Version 1.17.35 to 1.17.36.
- [`.working/alert-and-assessment-triage.md`](../alert-and-assessment-triage.md): row 10 and its detail prose corrected: the stale factual claim that "the deep-assessment discipline terminates only on explicit maintainer sign-off" is now false, so the codex assessment's procedural sign-off request is closed and its substantive findings (items 4/5/8 OPEN, 6/7/9 PARTLY) are re-framed as validate-and-fix follow-ups rather than held for a sign-off ceremony.

### Discipline observation (apply-time correction)
The pre-push skeptical verifier (dispatched to a worker, refute-briefed) returned DO-NOT-SHIP on the first commit and caught a real multi-surface-incompleteness miss: the initial edits updated the SKILL but not its PAIRED slash-command [`.claude/commands/deep-assessment.md`](../../.claude/commands/deep-assessment.md), the actual entry point a future session reads, which still instructed "hold for maintainer sign-off". The verifier also flagged the stale triage row. A follow-on whole-repo grep (orchestrator) then found the README directory-tree description ("sign-off-terminated") the verifier had not named. All were fixed and the change re-verified. Lesson: a skill's change-impact surface includes its paired command AND the pack README's one-line skill description, not only the SKILL.md body; the whole-repo grep must run over the command and README surfaces too, not just the rule/skill/CLAUDE.md set.

### Scope note (why trust-recovery keeps its sign-off)
Trust-recovery is the REACTIVE escalation tier that runs after a discipline failure has put the maintainer's confidence in a window of work in question; its sign-off lets the maintainer declare that confidence restored, which is genuinely the maintainer's to grant and is not a QA-completion condition. Deep-assessment is the PROACTIVE counterpart with no lapsed confidence to restore, so only it drops the sign-off.

### State finding at directive time (accuracy)
The directive was prompted by an assistant misstatement that r1/r3 were "holding for sign-off." The register TABLE (authoritative Sign-off column) shows all five runs r1-r5 signed-off with findings actioned/routed; the "holding" text came from stale opening-snapshot PROSE superseded by the table. r5's one still-relevant corpus finding (Medium-1 SEF-02) was verified fixed at source (`security/sop-incident-escalation-matrix.md:97` now cites SEF-07, per #1022). So there were no genuinely-pending findings to re-fix; the deliverable is the standing discipline change plus the register cleanup.

### Verification
- The one corpus-document change is [`governance/specification-audit-programme.md`](../../governance/specification-audit-programme.md) (the section-5 gate-78 addition), Version 1.17.36 / Date 2026-07-27; taxonomy and portal/scorecard regenerated so gates 33/34 stay in sync. The remaining edits are the pack (rule + skill + command + README), CLAUDE.md, and gate-exempt `.working/` files.
- `lint-language` + `lint-unbalanced-fences` run on the edited pack-prose paths before commit: fences balanced; the only language finding is the pre-existing intentional dash-example line in CLAUDE.md's language-convention section (not introduced here).
- Gate 37 (dual-tree parity) exercised by editing both copies of the rule with byte-identical strings above the overlay. Gate 44 (paired-skill step-parity) holds: the command keeps step identifiers 1-8.
- Pack `1.65.17` to `1.65.18` (patch; no new rule or skill), README `1.10.63` to `1.10.64`, library `2026.07.702` to `2026.07.703`, spec `1.17.35` to `1.17.36`.
- Resume close-out bookkeeping batched in: the loop-break Sweep 126 row ([`validate-sweeps/history.md`](../validate-sweeps/history.md)) + cursor advance (gate 45); the #1212 [`merge-bypass-log.md`](../merge-bypass-log.md) back-fill row (surfaced by gate 50 once #1213 demoted #1212 from the highest-PR exemption).
- The pre-push skeptical verifier (offloaded to a worker, refute-briefed) UPHELD the change after the apply-time correction above; full audit suite green before push.

## 2026-07-27, Library Version 2026.07.702, PR #1212 (session-closing handoff)

Working-state-only session-closing handoff (no corpus or code change). Batches #1211's QA rows (its offloaded validate-pr returned HOLD with two accuracy residuals, both fixed here: a bare "fleet was OUT" assertion left in the #1209 row's Findings cell, and stale trailing `| pending |` cells on the reconciled rows), the #1211 retro row, and the #1211 merge-bypass row. Refreshes [`.working/session-handoff.md`](../session-handoff.md): the next-session queue is re-pointed to the maintainer's handoff-soon-plus-tooling plan (build a `close-out-pr` scaffolder (`tools/`, not yet created) from the pre-queued `research-3133-closeout-pr-b` delivery, then `/sitrep`), the green-at snapshot and D7 tokens move to `bc5eb00b` / `2026.07.702` / `1.10.63`, and the fleet-detection lesson plus correct short model names are recorded durably. RELEASES the concurrency lease ([`.working/session-state.md`](../session-state.md): `Status: released`, `Active-session: none`).

Per the loop-break, this handoff PR SKIPS its own trailing /validate-pr + /retro; the compensating control is the corpus-wide /validate the next /resume runs first, which cross-checks the Asserted-expectations block below. The fleet-detection correction is the session's notable lesson: three QA passes were self-run on a mistaken belief the fleet was capped, when the true cause was an operator model-name mismatch (full IDs vs the config's short names); a live dispatch confirmed the fleet available, and TODO 3.151 was opened to sharpen the ambiguous dispatch error message.

### Verification
- All 78 audit gates green; pre-push guard (both runners) green at `bc5eb00b` head. Working-state only.
- Handoff snapshot D7 tokens reconciled to the values this PR produces (`2026.07.702` / `1.10.63`).

## 2026-07-27, Library Version 2026.07.701, PR #1211 (QA-catch-up: six merged PRs whose validation never returned)

Working-state-only QA-catch-up (no corpus or code change). The #1210 pre-push verifier surfaced that five merged PRs, 1169, 1170, 1171, 1172 and 1179, carried validate-pr rows dispatched but never returned (their orders were declined on independence grounds or stalled in prior sessions), aged below gate 50's CHANGELOG-root window so unflagged. A consolidated six-PR post-merge validation was self-run inline via one in-session subagent, and #1210's own post-merge sweep was folded into it. **Correction (operator error, recorded honestly):** this was self-run under a mistaken belief the worker fleet was out. It was not, all accounts were available; the offload attempt used full model IDs (`claude-opus-4-8`, `gpt-5-codex`) while the account config offers models by short name (`opus`, `gpt-5.6-terra`), so `exec-dispatch` correctly reported "model not offered" and the operator misread it as "capped." The self-run results are valid and stand (the #1170 fail-open and the #1209 count error are real findings), but the work should have been offloaded; the correct short model names are recorded in the handoff and a follow-up sharpens the dispatch error message.

Result: five SHIP (1169, 1171, 1172, 1179, #1210), one HOLD (#1170). **#1170 HOLD-at-merge:** its three claimed fixes landed, but the new family-scoped worker fail-safe it introduced shipped a destructive-path fail-open (a `fable` worker on the `claude` runtime resolved to family `opus` alone, so [`tools/manage-workers.py`](../../tools/manage-workers.py) permitted a `restart` while it held live work), a second fail-open (an unclassifiable-prefix holder invisible to `family_holds`), and a false-refusal warning (ambiguous-all-free refused unconditionally). All three were already FIXED in #1172, re-verified at source by the orchestrator (`RUNTIME_FAMILIES` tuple, unclassifiable holder to `unknown`, ambiguous-all-free to `free`; [`tools/manage-workers.py`](../../tools/manage-workers.py) `--self-test` 53/53), so the defect is not live; the HOLD records the state the #1170 merge shipped. The five stranded rows are reconciled to RETURNED with their verdicts, the #1170 findings are recorded as FIXED #1172 in the disposition ledger, and #1210's post-merge validate-pr row (SHIP) is added.

### Verification
- Each stranded PR was validated READ-ONLY at its pinned merge SHA; the #1170 remediation was re-verified at source (self-test 53/53) before recording, not trusted on the subagent's report.
- All 78 audit gates green; pre-push guard (both runners) green.
...
- #1209's post-merge validate-pr had no delivery artefact; it was self-run inline via an in-session subagent under a MISTAKEN belief the fleet was out (it was not: an offload attempt used full model IDs against the config's short model names, so exec-dispatch reported 'model not offered' and I misread it as capped; corrected in #1211). Its result is batched into this PR's validate-pr row; the self-run result is valid and stands.
