...
## 2026-07-29, Library Version 2026.07.723, PR #1234

Migration PR2b-2b (`.working/` -> `grc_library_private/.working/`): the prose-reword half of the maintainer-directed sequence (reword all `.working/` references and copy the tree to the private sibling in one PR, keeping public `.working/`; the public delete and the TODO 3.177 close follow in PR2b-3). Per the two maintainer decisions this session: project-local references are repointed to the concrete private-sibling location as plain prose (unlinked, matching the #1229 sever precedent and the cross-repo-refs-are-plain-text convention); distributable pack references are genericized (a private prefix would leak the maintainer's structure and break the adopter story).

### Changed
- **Project-local repoint (68 references, plain prose):** [`.claude/CLAUDE.md`](../../.claude/CLAUDE.md) (28 references: 27 repointed content-pointers, including the 3 bare-filename-anchor pending-decisions links, plus the glossary/adopter-note rewrite), the ten slash-commands under [`.claude/commands/`](../../.claude/commands/) (29 references across claim-fit, deep-assessment, fitness, full-qa, guardrails, reference-audit, resume, retro, validate, validate-pr), and the four rule-overlay copies under [`.claude/rules/governance/`](../../.claude/rules/governance/) (11 references, all below the `PROJECT-OVERLAY` marker, so gate 37 byte-parity is preserved) now name `grc_library_private/.working/<subpath>`. The 7 CLAUDE.md exempt-set / `.gitattributes` / conceptual mechanism mentions are intentionally retained (they are not path-pointers that break on deletion).
- **Pack-skill genericization (23 references, 9 skills):** the working-state path references in change-tracking-write-entry, deep-assessment, deep-qa-review, guardrail-review, library-fitness-review, pr-retrospective, reference-audit, validation-sweep, and validation-sweep-pr-scoped are reworded to "in the consuming project's working state", matching the established 1.62.2/1.63.1 phrasing (e.g. the high-assurance-verification skill, already generic). The three adopt-skill references are intentionally left (they name the adopter's own reset target). Pack README bumped to `1.65.20` with a version-history row.
- **Glossary + adopter note:** the CLAUDE.md Structure-section `.working/` glossary entry now describes the private-sibling location, and the former "adopters can delete `.working/` outright" sentence (which goes false once the public tree is deleted) is rewritten to state that the working space lives only in the maintainer's private sibling and is not required to use the corpus or run the public gates.

### Batched bookkeeping
- #1233's post-merge `/validate-pr` row (SHIP-with-nits; 3 fixes confirmed at ground truth, W-1 note-tally + N-2 docstring-fallback FIXED, N-1 #1232-CI verified), its `/retro` row, and its `--admin` merge-bypass row. The Sweep-129 history row's note tally is corrected `4`->`5` with N2-N5 enumerated (the W-1 fix).

### Verification
- Gate 37 (claude-rules-sync) green (rule-overlay repoints are all below the marker; pack bodies unchanged). Zero residual public-`.working/` content-pointers across CLAUDE.md, the commands, and the rule overlays (grep-confirmed). The `.working/` tree copy to `grc_library_private/.working/` is the companion `_private` commit. Pre-push guard (both runners) green.
...
### Fixed
- **Three PR-time delta gates re-scoped** so a normal PR does not fail once `.working/` leaves the public repo: [`tools/check-changelog-on-pr.py`](../../tools/check-changelog-on-pr.py) and [`tools/check-todo-rotation-on-pr.py`](../../tools/check-todo-rotation-on-pr.py) drop the `.working/`-file half of their two-part requirement (the detailed CHANGELOG mirror, [`.working/DONE.md`](../../.working/DONE.md)) once the in-repo `.working/` is gone (it becomes a cross-repo concern invisible to the public diff), keeping the public half (root CHANGELOG, [`TODO.md`](../../TODO.md)); [`tools/check-handoff-snapshot-on-pr.py`](../../tools/check-handoff-snapshot-on-pr.py) (D7) prints an explicit N/A rather than silently never triggering. Each keys on a local `_working_in_repo()` check, so pre-move behaviour is identical.
- **[`tools/audit-cross-repo-references.py`](../../tools/audit-cross-repo-references.py)** private-tree walk: added a `link_root` parameter so the post-move private-`.working/` walk resolves markdown links against the PUBLIC repo root (via the in-repo-equivalent source), because a `.working/` file's upward relative links point at the public corpus, not the private sibling. Without it, every such link became a false actionable finding.
- **Four #1230 CHANGELOG inaccuracies**: the root entry said "5 tools" (8 were rewired); the detailed entry had a malformed nested markdown link `[[...](...)](...)` for the statusline wrapper, a duplicated "Pre-push dual-family" lead phrase, and "2 tests" (3 were added).

### Changed
- **[`tools/lint_common.py`](../../tools/lint_common.py)** `resolve_working_for_write`: docstring now states the migration residue (a partial copy with both `.working/` trees present splits new-vs-existing writes; safe only when the copy is complete, which the deletion-step QA enforces). No behaviour change; the both-dirs split is correct given the complete-copy plan.
...
Migration PR2b-1: rewired the remaining `.working/`-consuming tooling (the non-gate half; the nine `.working/`-reading gates were done in #1225 and #1227) so it keeps working across the `.working/`->`grc_library_private/.working/` content move that lands in PR2b-2. Non-breaking: every consumer resolves the current `.working/` location at runtime.

### Added
- `lint_common.resolve_working_dir()` (the `.working/` DIRECTORY, private-preferred, for tree-walkers) and `resolve_working_for_write()` (a never-None write destination that tracks the CURRENT `.working/` home, so a new file lands beside its siblings and its relative links resolve). Both unit-tested (3 new cases in `ResolveSiblingTests`, resolved-path expectations, verified binding).
- [`tools/statusline-next-pr.py`](../../tools/statusline-next-pr.py): a private-aware `next:` statusline wrapper (resolves [`.working/next-prs.txt`](../../.working/next-prs.txt) via `resolve_working`), with a 4-case `--self-test`.

### Changed
- **Readers** repointed to `resolve_working` (private sibling preferred, in-repo fallback): [`tools/decisions-search.py`](../../tools/decisions-search.py), [`tools/preflight-changelog.py`](../../tools/preflight-changelog.py) (full-mirror link scan), and 3 hooks ([`.claude/hooks/block-on-open-findings.py`](../../.claude/hooks/block-on-open-findings.py), [`.claude/hooks/block-askuserquestion-unattended.py`](../../.claude/hooks/block-askuserquestion-unattended.py), [`.claude/hooks/block-answered-question.py`](../../.claude/hooks/block-answered-question.py)) via a fail-safe import block that degrades to the in-repo path if `lint_common` is unavailable.
- **Writers** repointed to `resolve_working_for_write`: [`tools/manage-workers.py`](../../tools/manage-workers.py), [`tools/audit-reference-breadth.py`](../../tools/audit-reference-breadth.py), and [`tools/pr-closeout.py`](../../tools/pr-closeout.py) (its three ledgers, the validate-detail file, and three latent-crash `relative_to(REPO_ROOT)` guards for a private-resolved path).
- **Tree-walkers** [`tools/residual-scan.py`](../../tools/residual-scan.py) and [`tools/audit-cross-repo-references.py`](../../tools/audit-cross-repo-references.py) now also scan the resolved `.working/` dir when it lives outside the repo (post-move), re-rooting private paths to `.working/...` rels so classification/coverage is preserved; pre-move they add nothing (the repo walk already covers the in-repo tree).
- **[`tools/tension-scan.py`](../../tools/tension-scan.py)** resolves its `.working/` targets at runtime and skips any that resolve outside the public repo (git-diff cannot reach the private sibling), scanning [`TODO.md`](../../TODO.md) plus in-repo targets with an honest skip note.
- **[`.claude/settings.json`](../../.claude/settings.json)** statusLine repointed to [`tools/statusline-next-pr.py`](../../tools/statusline-next-pr.py) (one line changed), fixing a latent parse bug where the old inline regex mis-parsed the `1)`-form first queue line.
- **`resolve_working_for_write`** now tracks the CURRENT `.working/` home (via `resolve_working_dir`) rather than eagerly preferring the private repo dir, so a new file is not written to the private sibling while its siblings are still in-repo (which would split the tree and break relative links pre-move). Caught by the `pr-closeout` dry-run during this PR.

### Verification
- All 78 audit gates pass; [`tools/run-linter-regression.py`](../../tools/run-linter-regression.py) rc=0; every rewired tool/hook self-tests green (`pr-closeout` 47/47, `audit-cross-repo-references` 5/5, `statusline-next-pr` 4/4, 3 new helper tests).
- #1229's (PR2a) post-merge dual-family `/validate-pr` (claude + codex) was consumed here: both NO DEFECTS; its history row + detail file, retro row, and merge-bypass row are batched in.
- Pre-push dual-family skeptical verify (claude + codex, distinct accounts): NO blocker/high/medium. Claude raised 2 LOW (a post-move classification flip in the cross-repo-reference audit's private-tree walk, and a cosmetic stale print label in the worker manager) and codex flagged that the writer resolver test did not bind the split-tree fix. All three FIXED in-window (a `rel_root` re-rooting parameter so a private `.working/` file keeps its intended-minimal classification, a resolved-path print, and a new no-split test case that proves a new file resolves in-repo while the private `.working/` is absent) and re-verified (self-tests + gates green).
