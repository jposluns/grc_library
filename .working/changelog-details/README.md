...
The in-repo `CHANGELOG-detailed.md` keeps only the **current week's** entries; completed weeks are swept to the `grc_library_private` archive as weekly Monday-dated files (`YYYY-MM-DD_detailed.md`, keyed to the week's Monday). This keeps the in-repo mirror small and browser-openable while the full audit trail accumulates in the private companion repo (and remains in this repo's git history regardless). The write path is unchanged: new entries still prepend to `CHANGELOG-detailed.md`; only completed weeks leave it.

- **Sweep tool:** [`tools/sweep-working-records-to-private.py`](../../tools/sweep-working-records-to-private.py) (not an audit gate; an orchestrator follow-up step). It is data-safe: `--emit-archive` writes the weekly archives to the `grc_library_private` archive, and `--prune` refuses to remove or rewrite anything in this repo unless `--verify-archived` confirms every artefact already exists in the archive (each rewrite also guarded by a re-parse assertion).
- **Gate 59 (mirror header-parity):** its cutoff is now a dynamic floor, `max(CUTOFF_PR, oldest PR still in the in-repo mirror)`, so a swept (now archive-only) entry is correctly out of parity scope rather than flagged missing, while a genuine in-window drift still fails.
- **Rollout:** the machinery (this tool + the dynamic gate) landed first (grc_library #708), the initial sweep of completed weeks ran next (the in-repo mirror was pruned to the recent/current-week window; older weeks moved to the archive, scratch #113 / TODO 3.16 PR 2; the sweep destination is repointed to the `grc_library_private` archive for the §1.19.9 dated-archive sweep), and the per-PR sweep-step wiring landed as an advisory close-out step (grc_library #813). The remaining step is the root reformat and compression (TODO 3.16 PR 3+, post-deep-assessment). The full audit trail remains in this repository's git history and the archives regardless.
