...
- **Claim precedence** (each poll, as the serve loop instructs): among `pending` orders the worker is eligible for (`egress-needs` and `capability-needs` satisfied), a priority-1 / blocking order outranks the role preference (a per-PR `/validate-pr` or the resume `/validate` is taken by whichever worker frees first, regardless of role, so urgent QA is never starved behind a role rule); otherwise the worker prefers an order matching its `role` kind and falls back to any eligible order when its role's queue is empty. An `any`-role worker just takes the highest-priority claimable order.
