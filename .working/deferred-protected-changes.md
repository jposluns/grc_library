...
- **QA:** guard-first (land the gate green on current skills) + two HA verifiers; full 69-to-70 count-ripple bare-token grep. Rotate §3.12 to DONE on apply.

### 10. [TODO §3.80 phase 3] Credit-offload orchestrator-side wiring

- **Why deferred / staged (2026-07-15 overnight, maintainer-directed):** phase 3 of the credit-offload build (design of record: [`credit-offload-design.md`](credit-offload-design.md)) wires the orchestrator side into `.claude/commands/resume.md` and `.claude/CLAUDE.md`, both protected AND load-bearing (the `/resume` flow). The maintainer chose to STAGE this for morning review rather than apply the resume-flow change unattended. **On overnight exit, the assistant pauses and reminds the maintainer to read this staged item** (this file is `.working/deferred-protected-changes.md` in the `grc_library` repo). Phases 1-2 (the scratch queue protocol + the `/credit-offload` worker command) are built separately on `grc_library_scratch`, so a worker is testable tomorrow independent of this phase-3 apply.
- **Protected edits to apply (at daytime, after review):**
  - `.claude/commands/resume.md`: add a credit-offload step, at resume, read the scratch `workers/` liveness registry; if >=1 live worker, ENQUEUE the loop-break `/validate` (Sweep N) as a blocking credit-offload order (pinned to the resume `main` SHA) and wait (poll the results plane) for the finding set, then consume + re-verify positives + record the sweep row, exactly as if run inline; if 0 live workers, self-run the `/validate` as today. Also add a queue/results check to step 3 (alongside `audit-brief-freshness.py` and `audit-delivery-status.py`) so the orchestrator sees any pending credit-offload results at resume.
  - `.claude/CLAUDE.md`: add a `## Credit-offload mode` section (or fold into `## Multi-session orchestration` + `## Attended-autonomous operating mode`) codifying: the orchestrator enqueues offloadable passes when >=1 worker is live and consumes at PR boundaries, self-runs on an empty pool (best-effort, never a QA skip); re-verify positive findings, trust clean results (the worker result carries the subagent returns as proof), and the orchestrator writes the audit-trail rows; mandatory-QA discipline is unchanged; the blocking-resume-`/validate` rule; a pointer to [`credit-offload-design.md`](credit-offload-design.md). If this adds a user-facing count-bearing collection, run the gate-39 / count-consistency ripple; the credit-offload worker command lives in `grc_library_scratch` (`.claude/commands/`), NOT grc_library, so it does NOT change the grc_library command count.
- **Non-protected companion (may apply in the same daytime PR or separately):** any orchestrator-side consume/enqueue helper (a `tools/` script) if one is warranted beyond prose; the `## Multi-session orchestration` runbook cross-reference to the design doc (a `.working/` edit, non-protected, already landable).
- **QA:** pre-push guard + a skeptical verifier (a resume-flow prose change); confirm `/resume` still reads coherently end-to-end and the credit-offload directive is internally consistent with the design doc and the mandatory-QA discipline. Rotate the §3.80 phase-3 sub-item on apply.
