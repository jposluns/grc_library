...
### 12. [maintainer-directed 2026-07-17] Codify the QA-activity completion standard in CLAUDE.md + the pack

- **Why deferred (2026-07-18 overnight):** the codification is a standing discipline that belongs in the protected `.claude/CLAUDE.md` (and, for portability, a pack rule). The durable form already landed non-protected in the orchestrator memory `qa-findings-are-top-priority`; the CLAUDE.md + pack halves are the protected apply.
- **Prepared content (apply at daytime):** add a short `## QA-activity completion standard` subsection near `## Throughput pressure does not authorize QA abbreviation` in `.claude/CLAUDE.md`: "A QA activity (a `/validate`, `/validate-pr`, `/matrix-fit`, `/claim-fit`, `/reference-audit`, `/screen-publications`, `verify`, `/fitness`, `/full-qa`, or `/deep-assessment` pass) is COMPLETE only when all five hold: (1) it ran in a sanctioned formal shape (never an abbreviated / spot / memory-only substitute, per `## Throughput pressure ...`); (2) every finding is triaged to a terminal disposition, fixed in-window OR routed to the backlog / `pending-decisions.md` with a severity tier (and a morning-review flag for a risk item), none dropped; (3) worker-delivered POSITIVES are re-verified at source before routing (a clean zero-finding result is trusted on its proof-of-run, per the credit-offload consume discipline); (4) the history row is recorded (a zero-finding run still gets its row); (5) any deferred (risk / authorial / irreversible) fix is documented for review, not silently left. The `/deep-assessment` and `/trust-recovery` tiers add: terminate only on explicit maintainer sign-off (an empty finding set is presented for sign-off, never self-declared done). Standing priority: fixing known QA issues outranks build / tooling / content; complete the then-current task, then fix."
