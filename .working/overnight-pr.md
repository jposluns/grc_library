...
- PR 4 (#556): W3 / GR-3 wave 1 + GR-4 tilde half: shared `parse_metadata_block()` + `parse_iso_date()` in lint_common, gate 31 migrated to fail-loud malformed-Date semantics (blast radius verified ZERO: 338/338 corpus files parse identically old-vs-new before wiring) with skip-count observability, `iter_non_code_lines` tilde-fence toggle, 7 new tests (suite 247 to 254), the section-5 spec clause amend (Version 1.16.29 + regen), GR-3/GR-4 reduced to residuals, and the batched #555 QA rows (0 findings).
