...
- **FR-63 (§2.7, adoption worked example) in flight as #697** on `2026-07-08-fr63-adoption-worked-example`: authored the new [`docs/worked-example-adoption.md`](../docs/worked-example-adoption.md) from a fresh-context research delivery (the 77-commit-stale scaffold reconciled), wired via mutual Related-Documents pointers (the five-path entry-point-list cascade correctly avoided per the fresh re-read), closes §2.7 / FR-63; carries the batched #696 QA rows. The egress-PRIORITY items remain blocked (GR-GAP-1 on iso.org upstream version confirmation; the 11 legislation deepenings on the maintainer's egress instance), so FR-63 was the sanctioned non-legislation fallback.
