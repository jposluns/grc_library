...
4. **Protected-file changes are DEFERRED in overnight mode (maintainer-directed 2026-06-30, mid-run).** Do NOT work items that require changing **protected files such as [`.claude/CLAUDE.md`](../.claude/CLAUDE.md) and the governance pack ([`dev-security/claude-rules/`](../dev-security/claude-rules/), [`.claude/rules/`](../.claude/rules/))** while in overnight mode: the maintainer must manually approve those and is asleep. (The #478 §4.8 CLAUDE.md codification was already in flight when this was raised; the maintainer approved finishing #478 specifically, then directed deferral of anything else needing explicit approval.) **Deferred until attended mode:** the §4.8 residual (compute-don't-ask into the `clarify-before-acting` pack rule; the new-skill-drafting checklist; the count-gate word-form remainder) and any other pack / `.claude/` / rule-governance change. **In overnight scope (not protected):** corpus content documents (the FR items, deepenings, FR-58, FR-167 matrix), corpus corrections, and the routine version / CHANGELOG / TODO / DONE / `.working/` bookkeeping those require.
