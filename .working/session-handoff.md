...
5. **Runs a full corpus-wide `/validate` as the first substantive task** — the loop-break compensating control for whatever session-closing handoff PR last closed (which skips its trailing `/validate-pr` + `/retro`). The next such sweep is **Sweep 40** — the control for the session-closing handoff PR **#322**, covering the #320 (DD-8 CPPA scrub) and #321 (FR-140) deltas plus this handoff PR's working-state changes. (Sweep 39 ran at the prior resume, covering the #313/#314 deltas, close-out #315.) Routes any findings to the backlog.
...
- **`/validate` cadence**: **Sweep 39** ran at the prior resume (#313/#314 deltas, close-out #315). The next `/resume` runs **Sweep 40** — the loop-break control for this session-closing handoff PR #322 (which skips its own trailing `/validate-pr` + `/retro`), covering the #320/#321 deltas. Routes findings to the backlog.
