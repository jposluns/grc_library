...
1. Consume `vpr-1199b` (disposition #1199 findings) + the `3.133` candidate. Apply the `3.145` fail-closed candidate as its own PR (skeptical-verify, merge). This GATES the ENABLE.
2. Enable concurrency>1 ONLY after 3.145 merges: jposluns-work (claude) + jeff-mailz (codex) to `max_concurrent` 2; keep the OTHER of each family at 1 as the never-zero fallback. Then run the duplicate-real-task concurrency-validation protocol (roadmap A). WATCH for auth errors; worst case, maintainer re-logs-in in the morning.
3. Apply umbrella-A worker deliveries impl-3111 (independence dispatch) + impl-3131 (per-worker logging): RE-DISPATCH fresh against the post-3.145 SHA (their earlier candidates are stale, exec-dispatch.py changed under them).
4. Reconcile the ECC assessment (`grc_library_private/ecc-project-assessment.md`) into TODO where useful.
5. P3 close-out tooling: apply the `3.133` candidate, then 3.134 / 3.136 / 3.137.
6. The P1-on-hold 1.26.x series and umbrella A-H proceed per their gating; roadmap `grc_library_private/roadmap.md`.
