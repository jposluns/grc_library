...
- **Current truth (verify against live files at `/resume`, which now runs from `/home/grc`)**:
  ...
  - **Version snapshot (D7 validates these tokens):** library `2026.07.685`, README `1.10.46` (reconciled to #1195 at the 2026-07-26 post-wind-down resume; the takeover-lineage session merged #1181 through #1194); pack `1.65.17` (unchanged). Counts re-verified this session (guardrail-review r16, worker-measured plus orchestrator-re-verified): gate **78**, rules **15**, skills **24**, commands **16**, Document-types **18**. The commands figure was **15** and is now 16 (`/restore-broken` landed in #1183).
