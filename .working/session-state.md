...
**Last-heartbeat-UTC:** 2026-07-19T01:12:00Z

**Current-task:** ACTIVE, OVERNIGHT-UNATTENDED (2026-07-19 resumed session, `/resume` from #1039; on the VM, gh-CLI, no GitHub MCP; transitioned attended-autonomous -> overnight-unattended ~01:12Z, maintainer going to sleep for ~10h; do NOT end overnight on a timeout). Lease ACQUIRED. #1040 = the Sweep 113 loop-break close-out (PASS for #1039). #1041 = the answered-question guardrail (§1.22.6, block-answered-question.py + decisions-search.py + CLAUDE.md discipline) after the 2026-07-19 re-ask-already-decided failure. ALL overnight decisions batched + recorded (pending-decisions.md 2026-07-19 block): §1.14 confirmed, §1.1 design-overnight, §1.18 scope+D8-overnight, §3.68/69/70/74 + §3.71 recorded, stricter-safe-where-defensible on the rest. Overnight queue: §1.22 block (guardrail hook / README fix / daily-tier CHANGELOG / ref-existence tool / .working cycle-out / §1.22.7 restructure) -> §1.16 COBIT normalization (GO) -> §1.1 -> §1.18 -> content forks -> P3. DEFERRED to AM: §1.19.12, §1.19.13 (maintainer-gated). Both workers Opus 4.8 LIVE. **This session's maintainer-directed work block (privatization/guardrail tightening, outranks the queue):** (1) wrong-repo guardrail = a probe-based PreToolUse hook (no list; self-discovers sibling `tools/` locations; blocks with the correct repo); (2) `.ref`/`.scratch`/`.private` README fallback-wording fix (placeholder is an adopter signpost + hard-gated empty marker, never a maintainer content fallback); (3) `.working` cycle-out to `_private` = generalize the §1.19.9 sweep tool + an advisory staleness reporter + a session-boundary dedicated cleanup PR policy + an initial sweep of clearly-done content. **`.project-governance/` = KEEP as-is** (maintainer confirmed after reviewing the design + gate-53-retirement cost; the move was NOT worth losing the public exemplar/spec/gate). THEN the prior-locked queue: §1.19.12 (CLAUDE.md sensitivity-trim apply, SENSITIVE, classification locked) -> §1.19.13 (history scrub, MAINTAINER-GATED, LAST) -> §1.19.10a + task #4 + P1. **`pending-decisions.md`: `:333` SEF-06/SEF-07 apply pending (maintainer-decided), else none open.**
