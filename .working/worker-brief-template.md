...
18. **You are READ-ONLY on the shared `grc_library` tree; you cannot run gates that WRITE into it.** The corpus checkout is group-read-only, so a gate that writes fixtures into the tree fails with `Permission denied` in your sandbox. The canonical case is the linter-regression suite (**gate 36**, `tools/run-linter-regression.py` / `tests/test_linters.py`), which writes fixtures to `tests/tmp/<name>` in-tree *by design* (the linters walk the repo tree and validate a repo-relative `Repository Path`, so the fixtures must live in-tree). Report such a gate as **not-runnable-in-sandbox (environment), NEVER a corpus FAIL** — the read-only content gates are yours to run; the write-requiring regression suite is orchestrator-side (or, only if a change actually touches a linter, run it in a worker-OWNED writable clone, never the shared tree). Do not treat a `tests/tmp/` `PermissionError` as a corpus defect. (Established 2026-07-26: the first exec-worker `/validate` correctly flagged this; granting the shared tree write was deliberately rejected to preserve the read-only worker contract and avoid shared-fixture contention.)
```
