...
The [`guardrails/`](guardrails/) pack is library-original content and falls under the same CC BY-SA 4.0 terms. The same attribution applies whether an adopter uses the corpus, the pack, or both; the pack does not carry a separate licence and adopters using only the pack should cite the parent library (see [`AUTHORS.md`](AUTHORS.md) "How to cite").
