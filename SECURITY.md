...
This repository contains documentation, not executable software, so the conventional notion of a "security vulnerability" does not strictly apply. The reporting paths below cover the issues that matter for a public-domain governance library:
...
- **CC0 licence concerns.** A document appears to reproduce restrictively-licensed third-party text without permission, or a contribution otherwise raises a CC0 compatibility concern.
...
For sensitive disclosures (for example a CC0 licence concern with potential legal implications), open the issue with the minimum public detail required to identify the artefact and indicate in the body that further detail is available on request.
