...
- **(c) (L, XS) CLAUDE.md widening (authorized-touch bundle).** Widen the RM-10 clause from
  ...
  The GR-8(b) closure pass (#628) staged five further clauses on this same authorized-touch
  bundle: the #593 bare-token fold into the section-close checklist line; the #594
  refutation-search extension of the bare-token contradiction-grep guard; the #612
  counts-timing rule for the prose-count line (counts are computed AFTER the verifier loop
  closes, from the suite run or the diff, never during drafting); the #625 grep-claim
  fidelity line (any record or CHANGELOG clause characterizing a grep result is written
  FROM the pasted output at authoring time, never from memory of an earlier run); and the
  #620 CHANGELOG count-reflex line (when a figure in a drafted entry changes during
  verifier rounds, bare-token grep the whole entry, all sections, before push).
