...
Stand up the INPUT half of the multi-session capability (its codification closed as section 3.6 in the DONE ledger) so separate-session / separate-account workers can pick up research tasks without waiting on the orchestrator. The 2026-07-03 maintainer direction expanded the original stage-on-dispatch design to STANDING WHOLE-BACKLOG COVERAGE: every TODO item has, in scratch, either a worker-ready brief or a recorded not-eligible verdict, kept current through the normal PR close-out. The settled design (the maintainer's three answers plus the accepted adjustments: eligible-only briefs with verdicts for the rest, pointer-light brief content, a `Verified-against:` freshness stamp keyed on the main-repo PR number, artefact-derived state with no status fields, TODO-authoritative with briefs as a wipeable derived projection, TODO-delta-batched sync, targeted-first refresh with an advisory cross-repo freshness check) is recorded in [`.working/design-decisions.md`](.working/design-decisions.md) under "Worker-ready brief staging" and operationally in the runbook [`.working/multi-session-orchestration.md`](.working/multi-session-orchestration.md) subsection 5.1 (both shipped with slice 1, 2026-07-03). Remaining build slices:

1. **Slice 2, scratch machinery**: the `research/<work-unit-id>/brief.md` convention in [`grc_library_scratch`](https://github.com/jposluns/grc_library_scratch) (pointer-light per the design: task statement, exact main-repo target paths, verified-disjoint partition, `path:line` evidence requirement, deliverable shape, contract pointer; no quoted lines, version numbers, or counts), the `research/COVERAGE.md` index (every TODO item mapped to its brief or its not-eligible verdict with reason), the `Verified-against:` stamp, the scratch `validate.py` extension shape-checking briefs and the index, and the scratch `WORKER-ONBOARDING.md` / worker `CLAUDE.md` pointer updates.
2. **Slice 3, main-repo wiring**: the close-out checklist pairing line (a non-empty TODO delta pairs with a queued-or-done brief sync plus an eligibility verdict for each new item), the `/resume` freshness-check step, and the advisory cross-repo freshness tool (target paths exist on `main`, TODO item exists, stamp-age report; orchestrator-side, not a CI gate, because neither repo's CI can see the other).
3. **Slice 4, the `/subagent` slash command** as the external-worker entry point (read the assigned brief, claim it in `claims-ledger.md`, read the named main-repo files read-only, produce findings, deliver to `inbox/`, stop; **read-only-on-main MUST be enforced by the worker GitHub account's permissions, not by the prompt**, a prompt is not a security boundary per [`.claude/rules/secrets.md`](.claude/rules/secrets.md)) plus the maintainer-facing quick start in the runbook (provision the account, point the worker session at scratch, assign a brief, collect from `inbox/<worker-id>/`; maintainer-suggested 2026-07-02).
4. **Slice 5, seeding**: whole-backlog coverage executed in priority-tier-sized scratch PRs (guard-first: the slice-2 gate extension lands before the briefs it validates). Brief drafting is itself partitionable research (workers may draft; the orchestrator verifies every brief and authors every eligibility verdict).
