...
### 1.12 Annual-review procedure domain-enumeration completeness (architecture) (annual-review-domain-scope, L, XS)

`governance/procedure-grc-programme-management-and-annual-review.md:29` (§2.1 Scope) carries a "applies to all documents held in the GRC library, spanning the following domains" enumeration that lists 10 domains and OMITS **architecture**, while its two sibling "spans all governance domains" enumerations both carry 11 including architecture: [`governance/standard-maturity-assessment-methodology.md`](governance/standard-maturity-assessment-methodology.md) line 34 (the authoritative methodology standard) and [`governance/framework-governance-performance-and-improvement.md`](governance/framework-governance-performance-and-improvement.md) line 27 (fixed in #685). `architecture/` is a real corpus domain dir with documents, so a procedure asserting it spans ALL library documents cannot omit it. **Fix**: add `architecture` to procedure line 29 (align all three carriers at 11 domains), bump the Procedure `Version` + `Date` in the same commit, and regenerate `taxonomy.yml` then [`docs/portal.md`](docs/portal.md) / [`docs/maturity-scorecard.md`](docs/maturity-scorecard.md). **Provenance**: #685's owed `/validate-pr` Subagent A finding A-1 (2026-07-07), out-of-window (pre-existing; #685 fixed only the framework carrier, and its Sweep-87 record wrongly asserted "no parallel enumeration carrier survives"). Orchestrator apply-time-verified the finding at source and confirmed the class is bounded to exactly these three enumerations (a corpus-wide family grep: only procedure line 29 omits architecture; the other domain-word prose lists such as README line 36 and `procedure-control-testing.md` line 21 are deliberately-scoped, not the "spans all domains" completeness class). This is the first queued corpus fix for the next (NUC) session, after its `/resume` corpus-wide `/validate`.
