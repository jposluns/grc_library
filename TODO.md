...
- **Project-governance separation: directory-scan-scope parity meta-check (M, S)** (surfaced by the #338 `/retro`, the mechanical backstop §7.4 currently states only in prose): a meta-check that enumerates every `tools/lint-*.py` / `check-*.py` selecting scan roots from an explicit `DEFAULT_SCAN_PATHS`/`DOMAINS`-style constant and asserts each includes every audited-not-exempt top-level directory (currently `.project-governance`). Generalizes the two per-gate scope-coverage regression tests added in #338 (gates 31 + 10) so the next "added the dir to the linters I happened to see" miss is caught mechanically; the #338 sweep showed this class recurring (the #336 migration under-enumerated the content-linter set, leaving gates 31/10 skipping the dir). Pairs with the §7.3 directional-dependency gate and the §4.6/§4.10 bookkeeping-parity gate family; candidate home is a new meta-gate or an extension of the gate-35/gate-39 parity family.
