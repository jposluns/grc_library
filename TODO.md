...
**PARTIAL: the dedupe-vs-pack half landed in #1251** (5 pack-duplicate CLAUDE.md sections -> pointer + overlay, 1506 -> 1400 lines). REMAINS (3.139.3b): relocate the 5 cadence sections (matrix-fit / claim-fit / deep-assessment / reference-audit / screen-publications) into each skill's project-wiring (augment the skill's cadence with CLAUDE.md's project-specific triggers FIRST, then trim CLAUDE.md to a pointer). Map: `grc_library_private/.working/dedup-3139-3/DEDUP-MAP.md`. Deferred to later (map): the Tier-3 wind-down-framework cut (riskiest) + the maintainer-GATED demotion of the loud '## Anything wrong' section (ASK first).
Slim the 120-line `## Security and governance requirements` to a one-line index of the 15 pack rules (it currently re-describes them); reduce the cadence-SOP prose (matrix-fit / claim-fit / reference-audit / screen-publications / deep-assessment) to one-line pointers to their existing skills; move the worker-mechanics prose (delivery-tray, pin-to-SHA, codex-single-shot, guard-inputs) to `_private` as project-only operational. Guard-validated.
