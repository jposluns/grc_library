...
### 1.10 Quebec Law 25 72-hour breach-clock confabulation sweep (H, S) — added 2026-07-04 (#631 verifier catch)

The corpus attributed a 72-hour CAI breach-notification clock to Quebec Law 25 in nine carriers across six documents, but the statute (CQLR P-39.1, s. 3.5) sets a promptness standard ("must promptly notify the Commission"), no fixed hour-count: confirmed against BOTH the held text (`grc_library_scratch/ref/legislation/Canada/Quebec-Law25-PI-Private-Sector-CQLR-P-39.1--full-text.md:307`) and the live consolidated Act at LegisQuebec (zero "72 hours" occurrences), 2026-07-04. The two Canada-annex carriers were fixed in #631 (the touched file); the seven remaining carriers across five untouched documents are [`dev-security/standard-security-baseline-and-standards-reference.md`](dev-security/standard-security-baseline-and-standards-reference.md) (lines 103 and 198), [`dev-security/standard-security-quick-reference.md`](dev-security/standard-security-quick-reference.md) (line 280), [`privacy/charter-privacy-management-programme.md`](privacy/charter-privacy-management-programme.md) (line 185), [`privacy/procedure-data-protection-and-privacy-breach-response.md`](privacy/procedure-data-protection-and-privacy-breach-response.md) (lines 174 and 321), and [`security/procedure-security-incident-response.md`](security/procedure-security-incident-response.md) (line 177); line pointers as of the #631 state. Single-session value-correction sweep (not partitionable, not worker-eligible); re-run the bare-token grep (`72` x `Quebec|CAI|Law 25`, both orders, proximity form) at apply time rather than trusting these pointers. This is the FR-120 "attributed value" class at corpus width; fold a `/claim-fit` per-batch row when the sweep ships. Two recorded companions ride the sweep (#631 `/validate-pr`): the same-file regulator rename in the breach-response table (line 177: the sole remaining spelled-out "Autoridade Nacional de Protecao de Dados" carrier after #631 re-described the ANPD as the Agencia per Lei No. 15.352/2026), and a disposition note for the historical FR-141 CHANGELOG entry whose "Quebec Law 25 ... verified accurate" clause is superseded by this item's refutation (frozen history: record the disposition, do not edit the entry).
...
- **(GR-4 residual, DECIDED 2026-07-04: standalone gate; now a build item, L, S) Unbalanced-fence detection gate.** The maintainer chose the tiny-standalone-check option at the 2026-07-04 morning round: build `tools/lint-unbalanced-fences.py` as its own gate (an UNBALANCED fence silently suppresses scanning of everything after it; the `~~~` toggle itself shipped in the W3 PR and the corpus census found zero unbalanced fences, so the gate is preventive). Standard shape: four-surface wiring + regression fixture.
...
- **(#629 retro, DECIDED 2026-07-04: build, L, S) Snapshot-line freshness check.** The maintainer-accepted mechanical graduation of the append-not-reconcile class (FOUR occurrences: #619 F1, #622, #628 F2, #629 a1; the prose checklist line did not extinguish it, per the GR-P3 thesis): a check that parses the session-handoff Current-truth snapshot line's backtick-quoted version tokens (library CalVer, README Version, pack, register and history versions) and compares each against the named live file's header, failing on mismatch. Shape consideration for the build: a PR-time delta check (fires only when the diff touches [`.working/session-handoff.md`](.working/session-handoff.md), validating the refreshed snapshot against the same tree) fits the invariant better than a standing gate, because the snapshot legitimately drifts forward between refreshes (its own preamble says so) and the close-out refresh batches into the NEXT PR by recursion-avoidance, so merged `main` lags by one PR by design; the invariant is snapshot-fresh-at-refresh-time, never snapshot-fresh-always. Version tokens only (mechanical, false-positive-free); prose claims like the a1 "IN the diff" clause stay convention-guarded. Queue with the other decided small builds behind the P2 applies.
...
- **(QA-2026-07-04 GR-DRIFT-1, L-M, S) Gate-54 four-surface rename.** The parity-surface name "Per-document control-code validity audit" under-describes the gate's NIST-CSF-2.0-only scope (its docstring says so; sibling gate 58 carries its ISO qualifier), inviting a false-coverage belief from the section-6 table. Rename to "Per-document NIST CSF 2.0 control-code validity audit" across the four parity surfaces in one PR (gate 35 locks them in step). Name APPROVED at the 2026-07-04 morning round; mechanical build.
- **(r4 D-F2, [guardrails], L, XS) Guardrail-review SKILL cadence clause.** The SKILL's auto-prompt description implies unbounded maintainer deferral while gate 60 bounds deferral at drift 3 (warn below, fail at threshold; r3 and r4 both fired this way), and the SKILL's mechanical-gates bullet never names gate 60 itself. One clause in the auto-prompt bullet (mirrored per gate 44 if step text changes).
- **(r4 D-F3, [guardrails], L, S) Evidence-grounded-completion one-liner corollaries.** All four enumeration surfaces (pack CLAUDE.md, pack README tree blurb and scope row, project CLAUDE.md index bullet) omit the rule's un-observable-state / inventory / external-version-currency corollaries, which other project surfaces cite by name; the r3 D-3 fix shape (one clause per surface), extended to this rule. Pack-surfaces PR; the project-CLAUDE.md clause rides the NEXT authorized CLAUDE.md touch (the #629 bundle did not carry it, so the pack-surfaces PR queues the protected-file clause for that future touch).
- **(r4 G-3, [guardrails], L-M, S) D-check prose-presence coverage.** No gate enforces section-6.1 presence for delta checks (gate 64 is numeric-gate-only; the parity registry is exclusion-only, so a stale entry for a removed Dn dies silently); either extend gate 64 to parse the D-row table and require the per-check narrative opener plus a registry-to-table cross-check, or adjudicate accepted-residual (D checks are rare and D6 landed with its prose). Maintainer scopes. DECIDED 2026-07-04 morning round: extend gate 64 (parse the D-row table, require the per-check narrative plus a registry-to-table cross-check); build item.
- **(r4 G-4, [guardrails], L, S) Binding-sentinel stability.** The `BINDING_SENTINEL` sentence gates 62 and 65 both depend on is enforced nowhere and duplicated across the two linters; DECIDED 2026-07-04 morning round: hoist the shared constants into `lint_common.py` (removes the duplication, no new gate; the sibling GR-3 wave-3 question was closed keep-as-is the same round); build item.
...
Coverage and identifier-currency map of the held ATLAS 2026.06 refresh against the corpus adversarial-AI content (which techniques lack carriers; which cited identifiers changed across the v5.6.0-to-2026.06 refresh), with a proposed update shape surfaced. Research DELIVERED (scratch inbox `inbox/worker-20260703-a/atlas-adversarial-alignment-research/`, 2026-07-04); research-only.

### 3.18 ATLAS technique-ID currency fix (M, S) — added 2026-07-04 (from the 3.17 delivery's verified finding)

The 3.17 delivery verified from the held ATLAS 2026.06 CSVs that three of the four ATLAS technique-ID mappings in [`ai/standard-ai-and-agentic-development-security.md`](ai/standard-ai-and-agentic-development-security.md) (the rows near line 760) are stale against the refresh: the supply-chain row's "AML.T0010 to T0013" range spans techniques that are no longer siblings, the tool-misuse row's AML.T0048 now names External Harms, and the resource-exhaustion row's AML.T0037 now names Data from Local System (the prompt-injection AML.T0051 row is correct). Orchestrator apply: re-map the three rows against the held CSVs directly (the delivery deliberately does not propose replacement IDs from memory); single-file, apply-stage, not worker-partitionable.

### 3.19 Decision-tree hygiene bundle (M, S) — added 2026-07-04 (worker QA run, validated FIT-1 + FIT-2 + FIT-6)

Three validated fresh-reader defects centred on [`docs/decision-tree.md`](docs/decision-tree.md), build-only: (1) strip the maintainer-internal PR/FR provenance parentheticals from adopter-facing prose (lines 109 and 119; provenance belongs in the CHANGELOG); (2) expand FMI (line 56) and 3PL (line 322) on first use and add both to [`governance/register-glossary.md`](governance/register-glossary.md) (0 hits today, while the doc claims glossary lookup is rarely needed); (3) add a one-line note reconciling the doc's "Small (under 50 employees)" band with the adopter guide's "under approximately 200 staff" tier (distinct axes, currently unstated).

### 3.20 Startup-roadmap module-count fix (S, S) — added 2026-07-04 (worker QA run, validated FIT-5)

The roadmap carries 24 module subheadings (E0 added after the count was authored) but two live surfaces say 23: the generated [`docs/portal.md`](docs/portal.md) (the string is hardcoded in `tools/build-portal.py` near line 339, so the fix is generator-plus-regen) and the hand-authored [`docs/template-quickstart.md`](docs/template-quickstart.md) line 57. The third carrier is a frozen historical CHANGELOG entry, left per the no-retroactive-edits convention. Build-only.

### 3.21 Citation and naming hygiene bundle (S, S) — added 2026-07-04 (worker QA run, validated FQ-B1 + FIT-7 + FIT-9 + GR-OVERLAP-1)

Four small validated fixes, build-only except one flagged cell: (1) [`specification-master-project.md`](specification-master-project.md) line 214 falsely states the audit suite does not automatically enforce monotonicity (gate 13 does exactly that); reword to cite the gate. (2) Expand ERC on first use in [`security/procedure-vulnerability-management.md`](security/procedure-vulnerability-management.md) line 168. (3) Normalize the two legacy `C-TPAT` occurrences to the corpus-standard `CTPAT` (about 245 occurrences), EXCEPT decide the matrix cell reading "C-TPAT Minimum Security Criteria" (arguably a legacy document-title quote; maintainer style call on that one cell). (4) Add the reciprocal See-Also link from the `fresh-reader-validation` skill to `library-fitness-review`. Optionally reword the five identical `tools/` PR-delta error strings to the "ensure that" form (FQ-F1) or record the tool-string carve-out.

### 3.22 PCI DSS version reconciliation + currency-linter `v`-prefix blind spot (M, S) — added 2026-07-04 (worker QA run, validated FQ-D1; DIRECTION IS THE MAINTAINER'S)

The register lists PCI DSS 4.0.1 current with 4.0 superseded, yet 12 line occurrences across 8 files cite "PCI DSS v4.0", and the citation-form template teaches BOTH `v4.0` and `v4.0.1` forms as good. The superseded pattern never fires because `tools/lint-standards-currency.py`'s separator regex (near line 180) admits `:`, `(`, or whitespace but not a `v` prefix. Two halves: (a) DECISION (morning): treat `v4.0` as an acceptable major-family label (and drop 4.0 from the superseded list) OR migrate the 12 citations to 4.0.1, moving the template's taught form in lock-step; (b) MECHANICAL: widen the linter separator with an optional `v` plus a regression fixture, whichever direction (a) takes.

### 3.23 Patch-vs-vulnerability SLA reconciliation (M, S) — added 2026-07-04 (worker QA run, validated FIT-3; VALUE IS THE MAINTAINER'S)

[`operations/procedure-patch-management.md`](operations/procedure-patch-management.md) line 55 gives non-exploited Critical a 72-hour deploy SLA while [`security/procedure-vulnerability-management.md`](security/procedure-vulnerability-management.md) line 90 gives the same case 7 days, and patch-management line 21 claims to operationalize the vulnerability-management SLAs. An operator gets two answers from two citable procedures. Decision (morning): which value governs (stricter-is-safer favours 72 hours, but rewriting the vulnerability-management SLA is a substantive policy change), or split patch-management's row into PoC-available (72h) and no-known-exploitation (7d) to mirror the source table.

### 3.24 AI access-permissions ISO mapping reconciliation via ad-hoc /matrix-fit (S, S) — added 2026-07-04 (worker QA run, validated FIT-4)

The compliance matrix row (line 384) maps the AI Access and Agent Permissions Standard to `A.8.3` without `A.5.17`, while the document's own table (line 259) claims `A.5.15 to A.5.18, A.8.2`; given the 2022 catalogue (A.8.2 Privileged access rights; A.8.3 Information access restriction) and the document's access-control-and-privileged-access scope, the matrix side is the likely error. Route through the ad-hoc `/matrix-fit` cadence (this is its trigger case: a control-code citation in doubt on the sensitive single-file matrix); fix both surfaces to the judged mapping in one PR.
