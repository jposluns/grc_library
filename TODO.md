...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half shipped in the batch-A PR and the judgment tier (the rollout-phasing note, the two retention resolutions, the two genericizations) in the batch-B PR; the original batch's items have all shipped (batch A, batch B, the coverage-gaps re-grade, L-j, and L-k, each in its own PR); the remaining bullet is the routed MEA02 title-and-fit candidate set below (the COBIT fit-pass recode PR, the lifecycle-pointer fix, the third coverage-gaps batch, and the ad-hoc fit pass shipped) (the coverage-gaps second batch and the lifecycle-vocabulary fork each shipped in their own PRs). (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)


- **(routed, the 2026-07-03 ad-hoc /matrix-fit pass, M, S) MEA02 mis-titled as Managed Assurance on five carriers, a title error carrying a latent fit question.** MEA02 is Managed System of Internal Control; Managed Assurance is MEA04 (held extract). Carriers: [`compliance/procedure-capa.md`](compliance/procedure-capa.md), [`compliance/procedure-audit-planning.md`](compliance/procedure-audit-planning.md), [`compliance/standard-internal-audit.md`](compliance/standard-internal-audit.md), [`risk/register-assurance-map.md`](risk/register-assurance-map.md), [`governance/template-document-review-record.md`](governance/template-document-review-record.md), plus the near-canonical variant in [`ai/procedure-ai-audit.md`](ai/procedure-ai-audit.md). For the audit-planning and internal-audit documents MEA04 is plausibly the righter objective, so judge fit per carrier before recoding or re-titling (the whole-table lesson); ships per the standing recodes-as-their-own-PR pre-authorization.
