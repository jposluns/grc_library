...
- **(r3, [guardrails], L, S) D8 candidate (relabelled from the r3 review's "D6" after the pack-README co-bump check shipped as delta gate D6 in #623, and again from "D7" after the handoff-snapshot freshness check shipped as delta gate D7 in #634): TODO section-close orphan check.** A PR-time check failing when a TODO `###` heading deletion leaves live `§N` / `section N.M` references on gate-exempt surfaces (three logged misses: #469, #471, the #593 carrier pair). The close-out checklist's whole-repo grep is the convention it would mechanize; gate 62 does not cover bare refs or gate-exempt trees. The 2026-07-04 worker QA run's validated GR-GAP-2 duplicates this class and adds the one open design choice: an event-triggered PR-time delta check (fires on a TODO heading deletion, this bullet's original shape) versus a STANDING corpus scan of `.py` docstrings and `.claude/` markdown for stale section pointers; decide the shape at build time. **ATTEMPTED as the event-triggered delta shape and census-VETOED (2026-07-06):** built the advisory (fires on a TODO heading deletion, greps gate-exempt surfaces for surviving `§N` refs, filtering closure-word lines) and censused it on the #677 deletion of the TODO 1.9 heading (which the pre-push verifier had confirmed has ZERO genuine orphans); it flagged 18 candidates with ZERO true positives (all frozen `.working/` archives, append-only history ledgers, code-provenance docstrings, and its own fixtures), because the legitimate-record-vs-stale-pointer distinction is semantic, not mechanical. The tool was discarded. Recommendation (maintainer to adjudicate): REST on the close-out checklist's §N-orphan whole-repo grep convention, OR retry as a much narrower STANDING scan restricted to forward-status pointers in `.claude/` + tool docstrings only (excluding all frozen/append-only `.working/` archives). Census evidence in [`design-decisions.md`](.working/design-decisions.md).
