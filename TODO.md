...
- **Reference gap (recurring, missing-reference SOP):** the EU AI Act full text (Regulation (EU) 2024/1689) is NOT held in `grc_library_ref` (only the GPAI Code chapters and the two Commission Guidelines are), yet the AI-delta folds and the EU annex cite AI Act articles throughout. Until it is acquired, verify each load-bearing AI Act article citation upstream in the current turn (as #844 did for Article 51). Acquire the AI Act into `grc_library_ref` (cross-repo) so the delta's article citations rest on a held primary source.
...
**STANDING REMINDER OBLIGATION (maintainer-directed 2026-07-12) — DISCHARGED.** The maintainer pinged the pack-hygiene worker on 2026-07-12 and Phase 3 (rules scrub + PROJECT-OVERLAY architecture) is being BUILT now; the delivery will land in `grc_library_scratch` for validate-then-apply. No further morning reminder needed unless the build stalls.
...
- **Phase 1, GR-P2 condense apply.** Two-layer operative-core / on-demand-rationale split across the 13 governance rules per the PR #441 method, with the removal ledger [`.working/claude-rules-considerations.md`](.working/claude-rules-considerations.md); KEEP a compact framework-alignment table in each condensed rule; `lint-language` on each before first commit. Tranche 1 shipped 2026-07-09 (method + ledger + `gate-discipline.md` condensed, keeping the operative core + framework table); tranches 2-12 are folded here (they proceed on the maintainer-confirmed keep-the-table direction). Phase-1 folded-in fixes: (a) migrate the pack-wide `LOG-02, LOG-08` audit-trail-row convention to `LOG-02, LOG-04, LOG-10` (LOG-08 "Audit Logs Sanitization" is a mis-fit; LOG-04 + LOG-10 fit; bare-token census first) and swap the apex rule's Integrity-row `A.8.34` to `A.5.33`; (b) reconcile the change-tracking rule's two-file-split "same lead-paragraph wording" text to the divergent-lead practice (#560). **PROGRESS 2026-07-12: PHASE 1 COMPLETE.** Tranches 2-4 shipped (#835/#836/#837), completing the **12-rule GR-P2 condense set**; the phase-1 close-out (#838) then landed the folded-in fixes and the 13th-rule condense: fix (a) the `LOG-02, LOG-08` to `LOG-02, LOG-04, LOG-10` migration across its 8 carriers (verified against held CCM v4.1) and the apex Integrity-row `A.8.34` to `A.5.33` swap (verified against held ISO/IEC 27001:2022), fix (b) the change-tracking lead-wording reconcile (#560), and the condense of the 13th rule [`dev-security/claude-rules/governance/session-lifecycle.md`](dev-security/claude-rules/governance/session-lifecycle.md) (RM-SL-1). All thirteen governance rules are now condensed. **One item deliberately deferred (a matrix-fit follow-up, recorded here), and GR-P5b folded in:** (i) the matrix-fit-class follow-up on `A.8.34` in high-assurance-verification (two rows) and trust-recovery-escalation (one row) was RESOLVED in #840: judged against the held ISO/IEC 27001:2022, `A.8.34` ("Protection of information systems during audit testing") is a mis-fit for those independent-review rows and was remapped to `A.5.35` ("Independent review of information security"), both trees; (ii) **GR-P5b** (the exception-register strict-mode hoist) was folded into #838 rather than deferred: its only named blocker (a concurrent gate-discipline condense) was already resolved (both target rules condensed), so the strict-mode stance was hoisted to a prominent lead line in `gate-discipline` and `change-tracking` (both trees). **Phase 2 status:** patch **0001** (the gate-37 PROJECT-OVERLAY sync-gate extension + 4 regression tests + 4 mutation-probe variants) was applied in #839 as a Phase-3 prerequisite (it `git apply`-applied cleanly, disjoint from Phase 1). Its gate-37 §6 spec narrative + spec bump ride the re-delivered 0002 per the phase-2 sequencing. Patches 0002/0003 were re-delivered by the worker as ONE combined bookkeeping-free patch (rebased on post-Phase-1 `main`, `git apply --check` clean) and **APPLIED in #842**: 14 skills generalized in place, 2 new derived skills (`validate-inference`, `surface-instruction-concern`, pack skills 21 to 23), the new `rule-provenance.md` register, the pack README rework, and the gate-37 §6 narrative; pack `1.59.12` to `1.61.0`; the formal `/guardrails` (r9) ran at apply (1 Medium See-Also reciprocity fixed in-window, a refute-briefed verifier caught 1 Medium bookkeeping contradiction fixed pre-push). **PHASE 2 COMPLETE.**
