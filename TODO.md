...
The public landing site is **built and live on Cloudflare Pages at grclibrary.ai** (a top-level `.web/` generator that reads the corpus live at build; the rendered HTML is an ephemeral build artefact, only the generator source and templates are committed). The build effort across the live-review + adoption rounds (#919-#928) is rotated to [`DONE.md`](.working/DONE.md). **ATTENDED-SESSION-ONLY** (maintainer interactivity required; does not run unattended). **Remaining, all maintainer-gated; §2.4 stays OPEN until publish:**

- Further live-review fixes as the maintainer surfaces them.
- The two confirmed adoption follow-ups: §2.15 (standards-list authoritative-source links) and §2.16 (two-level nested landing nav).
- §2.6 (the Cloudflare-console build-watch-paths action for the 11 domain directories).
- The **publication go-decision** (the site stays preview-until-then per the maintainer's judgment), plus the production branch name, the Cloudflare Pages project settings, and the About credential strip (CGEIT/CISSP framing) against the bio prose.
- Standing cleanup: DRY the landing/pack contents-sidebar CSS into the shared partial.
