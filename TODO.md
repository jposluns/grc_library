...
### 1.8 Ref-side currency corrections (cross-repo `grc_library_ref` PR, S)
...
### 3.5 Gate 31 future-dated-Date detection (deep-assessment r1 R8a, S)

[`tools/lint-document-date-staleness.py`](tools/lint-document-date-staleness.py) (gate 31) fails only on a Date that LAGS the commit, not one ahead of it, so a future-dated Date field is gate-invisible. Extend the gate to also fail on a Date after the commit date. (Not a current defect; surfaced by the r1 fitness pass.)
...
- **(R12-risk) Risk / ERM**: IEC 31010:2019 (risk-assessment-methodology + quantitative-risk-analysis); NIST IR 8286 series (enterprise-risk-management §10); NIST SP 800-221 + 800-221A (enterprise-risk-management).
- **(R12-privacy) Privacy**: ISO/IEC 29100:2024 + 29151:2017 (privacy-and-data-governance §5); NIST IR 8062 (privacy-by-design); NIST SP 800-226 (pseudonymization-and-anonymization differential-privacy row).
- **(R12-iam) Identity & access**: ISO/IEC 24760 (identity-management + policy-IAM); ISO/IEC 29146:2024 (access-control + policy-IAM); NIST SP 800-63A-4 (identity-management §1 enrolment); NIST SP 800-210 (cloud-security-baseline access control).
- **(R12-ir) Incident response & forensics**: NIST SP 800-184 (incident-response §5.3 recovery); NIST SP 800-86 (incident-response §5.2/§8 forensics); ISO/IEC 27037:2012 (incident-response §8, closing a sibling cross-ref asymmetry).
- **(R12-resilience) Resilience / EA**: NIST SP 800-160 Vol.2 Rev.1 (business-continuity-and-resilience); NIST SP 800-160 Vol.1 Rev.1 (enterprise-architecture); ISO/IEC 27040:2024 (data-classification-and-handling + media-handling-and-transport).
- **(R12-ai) AI**: ISO/IEC 38507:2022 (ai-governance-council charter); ISO/IEC 5338:2023 (ai-model-lifecycle); ISO/IEC 8183:2023 (training-data-governance); ISO/IEC TR 24027:2021 + NIST SP 1270 (ethical-ai-use fairness + checklist A7); ISO/IEC 12792:2025 + NIST IR 8312 (ai-model-documentation-and-transparency); NIST AI 100-2e2025 (ai-adversarial-test-reference).
...
**Progress:** R12-risk (risk/ERM) applied in #770; R12-privacy applied in #771; R12-iam (ISO/IEC 24760 2025 parts, ISO/IEC 29146:2024, NIST SP 800-63A-4, NIST SP 800-210) applied in #772 (2026-07-10). Remaining clusters: incident-response, resilience/EA, AI, dev-security/cloud/metrics/workforce, and the bonus precision pair. §3.29 closes when the last cluster lands.
...
### 3.40 Framework-column control-code family-validation gate (deep-assessment r1 R11 discovery, gate blind-spot, M)

R11 (`/matrix-fit` source-doc pass, closed by the R11 PR) found an invalid `ISM-01/02/03/04/05/10/12` code family cited in columns headed "CSA CCM v4.1" across [`security/policy-information-security.md`](security/policy-information-security.md) and [`security/standard-logging-and-monitoring.md`](security/standard-logging-and-monitoring.md) (8 instances, all remapped to real CCM controls in the R11 PR). CSA CCM v4.1 has no `ISM` domain, yet every instance passed all gates: the CCM control-code existence gates ([`tools/lint-ccm-aicm-citations.py`](tools/lint-ccm-aicm-citations.py), [`tools/lint-document-control-codes.py`](tools/lint-document-control-codes.py)) validate only tokens whose domain prefix they recognize as CCM/AICM, so an unrecognized-prefix token in a framework-as-column cell is silently skipped rather than flagged as an invalid citation. Propose: harden the control-code gate(s) so that a token which lexically looks like a control code (`[A-Z&]+-[0-9]+`) and sits in a cell governed by a CSA-CCM (or AICM) framework-as-column header, but whose prefix is not a known CCM/AICM domain, is reported as an invalid-family citation; ship a regression fixture (an `ISM-04`-in-CCM-column case). Scope carefully to avoid false positives on legitimately-mixed cells (e.g. cells that also carry non-CCM references). Deep-assessment r2 candidate. Surfaced by R11 2026-07-10.

### 3.41 matrix-fit source-doc loose-supporting improvements + media CCM reconciliation (deep-assessment r1 R11 residue, L, S)

R11 fixed the 12 confirmed wrong-control mismatches and folded the ISM remaps. It also surfaced (routed here, NOT fixed, since they are quality improvements rather than correctness defects) two residue classes: (a) **loose-supporting rows** where a better exact-fit control exists but the cited code is defensible, not wrong: [`operations/procedure-media-handling-and-transport.md`](operations/procedure-media-handling-and-transport.md) rows on `DSP-07` for handling/classification/digital-transfer/storage/certificate (DCS-04/DCS-06/DSP-10 fit per row); [`operations/procedure-endpoint-management-and-device-compliance.md`](operations/procedure-endpoint-management-and-device-compliance.md) `TVM-06` on the patch row (END-05 is the primary); [`governance/procedure-library-quality-and-review-cadence.md`](governance/procedure-library-quality-and-review-cadence.md) `APO01` (APO11 'Managed Quality' is squarer); [`governance/standard-records-retention-and-destruction.md`](governance/standard-records-retention-and-destruction.md) `DSP-02` (DSP-16 'Data Retention and Deletion' is the fuller primary, and the printed title "Data Retention, Disposal, and Destruction" does not match DSP-02's real title 'Secure Disposal', a citation-accuracy fix). (b) two **matrix-vs-per-doc CCM divergences** left after R11 fixed the per-document tables: (b1) media-handling, where [`compliance/matrix-grc-compliance-alignment.md`](compliance/matrix-grc-compliance-alignment.md):188 maps it to `DSP-04, DSP-05` (Data Classification / Data Flow Documentation) while the per-doc table now uses `DSP-07, DCS-05, DSP-02`; (b2) threat-modelling, where `compliance/matrix-grc-compliance-alignment.md:168` still carries `CCC-06` (Change Management Baseline) which R11 removed from the doc's own CCM column as a mismatch (the two rows went to `TVM-04`), so the matrix now cites a control the fixed source doc no longer carries. Reconcile each to one coherent set (CCC-06 is a valid code, so this is a fit-consistency reconciliation, not an existence defect). Low priority. Surfaced by R11 2026-07-10 (b2 by the R11 pre-push verifier).
