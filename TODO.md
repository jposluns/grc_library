...
### 1.27.7 Corpus-wide standards-mapping parallel-carrier sweep (from #1269 dual-family; H, M) `[content]`

The 1.27.2 dual-family verify found the R7/R8/R6 defect classes recur in CORPUS documents beyond the pack (the register scoped R7/R8/R9 to the pack only). Sweep the whole corpus and fix each carrier (per-doc Version/Date bump; dual-family): (a) INVALID SSDF ids: `standard-developer-security-requirements.md:354` (`VE.1 to VE.3` -> `PW.7.x`/`PW.8.x` per fit; VE is not an SSDF family), `standard-devops-security-requirements.md:208` (`DS.2` -> `PS.2`); grep the full corpus for any other `VE.`/`DS.` SSDF ids. (b) PER-GATE SLSA `Level N` miscategorization in framework tables (devops/SCA/developer/container security-requirements + `artefact-and-branch-discipline.md`): apply the R8 treatment (SLSA levels are build-platform/provenance properties, not per-control) doc-by-doc. (c) `CEK-10 to 21` on gate-bypass is R6 (1.27.5); the CEK-on-secrets/secret-scanning rows are defensible (crypto-adjacent), confirm each. SLSA version is v1.2 (confirmed upstream 2026-07-30). (d) PASSWORD-HASH allowlist consistency (from #1271/R19c widen decision): `standard-developer-security-requirements.md:152` and `standard-security-quick-reference.md:181` list 'Argon2id, bcrypt' only; widen to match the pack's OWASP-aligned set (scrypt/PBKDF2-for-FIPS).
...
### 3.169 The permission-denied (EACCES) case is in none of the three delivery-tool self-tests (2026-07-28 deep-assessment c4, claude, M, S)
...
### 3.185 Deferred #1223 dual-family-codification prose nuances (2026-07-28 final claude verify of #1223, maintainer-deferred, L, S)
...
### 3.97 `grc_library_ref` `upstream_url` enrichment for FREE trusted-bucket entries (§1.19.7c follow-up, 2026-07-17, M; cross-repo `_ref`) `[machinery]`
...
### 3.189 Harden pr-closeout `has_findings` detail-file path to the require-private resolver (PR2b-3 Fable verify F-6, 2026-07-29, S, S) `[machinery]`
...
### RB-8 reference-acquisition manifest regen (Sweep 128 loop-break `/validate`, claude lens; L, XS)
