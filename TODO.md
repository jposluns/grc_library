...
### 3.48 Verify and cite the DORA major-incident reporting windows against the now-held RTS (S)

The DORA legislation is now held in `grc_library_ref` (maintainer ingest, 2026-07-12), and the designations are confirmed: **Delegated Reg (EU) 2024/1772** is the incident-classification RTS (criteria, materiality thresholds); **Delegated Reg (EU) 2025/301 Article 5** is the RTS that prescribes the reporting WINDOWS (4h/24h, 72h, one month); **Implementing Reg (EU) 2025/302** is the ITS (templates/forms). The corpus annex [`compliance/financial-services/annex-dora-implementation.md`](compliance/financial-services/annex-dora-implementation.md) states the windows as "initial notification no later than 4 hours from classification as major; intermediate report within 72 hours; final report within one month", correctly attributed to "RTS / ITS" (no specific number, so no false citation shipped). Now verify those three windows against the held **2025/301 Article 5** and correct/complete them (in particular check the initial-notification element: Article 5 has a 4h AND a 24h component the annex's single "4 hours" phrasing may under-state), and add the precise RTS/ITS citations (2025/301 for the windows, 2024/1772 for classification, 2025/302 for the templates). Resolves the deep-assessment r2 F5 finding.
...
### 3.51 Deep-assessment r2 reference-breadth additions (S)

Three held-but-unused authoritative sources the r2 reference-audit surfaced (additive citations, not defects; each needs a per-touch `/reference-audit` apply with the source read at attach-time): (a) ISO/IEC 30111:2019 (vulnerability-handling processes, the dedicated standard for the process [`security/procedure-vulnerability-management.md`](security/procedure-vulnerability-management.md) documents; also ISO/IEC 29147 disclosure, both uncited corpus-wide), add to that procedure's framework-alignment table; (b) MITRE CAPEC (attack-pattern enumeration) as a supplementary input in [`security/standard-threat-modelling.md`](security/standard-threat-modelling.md)'s supplementary-frameworks list; (c) NIST IR 8374r1 (ransomware CSF 2.0 community profile) as a control-prioritization overlay in [`resilience/procedure-backup-and-recovery.md`](resilience/procedure-backup-and-recovery.md) (marginal).
...
### 4.8 Pack adoption-hygiene programme (phases 1-4) (GR-P2 + GR-P5 + pack-design GR-P3/GR-P4, HOLD maintainer-gated, M, XL) (consolidates the former GR-P5 residual item)

**HOLD, maintainer-gated (2026-07-11).** Do NOT begin any phase, and do NOT apply anything from the existing `grc_library_scratch` drops (the `inbox/worker-20260708-fable/gr-p2-rule-condense*` tranches and their `APPLY-ADDENDUM.md`, or `inbox/claude-pack-hygiene/`), until the maintainer gives an explicit OK. A consolidated scratch drop (single master plan + phased payloads) is now merged on scratch `main` at `inbox/claude-pack-hygiene/` (entry `MANIFEST.md`, then `MASTER-PLAN.md` with the phase status board); it SUPERSEDES those old drops (which are removed scratch-side, git history retains them). Phases 1-2 are apply-ready and worker-tested; note GR-345's GR-P3/P4/P5a content is ALREADY APPLIED via #727, only its GR-P5b section stays live (used by phase 1). Starting from the old drops would duplicate and conflict with the consolidated one.

**STANDING REMINDER OBLIGATION (maintainer-directed 2026-07-11).** The pack-hygiene WORKER session is gated on the maintainer's explicit ping and will NOT self-start; without a ping it stays idle. Remind the maintainer prominently in chat that the worker needs a go-ahead to build the phase-3 rules-scrub + overlay payload on the merged post-phase-1 base: (a) when the whole-project `/deep-assessment` finishes, and (b) whenever phase 1 (the GR-P2 condense) merges to `main`. Surface the reminder rather than assuming it is remembered.
