...
- **Maintainer-directed running order**: FR-167 (comprehensive matrix) is COMPLETE (closed 2026-06-30). The current order, superseding the original 2026-06-23 sequence: (1) the maintainer-prioritized **NIST SP 800 ingestion** then the **OT post-ingestion validation** (top of P1); (2) the **deferred-decision work** (DD-2..DD-12, queued as ordinary items in P2/P3) and the deepen-baselines; (3) **reduce the backlog by treating the smallest items in batches**, using 10+ well-briefed research-assistant agents (workers produce verified research from briefs built off [`.working/worker-brief-template.md`](.working/worker-brief-template.md); the orchestrator re-verifies every worker claim at apply-time and authors all final prose). The standing **fix-issues-first** directive (2026-06-27) governs within each tier.
...
- **NIST SP 800 ingestion (next task; prerequisite for OT content-deepening)**: Ingest the relevant free NIST SP 800-series documents into scratch `ref/standards/NIST/`. SP 800-82r3 (OT/ICS security) is top priority; add 800-53r5, 800-37 (RMF), 800-61 (incident handling), 800-137 (ConMon) as relevant. Mode A scratch ingestion (extract, catalogue, INDEX, `validate.py`, scratch MCP-PR). Maintainer arranges the paid ISO/IEC standards separately (IEC 62443 is the key OT gap; see the acquisition-needs note).
- **OT post-ingestion validation (follow-up to the above)**: Once SP 800-82 is held in scratch, audit the FR-167 OT matrix rows and the `operations/ot/*` documents against it, and fix or confirm. Closes the loop on OT work completed before the source was held.
