...
**Status (PR #1367):** the pack tree MOVED to root `guardrails/` and every tooling, gated, and link reference is updated (gate-37 MIRROR_MAP, the seven scan-root allow-lists, Python path segments, corpus and sibling links; derived artefacts regenerated; `.claude/rules/` mirror and byte-parity unchanged; all gates green). The repo-layout prose self-descriptions (`dev-security/README.md`, `README.md` directory listing, `specification-master-project.md` domain table, `dev-security/guideline-ai-coding-assistant-security.md`) were retargeted to the pack's top-level home in #1367/#1368, with the guideline's local-mode detector path completed in #1370. This item stays open only for part (b) below: the build tooling and generated coding-agent adapters, produced after the 1.26.5a baseline manifest defines their input set.
...
**Status (2026-08-03):** the baseline manifest (1.26.5a) landed in #1374 (`tools/publication-manifest.json` plus gate 83, classifying all 66 pack files into the core/adapter-input/grc-only buckets, dual-family and Fable-confirmed). This item stays open for the final manifest (1.26.5-final), reconciled just before the freeze (1.26.3).
