...
- **DD-2 / DD-3 / DD-11 (M, M)**: Harmonize the three risk-scoring docs to the canonical ERM scale (Very Low→Very High likelihood; Low/Medium/High/Critical rating; Catastrophic impact-5). Surfaces: [`supply-chain/register-concentration-risk.md`](supply-chain/register-concentration-risk.md):95 (old Rare…Almost Certain labels + "Severe" impact), the impact-5 label divergence (standard "Catastrophic" vs procedure/template "Critical"), and [`risk/template-operational-risk-register.md`](risk/template-operational-risk-register.md):123-127/:135/:222 ("Moderate" likelihood+rating, "Severe" impact-5). One coherent risk-vocabulary PR.
...
- **S4 Gate-48 section-aware title check (S) — surfaced 2026-06-24 (PR #299 `/validate-pr`)**: extend [`tools/lint-ccm-aicm-citations.py`](tools/lint-ccm-aicm-citations.py) so a control-listing row under a `## CSA CCM ...` section is title-checked against `CCM_V41` and one under an `## AICM ...` section against `AICM_V11`, rather than the AICM-wins union `ALL_TITLES`. Catches the two controls whose titles diverge between the catalogues (I&S-07, IAM-11) when cited in the wrong-version section; the current union check is blind to this near-identical-title confusion (it caught the I&S-07 case only via `/validate-pr`, not the gate). Low-frequency; pairs with the integrity-tooling phase.
- **S5 Gate-48 bare-domain-code check (S) — surfaced 2026-06-24 (Sweep 36)**: extend [`tools/lint-ccm-aicm-citations.py`](tools/lint-ccm-aicm-citations.py) to flag bare known-bad CCM domain codes (`IVS` → `I&S`, `NET` → `I&S`, `GOV` → `GRC`, `GRM` → `GRC`, `AUD` → `A&A`, `EKM` → `CEK`, `MOS` → `UEM`, `DSI` → `DSP`) appearing in a CCM/CSA context (framework-coverage family lists, glossary domain entries, the domain-by-domain crosswalk), not only `<DOMAIN>-<NN>` tokens. Sweep 36 found PR #298's reconciliation left ~15 bare-code residuals (fixed in PR #301) precisely because the gate and the #298 validator are token-scoped. This is the durable mechanical backstop for that finding-class. Care needed to avoid false positives: legitimate non-CCM uses exist (`.NET`, `AUD` currency, corpus-internal `MODEL-GOV`/`AI-GOV` identifiers, "government" prose), so scope detection to CCM/CSA-context lines. Pairs with S4 (both gate-48 enhancements).
