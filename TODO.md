...
**Status (PR #1367):** the pack tree MOVED to root `guardrails/` and every tooling, gated, and link reference is updated (gate-37 MIRROR_MAP, the seven scan-root allow-lists, Python path segments, corpus and sibling links; derived artefacts regenerated; `.claude/rules/` mirror and byte-parity unchanged; all 82 gates green). The repo-layout prose self-descriptions (`dev-security/README.md`, `README.md` directory listing, `specification-master-project.md` domain table, `dev-security/guideline-ai-coding-assistant-security.md`) were retargeted to the pack's top-level home in #1367/#1368, with the guideline's local-mode detector path completed in #1370. This item stays open only for part (b) below: the build tooling and generated coding-agent adapters, produced after the 1.26.5a baseline manifest defines their input set.
