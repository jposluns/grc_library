...
- **Last validation sweep**: Sweep 26 iter 1 (the `/resume`-triggered corpus-wide `/validate` the morning after the autonomous overnight run, the compensating control for handoff PR #268; full three-subagent dispatch A/B/C; **zero in-window regressions** across PRs #259-#268 (FR-134..141 + Sweep 25); three out-of-window candidate findings, after apply-time verification 2 confirmed + 1 refuted — (confirmed) stale docstring `lint-collection-enumeration-consistency.py`:20 "three"→"two" (fixed in PR #269), (confirmed) CPPA-as-live row `standard-enterprise-risk-management.md`:255 (folded into the deferred broader-CPPA sweep), (refuted) `matrix-grc-compliance-alignment.md`:151 "All 42 documents" is correct (42 unique, 43 rows, one dual-domain duplicate); all 9 pre-flight candidates confirmed false positives; closed out in PR #269).
