...
- **MITRE ATLAS scratch superseded-archival** (move the deprecated `v5.6.0` to scratch `ref/.superseded/`, fetch `v2026.06`): the corpus register was corrected to the upstream-current **`v2026.06`** (the 2026-06-30 release, superseding the `v2026.05` #505 recorded; verified upstream 2026-07-01) in #512, so the corpus side is current. The remaining residual is the scratch-side archival (scratch still holds `v5.6.0`); egress to the MITRE atlas-data repo was confirmed available 2026-07-01, so it is auto-fetchable via the scratch push / MCP-PR workflow (no maintainer download needed). Logged in [`pending-decisions.md`](.working/pending-decisions.md).
