...
**Status (PR #1367):** the pack tree MOVED to root `guardrails/` and every tooling, gated, and link reference is updated (gate-37 MIRROR_MAP, the seven scan-root allow-lists, Python path segments, corpus and sibling links; derived artefacts regenerated; `.claude/rules/` mirror and byte-parity unchanged; all 82 gates green). REMAINING (this item stays open): the repo-layout prose self-descriptions that still describe the pack as a `dev-security/` subdirectory, `dev-security/README.md` (the `## Claude Code rules pack` section + layout tree + intro item 2), `README.md` directory listing, `specification-master-project.md` domain table, and `guideline-ai-coding-assistant-security.md:58`, need authorial rework to reflect the pack's new top-level home (deferred from #1367 to avoid rushing semantic restructuring that interacts with the domain/taxonomy surfaces).

**Sequenced EARLY (maintainer-directed 2026-08-02): do the internal relocation BEFORE the new content phases so all subsequent work is authored at the final location and no reference or pointer needs repointing later.** Move `guardrails/` to root `guardrails/` (a canonical `guardrails/core/`, deterministic build tooling under `guardrails/build/`, and generated coding-agent adapters under `guardrails/adapters/`, Claude Code + OpenAI Codex first), extend gate 37 with the generator `--check`, repoint the ~493 references to the old `guardrails/` path (private migration inventory), repoint every hook / CI / runner path, and keep the full gate suite green. INTERNAL relocation only: it does NOT require the publication prerequisites (classify / agnostic / freeze), which gate the external publish (1.26.20). Non-partitionable, single atomic PR. The universal skill (chat) needs no adapter. Migration-design prep: the private `1264-migration` worker delivery. SPLIT (fable I4): (a) the genuinely-atomic move + repoint, then (b) build tooling + adapters, generated AFTER the baseline manifest (1.26.5a) that defines their input set, never before.
