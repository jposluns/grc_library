...
- **Workstream A (EU/CA source fold-ins):** A.4 + A.5 (the two Commission Guidelines to the EU annex + policy) **LANDED in #843**. Remaining: A.1 (GPAI Transparency Chapter to the transparency framework + card templates); A.2 (GPAI Safety-Security Chapter, cross-fold into foundation-model-lifecycle + agentic-dev-security + model-risk; it is the Article 56 co-regulatory Code for demonstrating Article 55 compliance, binding only GPAI-systemic-risk providers); A.3 (GPAI Copyright Chapter to training-data-governance + policy); A.6 (California CCPA/ADMT: new `annex-ai-us-california.md` + policy US-state row, binding, `[VERIFY]` ADMT dates upstream).
...
- **Workstream C (currency + residuals):** C.1 Canada TBS Directive third-review transition (`[VERIFY]`); C.2 ref-side `last_checked` sweep for the 6 new EU/CA sources (cross-repo); C.3 IEEE 7000 `/claim-fit` on the checklist row; C.4 ISO/PAS 8800 no-action (resolved).
...
- **BLOCKER (A.1, missing-reference SOP):** the held EU GPAI Transparency Chapter PDF in `grc_library_ref` is TRUNCATED at 6 pages, stopping where the Model Documentation Form begins, so the Form's field structure (needed for the Form-to-card mapping) is NOT held. At the A.1 PR: PAUSE, attempt to acquire the complete Transparency Chapter into `grc_library_ref` (cross-repo PR), else route with options (defer the mapping as accepted-unverified / cite the Form by reference only). Also: `grc_library_ref` `catalogue.yml` (the Transparency Chapter `notes`) wrongly asserts the Form is held; correct ref-side.
