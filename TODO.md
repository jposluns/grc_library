...
- **(item 24 residual, XS) "D1-D4" stale in the protected CLAUDE.md line.** The two tool-side carriers ([`tools/run-pr-time-checks.sh`](tools/run-pr-time-checks.sh) header and [`tools/pre-push-guard.sh`](tools/pre-push-guard.sh)) were updated to D1-D5 in the W6 PR; the remaining carrier is [`.claude/CLAUDE.md`](.claude/CLAUDE.md):152 (protected file, maintainer-gated morning edit); when making that edit, also glance at [`.working/claude-md-considerations.md`](.working/claude-md-considerations.md):119, whose condense-time "gate identities (D1-D4, 45/40/31) are retained" line becomes the only other D1-D4-as-current carrier, per the #558 post-merge sweep note; the #559 sweep adds a second protected-line item for the same edit: [`.claude/CLAUDE.md`](.claude/CLAUDE.md):215 enumerates the D5 closure form as FR-only, under-inclusive after the GR-13 coded-id widening).
...
- **(L-d, promoted M, S) Verbless "ensure that" sentences: corpus-wide repair (C10).** The #567 post-merge sweep enumerated the class at about 20 open carriers (patterns: `ensures? that (consistent|alignment|adherence|resourcing|integrity|compliance|transparency|integration)` plus `ensures? that [a-z-]+ (of|to|with|across|and)`, every hit read, grammatical hits excluded), an order of magnitude above both prior counts, PLUS two more the C9 verifier found by re-running the first pattern (encryption policy :39 "and ensures that compliance monitoring"; data-classification :44 "Ensures that alignment with privacy and data protection laws"), so the enumerated-to-date count is 22 and the named patterns are the METHOD, not a proof of completeness (C10's script re-enumerates before applying); the probable root cause is a past mechanical "ensure to ensure that" house-style sweep breaking noun-object sentences (the C10 verifier confirms or refutes against git history). Enumerated carriers: qa-and-testing :21 :40; grc-programme procedure :45; exception policy :54; digital-twin policy :46; cloud-baseline :21; privacy policy :47; China annex :44; DSR procedure :27; encryption policy :38 :39; logging standard :23 :55; crypto-key operations :21; infosec policy :42; acceptance-into-service :134; supplier due diligence :40; data-classification :44 :106; DLP :21; plus the two originals, both in [`compliance/policy-compliance-and-audit-management.md`](compliance/policy-compliance-and-audit-management.md) (:41 "The CEO ensures that resourcing and independence..." and :59 "to ensure that adherence to ... programs"). Repair ALL in one scripted PR (C10) with per-file Version+Date bumps and a refute verifier; each repair supplies the missing verb, never deletes "that".
- **(routed note, C9 verifier, L, S) Coverage-gaps dangling-pointer status triage.** Nine register cells pointed at backlog work TODO does not carry (the OT-depth pair, the three adopter-experience rows, the four deferred sector rows); C9 reworded them to the evidence-accurate "Backlog candidate: ... (not currently scheduled in TODO)" form, but several rows' Coverage/Status cells may also be stale (the adopter-experience asks are partly shipped in docs/; OT shipped as a suite). Maintainer triage: re-grade those rows or confirm the candidate framing.
...
- **(L-f, M, S) Forensic-vs-incident retention.** [`security/procedure-security-incident-response.md`](security/procedure-security-incident-response.md):239 (forensic evidence 7y) outlives [`governance/register-data-retention-schedule.md`](governance/register-data-retention-schedule.md):63 (incident record 5y); align the incident-record row to the evidence floor or document the rationale. Same-row sibling from the C4 completeness scan: privacy-breach evidence carries a 7-year minimum (the register row newly aligned to it) while the incident row (echoed in [`security/standard-logging-and-monitoring.md`](security/standard-logging-and-monitoring.md):92) stays at 5, and a privacy breach is a security-incident subclass; resolve both angles in the one row decision (stricter-safe leans 7). (A retention conflict; kept here per the audit's low-sev tiering but flagged M.)
- **(routed note, #565 `/validate-pr`, L, XS) COBIT objective-title carriers.** Three files cite the APO12 OBJECTIVE with the non-canonical title "Manage Risk" ([`risk/standard-enterprise-risk-management.md`](risk/standard-enterprise-risk-management.md):251, [`risk/procedure-risk-assessment-methodology.md`](risk/procedure-risk-assessment-methodology.md):184, [`risk/policy-enterprise-governance-and-risk-management.md`](risk/policy-enterprise-governance-and-risk-management.md):135); COBIT 2019's objective title is "Managed Risk" (used correctly by [`governance/procedure-grc-programme-management-and-annual-review.md`](governance/procedure-grc-programme-management-and-annual-review.md):336). Mechanical three-line fix, same class as the C6 practice-title corrections but objective-level.
- **(routed note, #563 `/validate-pr`, L, S) Retention-standard AI-domain minimum vs register AI-audit row.** [`governance/standard-records-retention-and-destruction.md`](governance/standard-records-retention-and-destruction.md):80 sets an "AI Systems | 5 years post-decommission" domain minimum while the register's AI-audit row is a flat 7 years; neither dominates for all decommission timings (the composed-conflict family shape). Compose the pair or declare the domain minimum a floor the record rows may exceed.
...
- **(GR-5, M, S) Guardrail-review cadence mechanization.** The [`guardrail-review`](dev-security/claude-rules/skills/guardrail-review/SKILL.md) auto-prompt cadence (:20, after any machinery-changing PR) has not been honoured: one run ever (2026-06-25, against a machinery inventory that was materially smaller on every axis; see the run's history row for the as-of counts) with dozens of machinery-touching commits since (the exact count depends on the touch definition; the gap, one run against sustained machinery growth, is the finding). Add a mechanical check comparing the current machinery inventory against the counts in the last guardrail-review history row (warn/fail on drift beyond a threshold), or demote the SKILL cadence claim to match practice.
...
- **(GR-12, M, M) Post-fix residual-scan discipline as tooling.** The narrow-scan class (the #443 bare-token lesson) has recurred well past three occurrences, most recently INSIDE Sweep 82's own fix verification (#553 `/validate-pr`: a surfaced grep hit was triaged from a truncated excerpt instead of a full read). Checklist lines reduce but do not extinguish the class (the guardrail review's effectiveness finding); build the deterministic residual-scan step as tooling (given a corrected token, scan the full live surface set and emit every hit with full-line context for mandatory per-hit reads), per the third-occurrence-to-gate escalation convention (§4.7 GR-P3).
