...
### 4.6a FR-167 matrix control-code-validity gate — NIST CSF 2.0 membership follow-up (M, M) — surfaced 2026-06-24 (maintainer asked)

**Gate 49 shipped** ([`tools/lint-matrix-control-codes.py`](tools/lint-matrix-control-codes.py)): it validates the matrix's framework codes for the columns gate 48 does not cover, ISO/IEC 27001:2022 Annex A membership (`A.5`-`A.8` within each theme's control count; 93 total) + clause format (`§4`-`§10`), and NIST CSF 2.0 **well-formedness** (Core Function prefix `GV/ID/PR/DE/RS/RC` + `FUNCTION.CATEGORY` shape). The CSA CCM column is already validated corpus-wide by gate 48 (not duplicated); the customs/trade columns (CTPAT/PIP/BASC/WCO SAFE/AEO-S) are free-text requirement labels with no closed code set (out of scope). Enforces code *validity*, not mapping correctness (the apply-time author-every-cell discipline stands).

**Remaining follow-up (its own PR, semantic):**

- **NIST CSF 2.0 category-membership.** Extend gate 49 to validate each NIST token's CATEGORY (not just its function prefix) against the real CSF 2.0 category set. The authoritative source (NIST CSWP.29, *The NIST Cybersecurity Framework 2.0*) was **supplied by the maintainer 2026-06-24** and is available locally at `/root/.claude/uploads/6e82ee0b-089a-5a05-9cf0-ceecb674d5bf/871a5a16-NIST.CSWP.29.pdf` (WebFetch of `nvlpubs.nist.gov` / `csrc.nist.gov` / `csf.tools` all return 403; the local PDF is extractable with `pypdf`). Encode the CSF 2.0 category set in a small reference module mirroring [`tools/ccm_aicm_reference.py`](tools/ccm_aicm_reference.py), then add the membership check.
- **Remap the 17 CSF-1.1-era cells the membership check will flag.** The "NIST CSF 2.0" column currently carries CSF-1.1 codes removed/moved in 2.0: `PR.IP` ×11, `ID.SC` ×5, `ID.BE` ×1 (verified by grep on the matrix; the format check passes them, which is why gate 49 is green today). `ID.SC` → `GV.SC` and `ID.BE` → `GV.OC` are near-mechanical; `PR.IP` ×11 needs a per-row semantic remap (CSF-1.1 "Information Protection Processes and Procedures" was redistributed in 2.0) — **surface the per-row mapping to the maintainer**. The membership check cannot ship green until these are remapped, so the remap and the check land in the same follow-up PR.
- **Relation to the bookkeeping-parity family**: distinct from §4.6/§4.10 (those gate QA-cadence/rotation *bookkeeping*); this gates matrix *content validity*.
