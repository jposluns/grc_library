...
Accuracy carriers surfaced by the WS-0.2 corpus-wide scans and the #785 `/validate-pr`, outside item 0.2's named scope. The AIDA obligation, alignment, and applicability carriers across the non-privacy corpus domains and both EO-14110 carriers have been reconciled to the lapsed / rescinded framing (recorded in the DONE ledger); this item now tracks only the remaining privacy-domain residual:

- **Remaining residual (privacy-domain AIDA section-29 mappings + "(pending)" hedges), needs the privacy verifier.** The privacy-domain AIDA obligation MAPPINGS are load-bearing (a DPIA / AI-impact-assessment obligation bound to the lapsed AIDA section 29): [`privacy/template-dpia.md`](privacy/template-dpia.md):25 and :204, [`privacy/procedure-privacy-impact-and-cross-border-transfer.md`](privacy/procedure-privacy-impact-and-cross-border-transfer.md):280, and [`privacy/annex-privacy-jurisdiction-index.md`](privacy/annex-privacy-jurisdiction-index.md):103 ("AIDA (pending)"). Plus mildly-stale "(pending)" hedges in registry/matrix cells ([`governance/matrix-cross-framework-alignment.md`](governance/matrix-cross-framework-alignment.md):69, [`governance/register-document-index-and-classification.md`](governance/register-document-index-and-classification.md):153) to normalize to "(lapsed)". Reconcile in a focused privacy/registry follow-up pass with the privacy-domain verifier (it overlaps the routed privacy item in [`pending-decisions.md`](.working/pending-decisions.md)); a lapsed statute's section number cannot carry a live DPIA obligation, so decide the replacement mapping (the Treasury Board Directive's Algorithmic Impact Assessment, or Quebec Law 25's PIA) with care. Bare-token grep `AIDA` corpus-wide before closing to confirm no other live-framing carrier survives (neutral registry cells that merely list AIDA are NOT carriers). Four such neutral bare-listing cells ([`governance/policy-exception-and-risk-acceptance-management.md`](governance/policy-exception-and-risk-acceptance-management.md):144, [`security/standard-data-classification-and-handling.md`](security/standard-data-classification-and-handling.md):146, [`governance/register-document-index-and-classification.md`](governance/register-document-index-and-classification.md):194 and :314) are correctly left as neutral; the same follow-up may optionally annotate them "(lapsed)" for corpus-wide consistency.

Surfaced 2026-07-10 during #785; AIDA scope broadened and the EO 14110 carrier added by Sweep 96; narrowed to the privacy-domain section-29 residual after the non-privacy obligation-carrier reconcile.
...
**Status (2026-07-10 resume, maintainer directed acquire-what-is-retrievable):** SIX of the seven acquired into `grc_library_ref` (PR #41, MERGED `d31ff03`) — the G7 Hiroshima Code of Conduct (`frameworks/G7/`), OWASP GenAI Red Teaming Guide v1.0 and OWASP MCP Top 10 (`frameworks/OWASP/`), PTES v1.0 (`frameworks/PTES/`), and the Mitchell 2019 / Gebru 2018 papers (`publications/`, `pending` screen). **IEEE 7000-2021 is now HELD** (`grc_library_ref` `standards/IEEE/IEEE-7000-2021--Ethical-Concerns-System-Design--full-text.md`, catalogue.yml:1276; ingested by the maintainer's worker. NOTE: the separately-priced joint IEEE/ISO/IEC 24748-7000:2022 was NOT acquired, so cite IEEE 7000-2021, not the 24748 designation). The one corroborative `synthesized from` IEEE 7000-2021 §7.3 locator in [`ai/checklist-ai-algorithmic-compliance.md`](ai/checklist-ai-algorithmic-compliance.md):107 was VERIFIED against the held text during PR 1: §7.3 ("Activities and tasks", in the ConOps and Context Exploration Process) covers context-of-use analysis and stakeholder identification (those affected by the system), a defensible corroborative anchor for Section D's stakeholder-affected human-oversight controls; the "synthesized from" framing is accurate (loose synthesis, not a prescriptive claim). **ACQUISITION AND VERIFICATION COMPLETE: all 7 citations held; §7.3 verified. §1.11 rotates to DONE in a near-term focused bookkeeping batch** (deferred from the large PR-1/PR-2 content builds so the §N-orphan cross-file cleanup lands in a bookkeeping-appropriate PR).
...
### 3.44 CHANGELOG nested-link malformation: historical cleanup + candidate gate (`/retro` #802, S)

The #802 pre-push skeptical verifier caught a bare-token global replace that re-wrapped two pre-existing historical CHANGELOG entries' links into malformed nested links (`[[`path`](path)](path)`); the in-scope corruption was reverted pre-push. Two **pre-existing** malformed nested links of the same shape survive in early-PR historical entries (the `.claude/CLAUDE.md` references at `CHANGELOG.md` lines ~3398 and ~3566, from the #37-era entries), untouched by #802 as out-of-scope. Two parts: (a) **cleanup** the two pre-existing nested links to their well-formed single-link form (a targeted historical edit, maintainer-authorized since it touches shipped entries); and (b) **candidate gate** to flag `[[`...`](`-shape nested-link malformation across `CHANGELOG.md` (including historical lines, which the current link-coverage gate and preflight do not inspect for nesting), so the class cannot recur gate-blind. FP-analysis: legitimate `[[...]]` wiki-style double brackets, if any exist, must be distinguished from the `[[`code`](url)](url)` malformation. Surfaced by the #802 retrospective 2026-07-11.
