...
### 3.22 PCI DSS currency-linter `v`-prefix blind spot (S) — rescoped 2026-07-05 (the migration half (a) shipped in #648; this is the mechanical half (b))

Half (a) shipped in #648 per maintainer decision B1: the 12 `PCI DSS v4.0` citations across 8 files were migrated to `v4.0.1`, and the citation-form template's taught form was moved to 4.0.1. Half (b) remains: `tools/lint-standards-currency.py`'s separator regex (near line 180) admits `:`, `(`, or whitespace but not a `v` prefix, so a superseded `PCI DSS v4.0` citation never fires; widen it with an optional `v` before the version (preserving the `(?![.\-][\d\w])` version-continuation guard that protects `v4.0.1`) plus a regression fixture (a `v`-prefixed superseded citation flagged, `v4.0.1` not).

Citation-specificity follow-up (surfaced by the #648 pre-push verifier): three bare `PCI DSS v4` family-label citations in the AWS, Azure, and GCP cloud-hardening baselines (`dev-security/standard-cloud-hardening-baseline-{aws,azure,gcp}.md`) use the citation-form template's discouraged `PCI DSS v4` shape (a family label with no specific version or requirement ID). They are current (the v4 family, not the superseded `v4.0`), so no currency migration was applied in #648; decide whether to normalize them to `v4.0.1` for specificity or leave the family reference (maintainer/citation-hygiene call).
