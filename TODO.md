...
### 3.18 Pack-tree ATLAS technique-ID currency refresh (M, S) — rescoped 2026-07-04 (the corpus half shipped; this is the pack half, surfaced by its apply)

The corpus-side fix (the standard's three stale framework-alignment rows) SHIPPED 2026-07-04 against the held ATLAS 2026.06 CSVs, upstream-confirmed current at apply time. Its close-out grep surfaced the same stale-ID class in the PACK tree, out of the corpus item's single-file scope: eight `AML.T0048` citations across four `dev-security/claude-rules/ai/` files (`agent-security.md` three, `ai-security.md` two, including a line-64 gloss naming T0048 "Exploit Public-Facing Application (via AI Agent)" which matches no 2026.06 title, `rag-security.md` one, `mcp-security.md` two), all in LLM06 / tool-and-agency contexts where 2026.06's T0048 is External Harms. The apply's scope is CLASS-WIDTH, not T0048-token width: check EVERY pack-tree ATLAS ID and gloss against the held CSVs (the pre-push verifier already confirmed two further gloss drifts in `ai-security.md`: line 39's AML.T0024 glossed "Training Data Exfiltration" where 2026.06 names it "Exfiltration via AI Inference API", and line 27's T0051 short-gloss "Prompt Injection" vs the title "LLM Prompt Injection"). Re-map each against the held CSVs at apply time; do NOT map from memory; pack change, so pack Version + version-history co-bump (D6) and the byte-identical local mirrors where files are mirrored.
