...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half shipped in the batch-A PR and the judgment tier (the rollout-phasing note, the two retention resolutions, the two genericizations) in the batch-B PR; the original batch's items have all shipped (batch A, batch B, the coverage-gaps re-grade, L-j, and L-k, each in its own PR); the remaining bullets are the routed lifecycle-vocabulary fork below (a maintainer-triage naming call) and the second coverage-gaps triage batch the #581 sweep surfaced. (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)

- **(routed, #581 sweep M-1 plus I-1, M, S-M; maintainer ANSWERED 2026-07-02 at the return round: execute re-grade-with-evidence, the #581 treatment, one PR; and widen the vocabulary's Partial definition to cover dedicated-adjacent-artefact cases, resolving the #581 I-2 tension in the same PR) Coverage-gaps triage, second batch: the register's remaining tables carry the same staleness class the first batch fixed.** Under the first batch's own re-grade logic (named in a shipped doc = Referenced; addressed within broader docs = Partial), the following rows of [`governance/register-coverage-gaps.md`](governance/register-coverage-gaps.md) contradict shipped corpus artefacts, each claim re-verified at file at routing time: the seven section-2.3 regulator rows graded None while the financial-services annex names every one of their regimes (UK PRA/FCA and Canada OSFI each also carry a dedicated annex narrative section, arguably Partial; Singapore MAS, Australia APRA, Hong Kong HKMA, Japan FSA, and Switzerland FINMA are annex regime-table rows, Referenced), internally inconsistent with the Referenced grades the register already gives the US bank regulators for the same shape of annex presence; the section-2.4 UK NHS DSPT row (annex regime row plus a dedicated narrative section, arguably Partial), Canada PHIPA (annex row), and Australia My Health Records (weaker evidence, the healthcare README candidates list); the section-2.5 UK AI framework row (a dedicated UK subsection plus a framework-table row in [`ai/policy-ai-compliance.md`](ai/policy-ai-compliance.md)) and US state-by-state row (the Colorado AI Act carries a canonical-citations register row and a privacy jurisdiction-index row); the section-2.2 EU AEO row (mutual-recognition prose in the PIP controls register and Union-Customs-Code rows in the supply-chain matrix); the section-3 TOGAF and ISO 9001 rows, graded None under Out-of-scope status despite framework-table rows in multiple shipped docs (ISO 9001 parallels the first batch's ISO 14001 re-grade exactly; whether an out-of-scope instrument that shipped docs cite should read Referenced is part of the triage); and the section-1 Manufacturing row (the sweep's I-1: the shipped operations OT suite addresses industrial/ICS within broader documents; its Notes cell already discloses the cross-cut). The coverage-vocabulary call the #581 mirror recorded as pending (the Partial-vs-dedicated-adjacent-artefact boundary) was answered in the same return round: widen Partial's definition (the capability is materially addressed by shipped documents, broader or adjacent dedicated, while the row's named ask remains unshipped); the definition-cell edit ships in this batch's PR since the None-vs-Referenced-vs-Partial answers turn on it.
- **(routed, #579 sweep L-2, L, S; maintainer ANSWERED 2026-07-02 at the return round: align the supersession vocabulary to "Superseded" across the four carriers, keeping "deprecated" only where it means something else; ships as its own small PR) Lifecycle-vocabulary fork: charter "Deprecated" stage vs the "Superseded" marker.** The L-j migration's marker value is `Status: Superseded`, but the corpus lifecycle vocabulary still names the stage "Deprecated": the governance charter's lifecycle stage 5, the library-health-report template's "Documents deprecated" count, the adopter-guide's "mark them deprecated in the index" advice, and the build-taxonomy docstring. The adopter-guide line arguably still instructs the old index-marking mechanism (the register's Deprecated row now documents the migration, softening it); the other three are vocabulary-only carriers, and the marker value has no anchor in the charter's stage enumeration. Options: align the supersession vocabulary to "Superseded" across the four carriers (keeping "deprecated" where it means something else), or document the stage-name-vs-marker-value distinction as accepted. Authorial naming call.
