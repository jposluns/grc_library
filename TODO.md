...
The §1.5 reference version-currency register **shipped in #505**: the `Upstream check location` + `Last verified (UTC)` columns across all 16 register tables (100 rows verified 2026-06-30, 51 `needs-reconfirm`), the advisory staleness cadence in [`specification-citation-verification.md`](governance/specification-citation-verification.md) §12.3, the scratch-is-storage / upstream-is-authority principle, the §7 publisher allow-list extension, and 7 upstream-confirmed register corrections (built under the high-assurance harness, dual-verifier-checked). Three standard-version upgrades were **deferred** because they ripple into corpus citations and need a coordinated per-document fix (gate-6 plus per-doc Version/Date bump), one tightly-scoped PR each:

- **ISO/IEC 27033 -> 27033-1:2015**: 2 corpus docs cite the non-existent `ISO/IEC 27033:2020` ([`operations/standard-network-security-and-segmentation.md`](operations/standard-network-security-and-segmentation.md), [`security/policy-network-communications-security.md`](security/policy-network-communications-security.md)); fix the register row and both citers together.
- **ISO/IEC 27036-2 2014 -> 2022**: 1 doc ([`supply-chain/procedure-supplier-due-diligence.md`](supply-chain/procedure-supplier-due-diligence.md) cites `27036-2:2014`).
- **NIST SP 800-88 Rev. 1 -> Rev. 2** (Rev. 1 withdrawn 2025-09-26): 1 doc ([`operations/procedure-media-handling-and-transport.md`](operations/procedure-media-handling-and-transport.md)); a substantive re-point, not a bare version-string swap.
