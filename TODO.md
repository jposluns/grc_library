...
- **FR-140 (H, M)**: Adopter starter-set divergence — quickstart:33 (6) / adopter-guide:115 (15) / decision-tree:104 (23) / README:271 (~37); quickstart vs startup-roadmap:49 name different 6th artefact; Tier 1 omits the IAM + acceptable-use the quickstart floor requires.
...
- **Day-1-floor risk-artefact drift (M, XS) — Sweep 34 / PR #293 `/validate-pr` out-of-window finding, needs maintainer direction**: two "six-artefact floor" definitions disagree on the sixth (risk-domain) artefact — [`docs/template-quickstart.md`](docs/template-quickstart.md):39 Day-1 floor names `risk/policy-enterprise-governance-and-risk-management.md`, while [`docs/template-startup-roadmap.md`](docs/template-startup-roadmap.md):56 core baseline names `risk/procedure-risk-register.md` (they agree on the other five). Options: (A) make the enterprise-governance-and-risk-management policy canonical in both (recommended — it is the posture-setting artefact, matches the Tier 1 starter set, harmonizes three surfaces; keep register-population as roadmap guidance, not the named floor artefact); (B) make the risk-register procedure canonical in both; (C) keep both deliberately and add a one-line note explaining the divergence.
