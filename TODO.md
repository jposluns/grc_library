...
While verifying, also confirm the two adjacent ANPD-resolution citations in the same annex (Resolution CD/ANPD No. 19/2023, the standard contractual clauses; Resolution CD/ANPD No. 2, the small-agents regulation), which are the same citation class. Provenance: Sweep-84 A-F1, routed to [`pending-decisions.md`](.working/pending-decisions.md) and resolved 2026-07-05 to annotate-now, plus the maintainer's 2026-07-05 direction to open a durable TODO item so the lookup is not forgotten. Sweep 86 (2026-07-06, F2) also flagged that the canonical-citations register row for Resolution CD/ANPD No. 15/2024 ([`governance/register-canonical-citations.md`](governance/register-canonical-citations.md)) carries a version-currency `Last verified` date with no content-attribution caveat, while the annex and the breach-response matrix flag the same resolution pending; when this verification lands, reconcile the register row's signposting in the SAME PR (do not add a content-attribution caveat to the version-currency column before then, to keep the register's two axes separate).
...
### 2.11 Publications-assessment / poisoning-detection process for the `grc_library_ref` reference base (M, M) — surfaced 2026-06-25 (was 4.12)

The `grc_library_ref` reference knowledge base is trust-bucketed: `grc_library_ref/standards/`
and `grc_library_ref/legislation/` (trusted, cite-as-authoritative), `grc_library_ref/frameworks/` and the other
catalogue buckets (trusted per the `grc_library_ref` trust tiers), and `grc_library_ref/publications/` (vendor
...
doc (likely a `skills/`-style workflow + a `grc_library_ref/publications/README.md` protocol pointer)
...
with the multi-session capability (closed as section 3.6 in the DONE ledger; the `grc_library_ref` base is part of it) and
...
### 3.18 Environment-detection intelligence for `/resume` (env-detection, M, M) — maintainer-directed 2026-07-07

The project now runs across at least two materially-different execution environments (the managed cloud sandbox and the maintainer's local NUC), and their transport/tooling assumptions diverge in ways a session hits live: the PR mechanism (GitHub MCP `mcp__github__*` vs the `gh` CLI), the CI-poll and merge transport (`gh pr checks` / `gh pr merge` use GitHub GraphQL, which shares a 5,000/hr pool that can be exhausted, forcing a fallback to the REST check-runs and `PUT .../merge` endpoints on the separate core pool, observed in #687), the stop-hook auto-commit-push (present in cloud, ABSENT on the NUC so commits/pushes are manual), the pipe-guard PreToolUse hook firing (fires on the NUC, silent in the cloud resumed-session per the TODO-1.9 `CLAUDE_PROJECT_DIR`-unset limitation), egress availability and method, and reference-repo access (the `additionalDirectories` for `grc_library_scratch` / `grc_library_ref`). **Build**: a `tools/`-style **detect-env helper** that probes these at session start (which MCP tools are present, `gh auth` status, stop-hook presence, egress reachability, which sibling repos are readable) and a **`/resume` step** that reads its output and picks the PR mechanism, the CI-poll/merge transport (with the GraphQL-exhaustion REST fallback), and the egress method, and verifies reference-repo access, per environment. **Hard constraint (design, not a bug)**: `permissions` and `additionalDirectories` bind at session launch and a `SessionStart` hook CANNOT grant them, so the design is **detect-and-adapt-tooling + detect-and-verify-access + a machine-local, git-ignored settings-override file**, never a runtime self-grant; where access is missing the step surfaces the one-line `--add-dir` / settings fix for the maintainer rather than attempting to self-grant. Own PR. **The `.claude/` parts (the `/resume` command edit, any settings) are maintainer-authored / protected-tree**, so surface them for authorization rather than shipping unattended. Provenance: the #686 MCP-token/git-credential outage that forced the NUC transfer, and the #687 GraphQL-exhaustion REST fallback (both in the improvement-log).
...
(item 28) `grc_library_ref`'s `validate.py` disk-to-catalogue orphan check covers only `--full-text.md`; ~20 substantive CSV extracts (ATLAS/ATT&CK tactics + mitigations CSVs, CSA implementation/auditing-guideline tabs) are uncatalogued and unchecked. (item 29) Its watermark/PII scan covers `.pdf` only; 22 EPUB + 16 office docs are unscanned. The audit directly scanned all 22 EPUBs and found NO watermark strings (a scan-coverage gap, not a live exposure); the .doc/.docx residual is unchecked. Widen both checks. The 2026-07-02 maintainer directive (watermarks always removed and never in the scratch repo; condition 3 of the keep-tracked decision) makes the item-29 widening load-bearing rather than hygiene: the widened scan is the mechanical enforcement of the scrub promise, so it covers every tracked binary format, not `.pdf` alone.
...
- **Reference-base work (`grc_library_ref`)**: 3 open items (SR-1 last_checked, decided but egress-gated; SR-2 screening-record check; SR-3 validate.py binary-scan gaps), plus the 2026-07-05 `originals/README.md` stale-snapshot finding (scoped in pending-decisions). SR-4 (catalogue/extract hygiene) closed 2026-07-05 (pre-split, scratch PRs #104, #105).
