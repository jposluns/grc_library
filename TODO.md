...
- **Maintainer-directed running order**: FR-167 (comprehensive matrix) is COMPLETE (closed 2026-06-30). The **NIST SP 800 ingestion** and the **OT post-ingestion validation** (formerly top of P1) are also COMPLETE (2026-06-30; the ingestion landed in `grc_library_scratch`, the validation in #495). The current order, superseding the original 2026-06-23 sequence: (1) the remaining P1 quality backlog (FR-58, FR-59, FR-60, FR-70), fix-issues-first within the tier; (2) the **deferred-decision work** (DD-2..DD-12, queued as ordinary items in P2/P3) and the deepen-baselines; (3) **reduce the backlog by treating the smallest items in batches**, using 10+ well-briefed research-assistant agents (workers produce verified research from briefs built off [`.working/worker-brief-template.md`](.working/worker-brief-template.md); the orchestrator re-verifies every worker claim at apply-time and authors all final prose). The standing **fix-issues-first** directive (2026-06-27) governs within each tier.
