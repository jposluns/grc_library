...
Items 12-14 and 16-19 closed in the C8 PR (stricter-safe or canonical-source resolutions, each logged in its CHANGELOG entry); item 11 (the GDPR DSR clock wording) closed in the DSR-clock PR. Remaining: the two routed notes below, both maintainer-ANSWERED at the 2026-07-02 overnight round and queued to their executing PRs (the composed-retention reword to the small-fixes batch; the acceptance-authority note, three carriers, to the section-1.3 DPO role-separation sweep).
...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half shipped in the batch-A PR and the judgment tier (the rollout-phasing note, the two retention resolutions, the two genericizations) in the batch-B PR; the original batch's items have all shipped (batch A, batch B, the coverage-gaps re-grade, L-j, and L-k, each in its own PR); the remaining bullets are the third coverage-gaps triage batch, the routed #585 pre-push verifier finding, and the routed #588 fit-pass candidates below (the COBIT fit-pass recode PR shipped) (the coverage-gaps second batch and the lifecycle-vocabulary fork each shipped in their own PRs). (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)

- **(routed, #588 pre-push verifier S-1 plus N-1, M, S) Ad-hoc fit-pass candidates exposed by the recode PR.** [`supply-chain/procedure-supplier-due-diligence.md`](supply-chain/procedure-supplier-due-diligence.md) (:23 alignment sentence, :159 references entry) cites COBIT 2019 BAI05, Managed Organizational Change, in the same supplier context whose BAI05.02 carriers #588 recoded to APO10.05 (likely target APO10 Managed Vendors; the title is canonical so gate 61 passes it, the FIT is the question), and the canonical-title restores expose two row-fit questions in [`compliance/policy-compliance-and-audit-management.md`](compliance/policy-compliance-and-audit-management.md) (the Compliance-governance row vs MEA01.01 Establish a monitoring approach; the Audit-planning-and-execution row vs MEA01.02 Set performance and conformance targets). The [`governance/register-digital-trust-and-assurance-metrics.md`](governance/register-digital-trust-and-assurance-metrics.md) MEA01/MEA02 descriptive-gloss row rides along for a fit read. Judge per the /matrix-fit ad-hoc cadence; any recode ships per the standing recodes-as-their-own-PR pre-authorization.
