...
- **`docs/` house-style (em/en-dash) enforcement gap (Low, S) — surfaced 2026-06-29 (Sweep 76, Finding 2)**: `docs/` authored prose carries 71 em/en-dashes across 4 files ([`docs/decision-tree.md`](docs/decision-tree.md) 58, [`docs/adopter-guide.md`](docs/adopter-guide.md) 5, [`docs/template-quickstart.md`](docs/template-quickstart.md) 4, [`docs/template-implementation-roadmap.md`](docs/template-implementation-roadmap.md) 4), which violate the CLAUDE.md house-style ("em-dashes and en-dashes are forbidden in prose") but sit outside [`tools/lint-language.py`](tools/lint-language.py)'s path scope (its `main()` default set is `AUDITED_DOMAIN_DIRS` + named files, not `docs/`), so gate 51-class dash enforcement never sees them. **Maintainer decision pending** (recorded in [`.working/pending-decisions.md`](.working/pending-decisions.md)): (A, recommended) extend the dash check to `docs/` and fix all 71 — closes the gate-blind gap so it cannot recur; (B) document `docs/` as an intentional house-style carve-out (no fix); (C) one-time fix the 71 without extending the gate (leaves the gap open). Out-of-window (pre-existing); routed per the `/validate` out-of-window protocol rather than auto-fixed.
...
Decided 2026-06-23 (maintainer triage): **build them all** from the 2026-06-22 review (S3 remains; the original S1 retention-consistency gate shipped in #462, and S2 was closed in #463 as a register consolidation, the role-consistency check already existing as gate 8 `lint-roles.py`); **S4 added 2026-06-29** (maintainer-endorsed, from the #458 `/retro` / Sweep 75). Each is a `lint-*.py` + 4-surface wiring + regression fixture; one gate per PR to keep diffs reviewable.
