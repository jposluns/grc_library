...
- **FR-167 (H, L)**: Expand [`compliance/matrix-grc-compliance-alignment.md`](compliance/matrix-grc-compliance-alignment.md) to **comprehensive** coverage across all 8 framework columns (CSA CCM v4.1 / ISO 27001:2022 / NIST CSF 2.0 / CTPAT / PIP / BASC v6 / WCO SAFE / AEO-S), honest N/A where a customs/trade column does not apply; the orchestrator authors and verifies every cell at apply-time (a wrong control mapping is not gate-caught). **All 11 per-domain batches have shipped** (architecture through privacy, #275 to #404; see [`.working/DONE.md`](.working/DONE.md)). **Remaining**: the matrix's partial sections, then matrix completion, then the closing whole-matrix `/matrix-fit`. **Maintainer decision (2026-06-28 resume): ADD a dedicated CSA AICM v1.1 column** for the AI-heavy rows whose tightest fits are AICM-only controls (reverses batch 10's interim CCM-v4.1-only choice). This is a matrix-wide structural change across ~250 rows, NOT partitionable, single-session apply; the per-row AICM-code *research* fans out by row-group against the scratch `ref/standards/CSA/` AICM v1.1 / CAIQ extracts and [`tools/ccm_aicm_reference.py`](tools/ccm_aicm_reference.py) (gate 48/49 ground truth), orchestrator applies every cell serially and runs the closing whole-matrix `/matrix-fit`. Sequenced FIRST this session per maintainer direction ("close FR-167 out earlier").
