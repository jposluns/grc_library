...
- **Last validation sweep**: Sweep 30 iter 1 (the `/resume` compensating-control corpus-wide `/validate` for the session-closing checkpoint handoff PR #276; full three-subagent dispatch A/B/C; clone unshallowed first; mechanical baseline **47/47**; re-examined the whole corpus + the #275-#276 deltas; **0 in-window findings**; **1 NEW out-of-window finding** — five documents cite a nonexistent "CSA Cloud Controls Matrix **v5**" (spelled-out form that evaded `lint-citations.py`'s abbreviated "CCM v5" entry; present since the initial release, never caught by Sweeps 22-29); maintainer authorized fix-now, shipped as **PR #277** (v5→v4.1 on all five, +EKM→CEK on the key-escrow row, + gate 27 broadened with a "Cloud Controls Matrix v5" denylist entry + regression test); apply-time worker-correction logged (orchestrator grep corrected an incomplete/mis-classified subagent finding set). Prior: Sweep 29 iter 1 (the #271-#274-window `/resume` control) — 0 in-window fixes, 2 notes (A's §6 gate-39 P8 prose gap dismissed at apply-time; B's operational-risk-register impact-5 "Severe" label → DD-11). Sweep 28 iter 1 (the #270/#274-window `/resume` control) fixed `resilience/plan-it-disaster-recovery.md`:97 Tier-2 backup-cadence contradiction (#271) and routed the operational-risk-register "Moderate" scale to DD-11; Sweep 27 (the #270 final pre-resume sweep) fixed 1 in-window note + addyosmani observation → DD-10.
...
- **DD-1** → **SHIPPED in PR #278 (new-entries-only)**: added PR-time delta gate D3 ([`tools/check-changelog-dash-on-pr.py`](tools/check-changelog-dash-on-pr.py)) that flags em/en dashes in lines a PR ADDS to [`CHANGELOG.md`](CHANGELOG.md); history left untouched. Scope was narrowed from the original "extend to all of CHANGELOG + fix existing entries" at action time, after investigation found ~130 dashes (not the ~5 the backlog item assumed) making a whole-file rewrite disproportionate. Rotated to [`.working/DONE.md`](.working/DONE.md).
...
- **DD-9** → **SHIPPED in PR #279**: broadened the DR plan's "Backup and restore requirements" header to "All systems must have:" (its bullets already span all four tiers). Rotated to [`.working/DONE.md`](.working/DONE.md).
...
- **DD-11 (from Sweep 28, extended Sweep 29, low, out-of-window)**: `risk/template-operational-risk-register.md`:123-127 + :222 uses its own ordinal scale — likelihood labels `Very low / Low / Moderate / High / Very high` with `Moderate` as the middle likelihood label AND as a rating label (:222) — diverging from the canonical ERM scale (`Very Low → Very High` likelihood + `Low / Medium / High / Critical` ratings, FR-134). Not in FR-134's named scope; line 119 self-scopes it ("this template proposes the following ordinal scales; adopting organisations may calibrate or replace them"), so it may be an intentionally-distinct operational-risk surface. Decide: harmonize the operational-risk-register to the canonical likelihood/rating vocabulary, or keep its distinct self-replaceable scale. **Extended by Sweep 29 (2026-06-23)**: the same template's impact-scale table (:129-135) labels the impact-5 (top) tier `Severe` (:135), a third corpus label for the conceptual top tier (the canonical standard uses `Catastrophic`; the DD-3 surfaces use `Critical`). DD-11's original 123-127/:222 scope did not enumerate this impact table, so the impact-label decision folds in here. (Ties directly to DD-2, which flags the same `Severe` impact label at `supply-chain/register-concentration-risk.md`:95, and to DD-3's impact-5 label question. Adjacent to DD-2/DD-3, the risk-vocabulary-drift cluster; triage the three together.)
- **DD-12 (from FR-167 batch 1, low, out-of-window)**: the compliance alignment matrix ([`compliance/matrix-grc-compliance-alignment.md`](compliance/matrix-grc-compliance-alignment.md)) carries two pre-existing framework-taxonomy inconsistencies surfaced while expanding it under FR-167. (1) **CSA CCM column**: row :103 (Network Security and Segmentation) uses `NET-01..06`, but CSA CCM **v4.1** (the corpus's canonical version, cited 122× vs v4.0 once) has no `NET` domain — network controls live under `IVS`; the corpus standardizes on `IVS` (33 uses) and uses `NET` essentially only in that one matrix row. (2) **NIST CSF column**: titled "NIST CSF 2.0" but the existing rows use `PR.IP` (e.g. :53, :90, :94, :102, :104, :107), a **CSF 1.1** category that CSF 2.0 removed (redistributed into `PR.PS` Platform Security + `ID.IM`); `PR.PS` has zero corpus hits, so `PR.IP` is the established corpus-wide convention. Per the maintainer's FR-167 decision (2026-06-23), new rows **mirror the corpus convention** (CCM v4.1 domains incl. `AIS`/`IVS`/`IPY`; `PR.IP` where the matrix does) to keep the matrix internally consistent, and these two anomalies are routed here rather than mixed into the expansion. Decide: (a) correct the `NET` row to `IVS` (small, matrix-local); (b) decide whether the corpus-wide `PR.IP`-under-CSF-2.0 convention should migrate to strict CSF 2.0 codes (a larger, corpus-wide question, since `PR.IP` is used well beyond this matrix); or (c) leave both as documented conventions. Note: option (b) is corpus-wide, not matrix-local.
