...
### 4.6a FR-167 matrix control-code-validity gate (M, M) — surfaced 2026-06-24 (maintainer asked)

The maintainer asked whether FR-167's control mappings should be gated (they are currently un-gated; the FR-167 item notes "a wrong control mapping is not gate-caught"). **Recommendation: yes, a control-code-validity linter, with a clearly-bounded scope.**

- **What it CAN gate (mechanical, worth building)**: every framework-control code cited in [`compliance/matrix-grc-compliance-alignment.md`](compliance/matrix-grc-compliance-alignment.md) is well-formed AND exists in that framework's known control set (CSA CCM v4.1 domain+control IDs; ISO/IEC 27001:2022 Annex A `A.5`-`A.8` codes; NIST CSF 2.0 function.category codes; CTPAT/PIP/BASC/WCO SAFE/AEO-S identifiers). This catches hallucinated codes, mistyped codes, and stale-catalogue codes (the #313 GRC-05-class mis-mapping and the AICM-vs-CCM confusion are exactly this class). Built the project way: `tools/lint-*.py` + four-surface wiring + regression fixture. Honest-backstop framing (it enforces code *validity*, a presence/format property, per [`gate-discipline`](dev-security/claude-rules/governance/gate-discipline.md)).
- **What it CANNOT gate (stays the orchestrator's apply-time job)**: whether a given doc *semantically* belongs under a given control. Semantic correctness is a judgement call, not mechanically checkable; the gate reduces but does not remove the un-gated risk, so the apply-time author-every-cell discipline still stands.
- **Sequencing**: build it as its **own PR** (a new linter is its own coherent change per "always split"), ideally **before batch 4 (supply-chain)** so the 18 new rows are validated on creation rather than retrofitted. Needs the per-framework known-code reference sets assembled first (the CCM v4.1 and ISO 27001:2022 sets are already partially encoded in gate 48's CCM/AICM checker — reuse, don't duplicate).
- **Relation to the bookkeeping-parity family**: distinct from §4.6/§4.10 (those gate QA-cadence/rotation *bookkeeping*); this gates matrix *content validity*. Not part of that family; its own gate.
