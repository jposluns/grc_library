...
- **Last validation sweep**: Sweep 24 iter 1 (the `/resume` compensating-control corpus-wide `/validate` for the session-closing handoff PR #255; full three-subagent dispatch A/B/C; 3 in-window should-fix findings, all in `dev-security/claude-rules/README.md` — the ninth-rule omission in the "two areas" enumeration plus two directory-tree descriptions still on the superseded single-tier routing phrasing, all corrected; 2 pre-flight candidates confirmed false positives; Subagent C clean; closed out in PR #256).
...
**Canonical values LOCKED 2026-06-22 (maintainer "accept all 6 as recommended"; apply these in the criticals remediation batch):** FR-134 → the **procedure's** scoring (`1-4/5-9/10-16/17-25`, likelihood "Very Low→Very High"); align the standard's §5.2 to it (template already agrees + cites the procedure). FR-135 → **TLS 1.3 everywhere**; migrate all four residual "1.2 minimum" surfaces including `operations/standard-production-security-requirements.md`:184 (B2B/EDI adapter) **unconditionally — no partner exception**. FR-136 → `governance/register-data-retention-schedule.md` is **authoritative**, tiered by log-class; change logging-standard:92's flat "seven years" to "per the retention schedule" and align records-standard:77. FR-137 → DSAR retention **3 years** (align records-standard:79 + the privacy procedure). FR-138 → scrub CPPA-as-live; cite **PIPEDA Schedule 1** principles (not invented section numbers) + a "CPPA pending reintroduction" note; consider Quebec Law 25 where rights are stronger. FR-139 → **1h RPO is binding; line :99's "24h for Tier 1" is the error** (tighten Tier-1 backup cadence to meet 1h; 24h belongs to lower tiers).
...
- **[fitness:P4] FR-137 (H[critical])**: DSAR-retention conflict — retention-schedule:74 ("3 years") vs records-standard:79 ("2 years post-closure") vs [`privacy/procedure-data-subject-rights-management.md`](privacy/procedure-data-subject-rights-management.md):335.
- **[fitness:P9] FR-138 (H[critical])**: CPPA cited as in-force — [`privacy/procedure-data-subject-rights-management.md`](privacy/procedure-data-subject-rights-management.md):66 (CPPA s.63 et al as live DSR basis), [`privacy/procedure-data-protection-and-privacy-breach-response.md`](privacy/procedure-data-protection-and-privacy-breach-response.md):171, [`privacy/policy-privacy-and-data-governance.md`](privacy/policy-privacy-and-data-governance.md):116 vs [`privacy/jurisdictions/annex-privacy-canada.md`](privacy/jurisdictions/annex-privacy-canada.md):28 ("Bill C-27 lapsed… CPPA is not in force").
- **[fitness:P7] FR-139 (H[critical])**: DR RPO vs backup-gap self-contradiction — [`resilience/plan-it-disaster-recovery.md`](resilience/plan-it-disaster-recovery.md):49 (Tier 1 RPO 1h), :99 (24h gap allowed), :101 (gap>RPO = P2 escalation).
...
- **Trust-recovery routing-convention revision (DECIDED 2026-06-22, this session; next substantive PR)**: route trust-recovery findings **tiered by severity — H[critical]+High → top-priority tier (P1), Medium+Low → next tier (P2)** — replacing the rule's current "every finding to top priority regardless of severity"; keep the no-silent-drop principle and maintainer sign-off. Scope = "routing flag only": revise the **convention** across ~8 surfaces (the `trust-recovery-escalation.md` rule + its `.claude/rules/` mirror; `deep-qa-review` SKILL + `/full-qa` command; `library-fitness-review` SKILL [the queued routing-flag amendment folds in here] + `/fitness` command; the pack `CLAUDE.md` + project `.claude/CLAUDE.md` rule-description bullets; check pack README). Pack rule/SKILLs stay project-agnostic ("top-priority tier" / "next-priority tier"); project P1/P2 mapping lives in `.claude/CLAUDE.md` + TODO. TODO P1/P2/P3 structure unchanged. Pack version bump. See `.working/design-decisions.md` "Trust-recovery findings routing".
