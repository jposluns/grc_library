...
**HOLD, maintainer-gated (2026-07-11).** Do NOT begin any phase, and do NOT apply anything from the existing `grc_library_scratch` drops (the `inbox/worker-20260708-fable/gr-p2-rule-condense*` tranches and their `APPLY-ADDENDUM.md`, or `inbox/claude-pack-hygiene/`), until the maintainer gives an explicit OK. A consolidated scratch drop (single master plan + phased payloads) is forthcoming from the pack-hygiene worker and SUPERSEDES those old drops; starting from the old drops would duplicate and conflict with the consolidated one.
