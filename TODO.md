...
- **(GR-11, L, XS) Preflight-changelog internal hygiene.** [`tools/preflight-changelog.py`](tools/preflight-changelog.py) uses two inconsistent code-span parsers in one file (the multi-backtick-run regex vs the manual single-backtick walk). The exit-code half of the original bullet was refuted at guardrail r3 (the 0/1/2 convention IS documented in the docstring, dated to the file's creation commit); the two-parser half stands.
...
- **(r3 O-F2, [guardrails], L, XS) Gate-9/gate-12 boundary documentation.** The two gates share six uncertainty-marker tokens across two independently-maintained lists with different exempt sets, and neither docstring references the other (the only same-token family pair with no documented boundary). Fix: one boundary sentence in each docstring (presence-vs-conjunction split, each gate's residual); optionally hoist the shared tokens into a common constant.
