...
Six acronyms used in [`docs/decision-tree.md`](docs/decision-tree.md) neither expand at first use nor resolve in [`governance/register-glossary.md`](governance/register-glossary.md): CPRA (line 83), PIA (line 140), BC and CM (lines 145-146, the BC/DR and BC/CM compounds), and ML plus bare AI (line 99's AI/ML compound and further bare-AI uses at lines 53/91/237; the #637 post-merge sweep's fold, which also names the alternative adjudication: treat AI as a lexicalized household term and strike BOTH halves of the compound from the list, the applier's call to surface). Add glossary rows (and inline expansions where first-use style warrants); check all six against other adopter-facing docs/ files while in the glossary.
