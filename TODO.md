...
- **Last validation sweep**: Sweep 29 iter 1 (the `/resume` compensating-control corpus-wide `/validate` for the session-closing handoff PR #274; full three-subagent dispatch A/B/C; clone unshallowed first; mechanical baseline **47/47**; re-examined the whole corpus + the #271-#274 deltas; **0 in-window fixes**; 2 notes — A's `governance/specification-audit-programme.md`:130 §6 gate-39 prose-vs-docstring P8 gap **dismissed at apply-time** (representative-examples prose + "and similar idioms" hedge, accurate as written), B's `risk/template-operational-risk-register.md`:135 impact-5 "Severe" label **routed to DD-11** (out-of-window; ties to DD-2/DD-3 risk-vocabulary cluster); all pre-flight candidates confirmed false positives; batched into the next substantive PR. Prior: Sweep 28 iter 1 (the #270/#274-window `/resume` control) fixed `resilience/plan-it-disaster-recovery.md`:97 Tier-2 backup-cadence contradiction (#271) and routed the operational-risk-register "Moderate" scale to DD-11; Sweep 27 (the #270 final pre-resume sweep) fixed 1 in-window note + addyosmani observation → DD-10.
