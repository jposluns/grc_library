...
- **FR-140 (H, M)**: Adopter starter-set divergence — quickstart:33 (6) / adopter-guide:115 (15) / decision-tree:104 (23) / README:271 (~37); Tier 1 omits the IAM + acceptable-use the quickstart floor requires. (The quickstart-vs-startup-roadmap "different 6th artefact" sub-part was resolved in PR #303, option A: the enterprise-governance-and-risk-management policy is now the named risk floor artefact in both, with register-population as guidance; the remaining FR-140 scope is the starter-set *count* divergence and the Tier 1 omissions.)
...
- **DD-8-followup / go.md TLS precision (Low, XS) — surfaced 2026-06-24 (Sweep 40)**: [`dev-security/claude-rules/languages/go.md`](dev-security/claude-rules/languages/go.md):200-202 groups `PreferServerCipherSuites` with `CipherSuites` as applying "only to TLS 1.2 and below"; per Go's `crypto/tls` docs `PreferServerCipherSuites` is deprecated and ignored entirely (distinct from `CipherSuites` being TLS-1.2-scoped). Reword the comment so the deprecated/ignored status is stated. Pack-prose change (bumps the pack version + lint-language pre-flight). Maintainer chose track-as-follow-up over fix-now (2026-06-24); the #318 load-bearing claim is accurate, only the secondary-field characterization is imprecise.
