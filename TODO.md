...
- **Project-governance separation Phase 1 (M, M)**: execute the citation-verification cluster migration defined in [`governance/specification-project-governance-separation.md`](governance/specification-project-governance-separation.md) §8.1. Create `.project-governance/` (NOT exempt; fully audited) and its `README.md` index; move the six §5.2 artefacts (`register-citation-verifications.md`, `register-citation-verification-bundle.md`, four `worklist-citation-verification-batch-*.md`); apply the §7 changes (corpus index → corpus-only; generators exclude `.project-governance/`); re-point the two path-targeted linters (`tools/lint-citation-verification-freshness.py`, `tools/lint-citations.py`) + `tests/test_linters.py`; sever/rework every corpus→project inbound citation first per §8.2. Single PR scoped to the move.
