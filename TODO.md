...
- **(GR-13, L, XS) D5 closure-pattern coverage for GR-form ids.** The D5 closure patterns ([`tools/check-todo-rotation-on-pr.py`](tools/check-todo-rotation-on-pr.py):85-104) match `FR-N CLOSED` but not the guardrail-intake `GR-N` id form (verified during the #555 pre-push verification: "GR-2 closed" matches no pattern), so a GR-item closure narrated without rotation would pass D5; the rotation convention is the only guard. Add the `GR-\d+ CLOSED`-form (and consider a generic `[A-Z]{2,3}-\d+ CLOSED`) to the pattern set + fixture.
