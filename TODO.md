...
- **(r5, [guardrails], protected-touch, S each) CLAUDE.md close-out-clause bundle (three shipped, one remaining).** SHIPPED: (1) the attended-autonomous rule-3 fallback reword dropping the longer-check-in option, in #652; (2) the close-out checklist's reconcile-bullet clause naming D7 as the mechanized version-token half (the gate-50 naming pattern), in #659; (3) the summary/description-lag close-out-checklist bullet (the #650-#653 /retro candidate, past the codification threshold at four in-window occurrences), in #659, grouped with (2) as both edit the same close-out-checklist section (the maintainer's 2026-07-05 group-by-logical-fit direction). REMAINING: the /claim-fit cadence-section clause (#630 out-of-window observation), deferred to the next session after a research pass to spec it (the maintainer chose wrap-and-resume-fresh at the #659 batch boundary, so #660 is the bookkeeping-only session-closing handoff and this substantive CLAUDE.md clause is not a handoff-PR change).
- **(#637 verifier F3, L, XS) D5 eighth escape form.** The bare closure phrasing `TODO N.M closed` (no `section` word, no `§`, no CLOSED marker) matches none of D5's seven forms; #637's lead was reworded to a detected form pre-push, but the phrasing is natural and will recur. Widen `tools/check-todo-rotation-on-pr.py` with an eighth form (backlog-file name then a bare decimal section token then a closed/closure word in the same clause, tolerating short intervening runs such as the #639 lead's "TODO 3.21's build-only half closed", forward-only window) AND a link-tolerant widening of the existing rotation-assertion form 6 (a markdown-linked DONE target defeats the literal, the #607 shape again: #640's "rotated to [`.working/DONE.md`](...)" was the NINTH escape phrasing), full-history FP census per the D5 convention, plus fixtures. The r5 gap lens adds the design-shape proposal for the same build: a DIFF-SIDE companion trigger (a PR whose TODO diff deletes a `###` heading or item block must carry the DONE ledger in the same diff, firing on the closure EVENT rather than its narration, closing the open-ended phrase tail from the other side; the existing `TodoRotation:` trailer covers the renumber/rescope false-positive cases, and the D8 candidate's heading-deletion detection is shareable code). **Census DONE and DAYTIME-DEFERRED (2026-07-06 overnight, protected-entangled):** the full-history FP census validated both widenings as false-positive-free: (form 8) `\bTODO \d+\.\d+(?:[^.\n]|(?<=\d)\.(?=\d)){0,80}?\bclos(?:ed|ure)\b` (case-insensitive, forward-only, dot-between-digits tolerance; the sibling of form 7 minus the `section` word; 16 historical hits, all genuine same-PR closures, zero negation FPs), and (form 6 widening) `(?<![Nn][Oo][Tt] )\brotated to (?:the DONE ledger|DONE\b|\[`?[^\]]*DONE)` (adding the markdown-linked target; 64 new hits, all genuine linked rotation assertions, zero bug-matches). The build is DEFERRED to the daytime protected-backlog clearance because the closure-form COUNT is restated in the protected [`.claude/CLAUDE.md`](.claude/CLAUDE.md) ("detects SEVEN closure forms", ~line 235), which must co-update to "EIGHT" in the SAME PR (multi-surface completeness); overnight mode cannot apply a protected edit, and shipping the linter/spec/tests while the CLAUDE.md count stays "seven" would strand a cross-surface inconsistency. Non-protected surfaces to co-update at build: the linter docstring + module comment ("seven" to "eight"), the `governance/specification-audit-programme.md` §6 D5 narrative (add form 8 + the form-6 widening), and fixtures.
