...
- **(GR-1, M, S) Extend gate 59 to CHANGELOG Library-Version ordering.** The change-tracking pack rule's CI-enforcement section describes a CHANGELOG version-monotonicity control that does not exist: gate 13 ([`tools/lint-library-version-monotonicity.py`](tools/lint-library-version-monotonicity.py):148-149) explicitly skips [`CHANGELOG.md`](CHANGELOG.md) ("Skip CHANGELOG (no Version metadata field)"), and gate 59 checks root/mirror header PARITY, not ordering. Extend gate 59 (it already parses both headers) to assert that the Library Version sequence in CHANGELOG entries is strictly decreasing top-down.
- **(GR-2, M, M) Regression tests for the PR-time delta checks and the pre-push guard.** [`tests/test_linters.py`](tests/test_linters.py) has zero references to [`tools/check-changelog-on-pr.py`](tools/check-changelog-on-pr.py), [`tools/check-version-bump-on-pr.py`](tools/check-version-bump-on-pr.py), or [`tools/check-changelog-dash-on-pr.py`](tools/check-changelog-dash-on-pr.py) (D1/D2/D3 untested); D5 is tested only at trigger-helper level; [`tools/pre-push-guard.sh`](tools/pre-push-guard.sh) is untested although its own header documents the #439 exit-code-swallow regression. Add temp-repo two-commit fixtures for D1/D2/D3/D5 and an exit-code chain test for the guard.
- **(GR-3, M, M) Shared `parse_metadata_block()` in [`tools/lint_common.py`](tools/lint_common.py) + fail-loud date parsing.** About a dozen linters privately re-implement metadata-block parsing. Gate 31's `DATE_RE` ([`tools/lint-document-date-staleness.py`](tools/lint-document-date-staleness.py):168-171) requires line-end after the date, so a trailing annotation ("**Date:** 2026-07-01 (note)") is silently counted `skipped_no_date` (:307,:325) and never staleness-checked: a silent fail-open. Introduce one shared parser (fail-loud on a malformed-but-present field) and migrate linters incrementally.
- **(GR-4, M, S) `iter_non_code_lines` fence coverage.** The shared fence toggle ([`tools/lint_common.py`](tools/lint_common.py):284-291) fires only on triple-backtick fences; a `~~~` fence or a stray unbalanced fence silently suppresses all subsequent scanning for the linters that iterate through it. Add `~~~` toggling and an unbalanced-fence warning.
...
- **(GR-7, L, XS) Docstring corrections in the working-prose-hygiene linter.** [`tools/lint-working-prose-hygiene.py`](tools/lint-working-prose-hygiene.py):4 cites lint-language as "gate 19" (it is gate 2), and :22 cites a stale `check-changelog-dash.py` filename (actual: [`tools/check-changelog-dash-on-pr.py`](tools/check-changelog-dash-on-pr.py)). Extends the §3.14 L-a docstring batch.
