...
P1 currently holds the **1.26** quality-machinery series (a goal-description umbrella with four independently-closeable phases, 1.26.1 to 1.26.4, on hold pending the Priority-3 tooling), **1.23** the trust-recovery validation-coverage build, and **1.19.13** the maintainer-gated history scrub (deferred). Behind the Egress-Gated divider sits **1.22**, the privatization / guardrail-tightening block. Point-fix items are opened and closed in place as they arise; the closed ones live in `.working/DONE.md`, not here.

### 1.26 Consolidate, harmonize, and distribute the quality machinery across AI toolchains (goal-description umbrella; multi-phase series; maintainer-directed 2026-07-26; ON HOLD pending all Priority 3 tooling) `[private]`
...
**Directional considerations (maintainer-flagged 2026-07-30, need full pros/cons analysis before the reconcile-and-distribute phases 1.26.3 / 1.26.4 execute).** The pack has matured to the point where its shape as a product is now a first-class decision rather than an afterthought. Three structural questions the maintainer wants weighed, with a full analysis, when the series reaches those phases:
- **Promote the pack from `dev-security/` to root-level prominence.** The rule pack has grown strong enough that its position nested under `dev-security/` understates its importance; the maintainer leans toward relocating it to a root-level home so its standing in the repository matches its weight.
- **Ship a ChatGPT / Codex-native equivalent alongside the Claude form.** Sharpens 1.26.4's tool-agnostic distribution into an explicit, first-class equivalent for the ChatGPT/Codex toolchain (not only a generic fallback form), so a team on that stack inherits the same guardrails in its own idiom.
- **Separate the pack into its own standalone public repository, cross-referenced from `grc_library`.** The maintainer is weighing splitting the pack out so that `grc_library` becomes the CONTENT plus the TOOLING THAT MANAGES that content for an adopter, and it FEEDS a standalone rules repository any project can adopt on its own, independent of the GRC corpus. This is the largest and least-settled of the three: it reshapes what `grc_library` is, so it wants the full pros/cons treatment (adoption friction, cross-repo sync burden, versioning and release cadence, discoverability, the ShareAlike lineage, and whether the pack's value is strongest bundled with the corpus or standalone) before any move.

The maintainer has explicitly deferred the decision ("this needs some directional considerations and I don't know what's best at the moment ... when we get there, I'll want your full analysis with pros and cons"), so this note captures the need and routes the analysis to 1.26.3 / 1.26.4 planning time. It consumes no new backlog number (guidance on an existing umbrella, per the multi-phase-series convention).

### 1.26.1 Consolidate and simplify the in-project machinery (H, XL) `[private]`
...
### 1.26.3 Reconcile with the public pack for true parity (H, M) `[public]`
...
### 1.26.4 Distribute in aligned forms across AI toolchains (H, XL) `[public]`
...
### 1.23 Trust-recovery build: validation-coverage + deep-assessment coverage of unvalidated operations (maintainer-directed 2026-07-24, H) `[private]`
...
### 1.19.13 History scrub (Phase 6; deferred, maintainer-gated; H, L) `[private]`
...
### 1.22 Privatization / guardrail tightening (maintainer-directed 2026-07-19, this-session block, outranks the queue; H, M) `[private]`
...
### 2.20 Ref-side `last_checked` sweep for the 6 new EU / CA AI sources (M, S; cross-repo) `[private]`
...
### 3.194 Worker-dispatch model-validity guard (fail-loud on unknown model + easy model-list update) (maintainer-directed 2026-07-30; M) `[machinery]` `[private]`
...
### 3.142 `/sitrep` situation-report command (maintainer-requested; dropped-and-recovered 2026-07-27, M, M) `[machinery]` `[private]`
...
### 3.144 Multi-orchestrator / self-refreshing orchestration (SCOPE/SPEC-GATED, needs maintainer discussion; M, XL) `[needs-decision]` `[private]`
...
### 3.147 Worker OAuth token refresh does not persist (the --worker-id snapshot deletes it), so accounts expire and need manual re-auth (2026-07-27, M, M) `[machinery]` `[private]`
...
### 3.146 Group-writable files re-appear in the READ-ONLY repos via the inherited default ACL, not umask (check_perms recurrence, 2026-07-27, M, S) `[machinery]` `[private]`
...
### 3.141 Per-account concurrency >1: config-dir snapshot remnant (in-flight registry DONE; verified #1295, M, M) `[machinery]` `[private]`

**PARTIALLY DONE (verified at source #1295):** the `max_concurrent` enforcement + in-flight registry/counter are ALREADY BUILT in `exec-dispatch.py` (`_reserve_slot`/`_release_slot`, `inflight.json` + `inflight.lock`, keyed by the account config-dir unit, with self-tests); the item's `stateless today` premise is stale. **REMAINING (the genuinely-open, HIGH-blast-radius half):** the per-worker config-dir SNAPSHOT (copy the auth/credential state, isolate session/lock/cache, with cleanup) that would let the wrapper's `$CFGBASE/$account/.wrapper.lock` loosen past its concurrency-1 cap without racing the shared `CLAUDE_CONFIG_DIR`/`CODEX_HOME`. This touches worker AUTH state (a bad snapshot could leave an account with zero usable workers), so it is deferred to a maintainer-reachable / high-assurance pass with the design's real-work-duplicated concurrency test (roadmap A validation requirement). Lower priority now that cross-account parallelism covers the immediate need. Design: `design-multiworker-per-account`.

### 3.192 Rework or retire `tools/pr-closeout.py` (retired-model residual from 3.137b) (2026-07-30, M, M) `[machinery]` `[MORNING-DECISION]` `[private]`
...
### 3.138 Worker full-suite `/validate` via a per-job writable checkout (2026-07-26, M, M) `[machinery]` `[private]`
...
### 3.139 Right-size CLAUDE.md (umbrella goal; not itself a task) (2026-07-26, M, L) `[machinery]` `[private]`
...
### 3.139.4 Phase 4: final pass to apex + behavioral core + lean index (2026-07-26, S, M) `[machinery]` `[private]`

**PARTIAL: the non-gated Tier-4 trims landed in #1253** (missing-reference-SOP / express-GO / clarify-para-1 -> pointer+overlay, 1274 -> 1239). REMAINS (MAINTAINER-GATED, do NOT do without asking): (a) demote the deliberately-LOUD `## Anything wrong` section to a pack pointer? (b) the Tier-3 wind-down-framework cut (`## Wind-down decision framework` / `## Attended-autonomous operating mode` / `## Wind-down pre-queues`, riskiest; the second-compaction gate + option-D have NO pack counterpart and must stay). Map: `grc_library_private/.working/dedup-3139-3/DEDUP-MAP.md`. Also optional: cosmetic tightening of the 4 longest rules-INDEX entries.
...
### 3.117 A maintainer-placed inbox drop is invisible to every instrument (2026-07-25, M, S) `[private]`
...
### 3.119 The Codex hook port is absent, so codex workers run unguarded (2026-07-25, M, S) `[private]`
...
### 3.148 Two gate-44 fail-open routes of the same class remain open (successor to 3.115; 2026-07-27, M, S) `[private]`
...
### 3.149 Colon-adjacent disposition ref is false-blocked by the open-findings grammar (successor to 3.126; validate-pr-1209 F-3; 2026-07-27, L, XS) `[private]`
...
### 3.150 Gate-50 `pending`-state recall hardening (successor to 3.120; #1210 pre-push verifier findings; 2026-07-27, M, S) `[private]`
...
### 3.153 Build the forward/reverse crosswalk pair-consistency GATE (the A.8.29 data outlier fixed #1257; enforcement remains) (2026-07-28 deep-assessment c2, ERROR->gate, S) `[private]`
...
### 3.154 Matrix `N/A` is an unaudited escape hatch + semantic-mapping blind spot (2026-07-28 deep-assessment c2, W, S) `[private]`
...
### 3.155 Cross-document statutory-date coherence is untracked beyond one term (2026-07-28 deep-assessment c2, W, S) `[private]`
...
### 3.156 Role-vocabulary closure does not cover body prose (2026-07-28 deep-assessment c2, W, S) `[private]`
...
### 3.157 Metadata `Classification`/`Confidentiality` allowed-value sets are unenforced (2026-07-28 deep-assessment c2, W, S) `[private]`

`lint-metadata.py` enforces `ALLOWED_TYPES` for `Document Type` only; `Classification` and `Confidentiality` are presence-only, so changing a doc to `Classification: Restricted` passes all 80 gates while contradicting the CC BY-SA public basis. Add allowed-value sets.

### 3.158 Gate-50 `SUBSUMPTION_FINDINGS` fail-open: incidental "not run" prose mis-classifies a row as exempt (2026-07-28 deep-assessment c3, W, S) `[private]`
...
### 3.159 Gate-50 naive pipe-split misparses ledger rows with inline `| pending |` code (2026-07-28 deep-assessment c3, W, S) `[private]`
...
### 3.160 An abbreviated `/validate-pr` stands as the QA-of-record for a gate-logic change (#1210) (2026-07-28 deep-assessment c3, both families, W, S) `[private]`
...
### 3.161 The GFM delimiter row in the QA ledgers sits at file-end, not after the header (2026-07-28 deep-assessment c3, W, XS)  [ROUTED 2026-07-30: operational-hazard (active-append gate-50-parsed ledgers, non-fleet session) + decide-correct-vs-document; pending-decisions] `[private]`
...
### 3.162 The deep-assessment register self-contradicts its own phase state (2026-07-28 deep-assessment c3, note, XS)  [ROUTED 2026-07-30: needs maintainer knowledge of r4 final state (signed-off vs superseded); pending-decisions] `[private]`
...
### 3.164 QA-ledger stated-rule-vs-content drifts (incl. bypass-log within-date ordering) (2026-07-28 deep-assessment c3, note, XS)  [ROUTED 2026-07-30: operational-hazard (active-append gate-50-parsed ledgers), non-fleet session; pending-decisions] `[private]`
...
### 3.166 `audit-delivery-status.py` is single-plane, missing live file-drop deliveries (2026-07-28 deep-assessment c4, codex, H, S) `[private]`

It reads only the scratch inbox manifest; live `collect-deliveries --dry-run` showed 47 file-drop deliveries while delivery-status reported "nothing to report" because scratch was absent, the same single-plane-blindness class. A "backlog cleared/applied" claim from it can omit live deliveries. Read both planes.

### 3.167 `collect-deliveries.py` fail-opens (TOCTOU, dry-run mislabel, collision race, EACCES traceback) (2026-07-28 deep-assessment c4, both families, M, S) `[private]`
...
### 3.168 `exec-dispatch.py` eligibility diagnosis collapses distinct causes into "no eligible account" (2026-07-28 deep-assessment c4, both families, M, XS) `[private]`
...
### 3.169 The permission-denied (EACCES) case is in none of the three delivery-tool self-tests (2026-07-28 deep-assessment c4, claude, M, S) `[private]`
...
### 3.175 Acquire the load-bearing references the reference base does not hold (2026-07-28 deep-assessment c5, both families, MAINTAINER-GATED) `[private]`
...
### 3.185 Deferred #1223 dual-family-codification prose nuances (2026-07-28 final claude verify of #1223, maintainer-deferred, L, S) `[private]`

The #1223 final dual-family verify surfaced 8 MEDIUM prose/precision nuances (M1 to M8) after the substantive dual-family requirement was confirmed sound; representative examples: M1 the substantive tier calls dual-family "the default" where stage 3 frames it as a structure; M2 `CONSEQUENTIAL` is undefined and the escalation axis now has several unreconciled thresholds; M4 the normative form is narrower than its family-generality rationale; M5 the rule-index enumeration surfaces; M7 the CHANGELOG's `six defects` is an unsourced integer; M8 a detailed-mirror file-reference-rule nit. Maintainer deferred these (2026-07-28) to "much later when there's nothing important to do." Full findings in the #1223 verifier logs (grc_working/logs). Low priority; prose only, the shipped requirement is verified correct.

### 3.114 The advisory CHANGELOG-length tool is looser than the gate that enforces it (2026-07-25, L, XS) `[private]`
...
### 3.113 Worker-id ownership layer-2 flip decision (BUILD DONE + live in both planes; verified #1295) `[machinery]` `[needs-decision]` `[private]`

**BUILD DONE (verified at source #1295):** nonce worker-id ownership validation is implemented and LIVE in BOTH transports. File-drop `credit-offload-filedrop.py` `guard_and_touch` runs layer 1 (warn-and-serve) and layer 2 (refuse) with `--takeover` and sticky-conflict tracking, called from claim/heartbeat/deliver; git-scratch `credit-offload-queue.py` `cmd_register` carries the `session_nonce`/`nonce_conflict` fields and withholds the dangerous `held_order`/`held_token` on a fresh mismatch. Layer 1 is the DEFAULT; layer 2 (refuse) is opt-in via `--enforce-id` / the ENFORCE env. The item's `remaining work is apply` is stale: the apply landed. **REMAINING (maintainer decision, NOT a build):** decide WHEN to flip layer 2 (refuse) on by default (registered in pending-decisions). The 2026-07-25 two-sessions-wore-one-id incident is DETECTED today (layer 1 warns + records a sticky conflict) and PREVENTED under opt-in layer 2.

### 3.109 Sweep 120: tooling-docstring and record-accuracy residuals (2026-07-25, L, S) `[private]`
...
### 3.3 CLAUDE.md removal-ledger review cadence (standing) (was 3.12) `[private]`
...
### 3.6 Register-ageing advisory tool (GR-8 follow-on A, guardrail review 2026-07-02, L, S) `[private]`
...
### 3.7 Expiry-tail one-pass batch review (GR-8 follow-on B, guardrail review 2026-07-02, IN PROGRESS, M) `[private]`
...
### 3.12 Bidirectional See-Also parity gate (r7 guardrails, M, M) `[private]`
...
### 3.13 Expand the mutation-probe variant library, structured-surface gates (deep-assessment r1 R10 remainder, S) `[private]`
...
### 3.31 `/reference-audit` per-touch staleness backstop (r6 guardrails, M, M; DEFERRED) `[private]`
...
### 3.33 Formalize the (severity, effort) convention across surfaces (S) `[private]`

The `(severity, effort)` tags are in use in this file and the scale is now stated in the header, but the convention is not yet propagated to its sibling surfaces. When the convention formally lands, update: `library-fitness-review/SKILL.md`; `validation-sweep/SKILL.md`; the `grc_library_private/.working/DONE.md` heading shape; future fitness-review templates and sweep detail files. Schedule: after the current FR backlog closes.

### 3.38 Broaden gate-39 count-idiom coverage (log-mining #272/#465, mechanizable half, S) `[private]`
...
### 3.54 Back-fill and surface the `doc_type` facet in grc_library_ref (maintainer-directed 2026-07-12; DISPOSITION 2026-07-24: populate-on-touch + low-priority back-fill; meticulous, maintainer-collaborative, explicitly NOT automated; L, XL) `[private]`
...
### 3.56 Pack-hygiene Phase-4 routed cleanups + mechanization candidates (pack-hygiene Phase-4 close, S-M) `[private]`

**Numbering: LEAVE AS IS (maintainer-decided 2026-07-25c).** This item is tracked as `3.56` while
`grc_library_private/.working/DONE.md` records three post-rule partial closes against `3.56a` (2026-07-24, PRs #1139,
...
### 3.57 Reference-breadth new-ingest apply (held-but-uncited over the 63 2026-07-12 ref sources, M) `[private]`
...
The reference-breadth new-ingest apply wave over the 63 2026-07-12 ref sources (the `grc_library_scratch` PR #163 delivery, `inbox/reference-breadth-2026-07-12/`) is **complete**: all High rows plus the maintainer-named version-sensitive `[V]` rows were applied across #866-#883, each verified verbatim against the held source, upstream-currency-confirmed this turn, refute-verified, and each with a canonical-citations register row (rotated to `grc_library_private/.working/DONE.md`). **ONE residual keeps this item open:** the deferred **matrix TSC-column mapping** (register-coverage-gaps line 170), a single-file sensitive change needing a `/matrix-fit` follow-up, not the from-scratch citation apply the wave was. When it is scheduled and closed, rotate §3.57 to DONE. See `grc_library_private/.working/pending-decisions.md`.

### 3.60 Per-process test-fixture tempdir to eliminate the concurrent-suite race (surfaced 2026-07-13, S; spun off from the read-only-git codification closed in #870/#871) `[private]`
...
### 3.73 Ledger-table-row-integrity check for the `.working/` bookkeeping tables (Sweep-103 `/validate-pr` escape #915, 2026-07-14, S-M) `[machinery]` `[private]`
...
item, found **five** fused rows live on `main` in `grc_library_private/.working/validate-sweeps/history.md`
(lines 44, 45, 46, 84, 103) and one in `grc_library_private/.working/matrix-fit/history.md`, confirmed two
...
### 3.121 A sweep order that strips `origin` makes the pre-push-guard claim un-re-derivable (Sweep 122 Part 5, L, XS) `[private]`
...
### 3.122 #1176 merged unvalidated; its `/validate-pr` must return and be dispositioned (maintainer-directed 2026-07-25c, H, S) `[private]`
...
### 3.123 `decisions-search.py` does not scan CLAUDE.md, so it false-negatives on maintainer directives (orchestrator-found 2026-07-25c, M, XS) `[private]`

`SEARCH_STORES` covers `grc_library_private/.working/pending-decisions.md`, the `_private`
design-decisions record and `grc_library_private/.working/DONE.md`. It does NOT cover
...
### 3.124 Two verified `grc_library_ref` catalogue defects, neither needing egress (worker-reported 2026-07-25, H and L, XS; cross-repo) `[private]`
...
### 3.125 Codex worker guard rails, phase 1: a heartbeat emitted by the work loop, and liveness derived not self-reported (maintainer-directed 2026-07-25c, H, M; cross-repo) `[private]`
...
### 3.127 `submit_state` residual paths beyond the fixed one (validate-pr-1176 E1 follow-on, M, S) `[private]`
...
### 3.128 The token-spend parser: per-worker display conflates measured and estimated (residual of validate-pr-1176 W1/W2, M, S) `[private]`
...
### 3.130 Portable pack form of the mid-session-death recovery protocol (`/restore-broken` companion, 2026-07-26, M, M) `[pack]` `[private]`
...
### 3.74 Standards-reference-format standardization (maintainer-directed 2026-07-14, M) `[private]`
...
### 3.75 Website-to-corpus link integrity: generated manifest + resolution gate + resolve-by-id (maintainer-confirmed 2026-07-15, M, M) `[private]`
...
### 3.79 For-AI page: give corpus coverage to the "named, not yet covered" instruments, then sync the page (maintainer-flagged 2026-07-15, M-L; corpus-first, NOT independent) `[private]`
...
### 3.88 Credit-offload worker-registry stale-entry auto-prune (maintainer-flagged 2026-07-17, credit-offload thread-7, S) `[machinery]` `[private]`
...
### 3.92.a `/resume` must consume the adopt-config validity flags detect-env already emits (S, S) `[machinery]` `[private]`
...
### 3.92.b Pre-flight maintainer-clone guard for `/adopt` (M, S) `[machinery]` `[needs-decision]` `[private]`
...
### 3.92.c Classification-coupling parity across four surfaces (L, S) `[machinery]` `[needs-decision]` `[private]`
...
### 3.94 Landing-page Contents sidebar overflows its viewport, hiding trailing links (maintainer-flagged 2026-07-17, L, S) `[website]` `[private]`
...
### 3.97 `grc_library_ref` `upstream_url` enrichment for FREE trusted-bucket entries (§1.19.7c follow-up, 2026-07-17, M; cross-repo `_ref`) `[machinery]` `[private]`

The `/adopt` `.ref` bootstrap planner (`tools/adopt-bootstrap-ref.py`, §1.19.7c) can only AUTO-FETCH a FREE source that carries an `upstream_url`; currently only 8 of the 534 FREE trusted-bucket entries have one (all legislation), so 526 FREE sources fall into the "free-manual" bucket. Enrich `grc_library_ref/catalogue.yml` with `upstream_url` for the FREE `standards`/`frameworks`/`programs` entries (authoritative download pages) so the adopter bootstrap auto-fetchable set grows materially. Cross-repo (`grc_library_ref` PR); the manifest + planner then surface the URLs automatically on the next regen. Egress/lookup-heavy (each URL confirmed at its authoritative source); can be partitioned to workers per the research-assistant discipline.

### 1.14 External-source currency detection, Layer B: scheduled upstream-signal sweep (AUTHORIZED 2026-07-25; low priority, after the backlog is caught up) `[private]`

**Layer A is DONE** (gate 72 plus `audit-register-currency.py`; closed 2026-07-25 on a measured
182-of-182 register coverage, see `grc_library_private/.working/DONE.md`). Only Layer B remains, and the
...
### 3.2 Authoritative standards register + designation-correctness gate (M-H, L; egress-gated) `[private]`
...
### 3.9 Require-registration citation-currency gate (GR-GAP-1, guardrail review 2026-07-02, M-H, M; egress-gated) `[private]`
...
### 3.42 New-ingest reference-breadth pass: Canadian/international AI-governance sources (post-ref-resync 2026-07-10, M) `[private]`
...
### 3.186 Extend the CCM/ISO/NIST/COBIT citation gates to title-check framework-alignment tables (Sweep 129 N1, 2026-07-28, M, S) `[machinery]` `[private]`
...
### 3.187 Codify dual-family (Claude + Codex) QA as a permanent project standard for ALL QA tasks (maintainer-directed 2026-07-29, M, M) `[machinery]` `[private]`
...
### 3.188 Widen delta gate D5 closure-form patterns to recognize the "Closes TODO N.M" word order (PR2b-3 dual-family verify, codex F5 / Fable F-10, 2026-07-29, S, S) `[machinery]` `[private]`

Codex F5 / Fable F-10 (PR2b-3 dual-family adversarial verify): the D5 delta gate's eight closure-assertion forms do not recognize the `Closes TODO 3.177` word order (a `clos...` verb then a bare `TODO N.M` token with no section marker and no `backlog item` phrase), so a CHANGELOG entry using that phrasing passes D5 vacuously ("no TODO-item closure asserted"). Harmless when `TODO.md` is in the diff anyway (as in PR2b-3), but a gate-coverage gap. Add a false-positive-free ninth form (a `clos...` verb then `TODO` then a bare decimal section token, forward-window, census-tested against the full CHANGELOG history) plus a regression fixture.

### 3.189 Harden pr-closeout `has_findings` detail-file path to the require-private resolver (PR2b-3 Fable verify F-6, 2026-07-29, S, S) `[machinery]` `[private]`

Fable F-6 (PR2b-3 verify): `tools/pr-closeout.py`'s `has_findings` detail-file branch resolves via `resolve_working_for_write` (the never-None, public-capable resolver) rather than `resolve_working_for_write_private`. Not a live defect (proven: `build_plan` raises `WorkingStateUnavailable` at the merge-bypass-log bump before this branch is reached, exit 3 clean, no public working tree recreated, self-test 47/47), and the recreate-guard hook is a mechanical backstop, but the writer contract here rests on statement ordering rather than the resolver, so a future `build_plan` reorder could silently reopen the public-recreate path. Cheap defence in depth: switch to `resolve_working_for_write_private` plus refuse, and add a both-absent self-test asserting the detail branch also refuses.
...
### 4.3 Overnight unattended-run driver (M, L) `[private]`
...
### 4.4 Worker-ready brief staging (slice 4) + `/subagent` external-worker entry (M, L) `[private]`
...
### 7.1 Ruleset `non_fast_forward` (force-push) rule (deep-assessment r1 R8b, maintainer-owned) `[private]`
...
### TF-2 Review the pack-parity coupling (convention + cadence), decide on the hard gate (2026-07-23, PR #1099) `[private]`
...
### SR-1 `last_checked` currency mechanism is inert (item 26, P2, S) `[private]`
...
### RB-8 reference-acquisition manifest regen (Sweep 128 loop-break `/validate`, claude lens; L, XS) `[private]`

The committed static reference-acquisition manifest (generated by [`tools/build-reference-manifest.py`](tools/build-reference-manifest.py), the source `/adopt` and `/resume` bootstrap a `.ref` from) is stale versus its sibling `grc_library_ref` catalogue: standards the maintainer ingested 2026-07-28 are not yet reflected, so an adopter or a `/resume` bootstrapping a `.ref` from the static manifest gets a reference base under-reporting the acquisition catalogue until it is regenerated. Surfaced as an out-of-window note by the Sweep 128 loop-break `/validate` (claude lens). Resolution: run `python3 tools/build-reference-manifest.py` and commit the regenerated manifest the next time the reference base is touched (a low-severity generated-artefact refresh; no egress needed).

### RB-R6 source-not-held acquisition (deep-assessment r1 R6, maintainer research-agent, S) `[private]`
...
| MEG-44 | 3.80 (closed #1295)/3.81/3.82/3.83/3.88 | Credit-offload design thread: several maintainer-flagged design rows (cross-repo scratch/design; split per row when worked). |
