...
Items 12-14 and 16-19 closed in the C8 PR (stricter-safe or canonical-source resolutions, each logged in its CHANGELOG entry); item 11 (the GDPR DSR clock wording) closed in the DSR-clock PR. Remaining: the one routed note below, maintainer-ANSWERED at the 2026-07-02 overnight round and queued to the section-1.3 DPO role-separation sweep (the acceptance-authority note, three carriers). The composed-retention reword shipped in the small-fixes PR.
...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half shipped in the batch-A PR and the judgment tier (the rollout-phasing note, the two retention resolutions, the two genericizations) in the batch-B PR; the original batch's items have all shipped (batch A, batch B, the coverage-gaps re-grade, L-j, and L-k, each in its own PR); the remaining bullets are the third coverage-gaps triage batch, the routed #588 fit-pass candidates, and the routed #589 verifier note below (the COBIT fit-pass recode PR and the lifecycle-pointer fix shipped) (the coverage-gaps second batch and the lifecycle-vocabulary fork each shipped in their own PRs). (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)

- **(routed, #589 pre-push verifier N-2, L, XS) Retirement-recording claim in the quality-cadence procedure.** [`governance/procedure-library-quality-and-review-cadence.md`](governance/procedure-library-quality-and-review-cadence.md) still says document retirement is "recorded in the changelog and the document architecture framework"; the #589 lifecycle-pointer rationale established that the framework carries no lifecycle content, so this adjacent recording claim needs the same fit-check (re-point, reword, or confirm the framework really is a retirement-recording surface). Pre-existing; surfaced by the #589 pre-push verifier.
