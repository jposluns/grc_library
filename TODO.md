...
- **AIDA-in-force framing** (from the corpus-wide AIDA scan). The reconcile of [`ai/policy-ai-compliance.md`](ai/policy-ai-compliance.md) §7.2/§12 to the AIDA-lapsed framing (Bill C-27 died at the January 2025 prorogation; Canada has no in-force federal AI statute, per the `grc_library_ref` legislation register) caught other carriers. The [`ai/checklist-ai-algorithmic-compliance.md`](ai/checklist-ai-algorithmic-compliance.md) carriers (the :122 provenance note AND the E2 control item) were reconciled to the lapsed/successor framing in WS 0.4 / #787. **Remaining for this item:** [`dev-security/standard-security-baseline-and-standards-reference.md`](dev-security/standard-security-baseline-and-standards-reference.md):115 presents "Canada's AIDA requires: ... Treat as near-term compliance target" as a live requirement (a genuine accuracy error, reword to lapsed framing + cite the in-force TBS Directive on ADM / ISED Voluntary Code). Bare-token grep `AIDA` corpus-wide before closing to confirm no other live-framing carrier survives.
