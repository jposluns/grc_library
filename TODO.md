...
`_read_inflight` in [`tools/exec-dispatch.py`](tools/exec-dispatch.py) returns `[]` on ANY error (missing, unparseable JSON, OSError, non-list), so a corrupt or unreadable `inflight.json` reads as EMPTY and lets a dispatch UNDER-COUNT in-flight jobs, a bounded fail-open on the registry cap. NOTE (validate-pr-1199 W1, corrected 2026-07-27): #1199 did NOT remove the wrapper flock (the root-owned wrapper is outside the repo); the WRAPPER flock still caps same-account concurrency at 1 today (see the wrapper comment in [`tools/exec-dispatch.py`](tools/exec-dispatch.py)). The registry becomes the SOLE per-account cap only when that wrapper flock is loosened at the WIRE-IN step, at which point this fail-open becomes live. It is bounded and self-healing (the atomic `os.replace` write makes corruption rare, and the next valid write repairs it), which is why the skeptical verifier graded it non-blocking, but it violates refuse-on-doubt. FIX: distinguish `FileNotFoundError` (-> `[]`, correct: no registry yet means no in-flight jobs) from a corrupt or unreadable file (-> fail CLOSED for RESERVE, refuse the dispatch with a clear operator message naming the file to remove; on RELEASE, NO-OP on corruption, leave the file for the operator, per W1 below). **W1 (Sweep 125, 2026-07-27, re-verified at source): RELEASE has its OWN fail-open, distinct from and worse than the RESERVE one, so "RELEASE may stay best-effort" was WRONG.** Current `_release_slot` reads via `_read_inflight` (fail-open `[]` on corruption) then `_write_inflight`s the filtered list, so a corrupt registry during a release persists `[]` and WIPES every live co-tenant entry across all keys. Unlike a leaked OWN entry (which pid-liveness reaping restores by dropping it), wiped co-tenants are NEVER re-added (entries append only at reserve), so the under-count is permanent -> over-admission at `max_concurrent > 1`. So the fix must ALSO make RELEASE not-write on corruption (return no-op, leave the file), which is exactly what the delivered fail-closed candidate does (`_release_slot` catches `InflightCorruptError` and returns without writing). MUST land BEFORE `max_concurrent` greater than 1 is enabled: both fail-opens are MOOT at cap 1 (the wrapper flock serializes same-account jobs, no co-tenant to over-admit or wipe), so they gate the enable step, not the #1199 merge. Umbrella A.
...
The #1158 post-merge sweep caught a false-completeness claim: the CHANGELOG called the reference-label strip "the eleventh and last fail-open route" when it is the TWELFTH (five closed in #1154 plus six in #1155 is eleven already, and the module docstring shipped in the SAME PR says twelfth) and, more importantly, **it is not the last**. The `validate-pr-1155` sweep reported four further non-prose routes that survive `content_tokens` in [`tools/lint-paired-skill-step-parity.py`](tools/lint-paired-skill-step-parity.py), each verified there by running the gate on a constructed case: **(a)** YAML front matter, **(b)** a table cell of pure metadata, **(c)** a link target containing a literal `)` (which `LINK_TARGET_RE`'s `[^)]*` stops short of), and **(d)** an HTML attribute value appearing after a `>`. **A thirteenth route, the image REFERENCE form `![alt][label]`, was found by the #1159 sweep and CLOSED in #1160** (its alt text leaked as prose because `IMAGE_RE` matched only the inline form, so the reference-form image fell through to the label strip, which keeps bracketed text as though it were link text). None was recorded in any backlog item, which is exactly the accepted-unverified-without-a-tracker failure the close-out checklist names; this item is that tracker. **Do NOT assume the class is exhausted when these four close:** the observed per-round base rate is 2, then 5, then 1, then 1, then 1, and **every completeness claim made about this class so far has been false**: "the last route" (#1158) was wrong, and its narrowed replacement "the last reference-form route" (#1159) was ALSO wrong, caught by the very next sweep. So treat each round as sampling an open class rather than draining a closed one, and make NO completeness claim about it at all, only an ordinal one, and keep the false-positive census as the gate on any widening. Route (c) is the most interesting, since it is a defect in an existing strip pattern rather than an unhandled construct.
...
`credit-offload-filedrop.py dispatch --order <f> --family <fam>` routes to a FAMILY by dropping the order into that family's `available-work/`, and claiming is first-come. There is no way to pin an order to a worker or to exclude one. The consequence is not theoretical: on 2026-07-25 the SAME worker claimed all three passes over PR #1151 (the pre-push skeptical verify, the post-fix re-verify, and the post-merge `/validate-pr`), so each check of the prior finding was performed by the worker that made it. The worker disclosed this unprompted all three times, which is the only reason it was visible. This is the concrete mechanism behind open `MAINTAINER_ALERT` 2026-07-24-a (verify-independence, asymmetric verify routing): the alert names the symptom, this item names the cause. A re-verify or an adversarial audit is structurally weaker when it lands on the author of the finding, because iteration one's blind spot is exactly what iteration two repeats. Options to weigh: an `--exclude-worker <id>` flag honoured at claim time; a `--pin-worker <id>` form that drops the order straight into that worker's inbox; or an order field the claim path checks. The pin form is simplest but reintroduces head-of-line blocking if the pinned worker is busy, so the exclude form is probably right. Whichever lands, the orchestrator also needs a convention: a re-verify names the worker to exclude, defaulting to the one that produced the verdict under review.

**Insights folded from four inbox drops (2026-07-25/26; moved to `done/drops/2026-07/`), for the exec-dispatch `--not-worker` design.** (1) **Exclude by DROP-authorship and TODO-source-authorship, not just delivery-authorship** (`issue-validate-pr-1179-released-independence-conflict`): when a PR dispositions or refutes an inbox drop, carry that drop's `From:` author into the QA order's exclusion; same for a TODO item created from a worker's drop, exclude the worker whose evidence the item records. The 2026-07-25/26 hand-backs happened because the exclusion was populated from delivery authorship only. (2) **The prio-0 self-verify DEADLOCK** (`issue-resume-validate-takeover-both-opus-conflicted`): a priority-0 order that no live worker can independently serve ping-pongs on release (claim always returns the highest-priority available order), blocking the whole fleet; the resolutions are an orchestrator self-run, a fresh unconflicted worker, or an order split. (3) The exec-dispatch harness already added `--worker-id` (#1199); the enforcement (exclude/require, TODO 3.113) is the remaining half. A related OLD-tool defect (`issue-validate-pr-1179-claim-stamp-storm`): `credit-offload-filedrop.py cmd_claim` appends a claim stamp unconditionally after `os.rename` (which overwrites on POSIX), so one transient claim loop grew an order file to 52 stamps; make the stamp idempotent (use `_set_field_lines`, which replaces). Likely moot for exec-dispatch (fresh worker per exec, no claim loop) but live for any remaining filedrop claim path.
