...
- **Last validation sweep**: Sweep 31 iter 1 (the `/resume` compensating-control corpus-wide `/validate` after the session-closing handoff PR #280; full three-subagent dispatch A/B/C; clone unshallowed first; mechanical baseline **47/47**; re-examined the whole corpus + the #277/#278/#279/#280 deltas; **0 in-window corpus findings**; 2 note-level gate-number mislabels in this file fixed in the close-out — the `lint-citations.py` hallucination gate is **gate 5** (not "gate 27"), and `lint-standards-currency.py` is **gate 6**). Prior: Sweep 30 iter 1 (#277) fixed five "Cloud Controls Matrix v5"→v4.1 citations and broadened gate 5's denylist; Sweep 29 (no in-window fixes; 2 notes → DD-11 / dismissed); Sweep 28 (#271) fixed the DR Tier-2 backup-cadence contradiction.
...
- **FR-142 (H, S) ⚠**: Two AI procedures name no roles for any step — [`ai/procedure-ai-model-risk-assessment.md`](ai/procedure-ai-model-risk-assessment.md):69-71 & [`ai/procedure-ai-system-impact-assessment.md`](ai/procedure-ai-system-impact-assessment.md):156-158.
- **FR-143 (H, S)**: Circular escalation chain DPO→CISO→DPO — [`supply-chain/procedure-supplier-onboarding-security-review.md`](supply-chain/procedure-supplier-onboarding-security-review.md):139.
- **FR-144 (H, S)**: Privacy-breach individual-notification leg has no internal clock — [`privacy/procedure-privacy-incident-and-breach-response.md`](privacy/procedure-privacy-incident-and-breach-response.md):169-174.
...
- **FR-146 [fitness:P3+P6] (M, S)**: ERM template sample rows use retired status "Implemented"/"Verified" — [`risk/template-enterprise-risk-register.md`](risk/template-enterprise-risk-register.md):91,169-175.
- **FR-147 [fitness:P4] (M, S) ⚠**: Audit final-report timeline 10d ([`compliance/standard-internal-audit.md`](compliance/standard-internal-audit.md):268) vs 15d ([`compliance/procedure-audit-planning.md`](compliance/procedure-audit-planning.md):366).
- **FR-148 [fitness:P4] (M, S) ⚠**: CAPA 90-day effectiveness validation ([`compliance/policy-compliance-and-audit-management.md`](compliance/policy-compliance-and-audit-management.md):85) not anchored in [`compliance/procedure-capa.md`](compliance/procedure-capa.md).
...
- **FR-100 (M, S)**: Cloud baseline cites control families not sub-controls.
- **FR-101 (M, S) ⚠**: Closure sign-off authority implicit (continuous assurance / 3LoD).
- **FR-102 (M, S)**: Change management binary (continuous assurance / 3LoD).
...
- **FR-77 (M, S)**: 3LoD model used without explanation (coverage-gap small).
- **FR-78 (M, S)**: Framework-document-architecture maintainer voice (coverage-gap small).
- **FR-83 (M, S)**: IC (independent challenge) checklist absent.
...
- **P2 (substantive improvements)**: ~40 items (Medium fitness findings + the former Phase 2 clusters + DD-2/3/4/5/6/7/8/11 + relocations + loop-break-generalize).
