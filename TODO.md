...
Routed from the pack-hygiene Phase-4 triage (full detail: [`.working/pack-hygiene-acceptance/2026-07-12-phase4-acceptance.md`](.working/pack-hygiene-acceptance/2026-07-12-phase4-acceptance.md)). (a) Three proposed cheap mechanizations (guardrail-machinery candidates, maintainer-decision): a linter scanning skill bodies outside the wiring section for internal-reference token classes (convention-erosion guard); an instrument mapping "gate N (name)" citations in wiring sections/command stubs to the §6 inventory (renumbering guard); the provenance register as a gate-41 enumeration surface (a new rule cannot ship without its entry). (b) Apply-time command-stub co-updates: the `/fitness` stub's ten-persona step heading; the `/retro` stub's concrete register format. (c) Small pre-existing accuracy cosmetics: vetting-log "Spot-scanned" vocabulary + the line-254 summary; the validation-sweep zero-finding record/register wording; the session-lifecycle "prevented by luck" phrasing; `.working/README.md` activities inventory stale for 8 pre-existing subdirectories; adoption-path numbering (pack 1/2/3 vs adopter-guide A/B/C); root-README "claude.md" lowercase option headings (anchor-safety check first). (d) Two defensible-judgement flags for the maintainer: the pack README "shaped by the parent library's real maintenance practice" wording; "authoritative" for the reference-audit motivating source. (e) Sweep-100-routed pack project-flavour notes (out-of-window, note-level, low-confidence; neither contradicts the narrowly-worded #846 asserted-clean property): [`dev-security/claude-rules/governance/change-tracking.md`](dev-security/claude-rules/governance/change-tracking.md) line ~73 uses `.claude/`/`.working/` directory paths as illustrative examples in a scrubbed pack rule (candidate to genericize to a placeholder path); [`dev-security/claude-rules/governance/project-integrity.md`](dev-security/claude-rules/governance/project-integrity.md) line ~98 names "the consuming GRC library's corpus principle document" (project-integrity was Phase-1 condensed, not Phase-3-scrubbed, so outside the acceptance-sweep scope; candidate to reword to "the consuming library"). Both are optional strict-project-agnosticism polish, protected-file class.

### 3.57 Reference-breadth new-ingest apply (held-but-uncited over the 63 2026-07-12 ref sources, M)

Apply-work (validate-then-apply) on the reference-breadth delivery in `grc_library_scratch` (PR #163, `inbox/reference-breadth-2026-07-12/`; MANIFEST + CANDIDATES). A worklist of held-but-uncited citation candidates from the 63-doc ref ingest, ~20 target documents (9 High). Run via `/reference-audit` new-ingest mode; the 9 High rows are the natural first slice (EDPB Guidelines 4/2019, 05/2020, 01/2022, 9/2022 to the matching privacy docs; Implementing Decision (EU) 2021/915 to template-dpa-article-28; ANPD Resolution 18/2024 to annex-privacy-brazil; SOR/2018-64 to annex-privacy-canada; AICPA TSP 100 to register-canonical-citations; NIS2 Implementing Regulation (EU) 2024/2690 to annex-nis-2-implementation). Candidates are hypotheses: re-read each target + source clause and re-confirm "not already cited" at apply (checks were at 8c62ac5; ai/ rows are in the actively-changing delta region). Version-sensitive [V] rows: confirm current upstream this turn. Split into coherent per-PR bundles (EDPB-privacy cluster, eIDAS annex, NIS2/CER pair, AICPA register row, non-EU jurisdiction annexes); the eIDAS/implementing-reg rows also add a canonical-citations row in the same PR; the matrix TSC-column mapping is the deferred single-file sensitive change (register-coverage-gaps line 170) needing a `/matrix-fit` follow-up. Non-urgent; independent of the AI-domain delta; schedule after the delta + website-critical-path work.

### 3.58 Delivery-pipeline reconciliation: recycled-number mapping + stale/consumed delivery disposition (surfaced 2026-07-13 overnight, M; maintainer-disposition)

The scratch-inbox delivery pipeline needs a reconciliation pass, surfaced when the post-3b queue reached the "apply-ready scratch deliveries" tier. [`tools/audit-delivery-status.py`](tools/audit-delivery-status.py) maps each inbox delivery to a backlog item by the TODO / coded token in its MANIFEST, but TODO numbers have been RECYCLED repeatedly (§1.2, §3.15, §3.19 among them), so a 10-day-old manifest's token now points at a different item than it did when written. Concretely: `changelog-root-reformat-build` (token 3.19) is CONSUMED, its `reformat-root-changelog.py` tool plus the stage-3b protocol built #855 through #862, while the current §3.19 is an unrelated worker-provenance-link item; `gr-10-history-gate-batching` and `gr-gap-1-register-population` both carry token 3.15, but the current §3.15 is the MITRE ATLAS map (GR-10 and GR-GAP-1 moved). So the tool's "13 PENDING" count is unreliable, and the `worker-20260703-a/*` deliveries are 10 days old (read-bases #618 / #718, drift risk). Reconcile each delivery by its DESCRIBED WORK against the CURRENT backlog (not its token) and give each a disposition, APPLY (work still wanted and the research still useful as input; validate-then-apply re-reading current state), SUPERSEDE (consumed or overtaken; remove the seed from scratch), or DISCARD (no longer wanted; route to maintainer). The per-delivery apply / supersede / discard call is maintainer-informed (which old research is still wanted is authorial), so it is surfaced in [`.working/pending-decisions.md`](.working/pending-decisions.md). Then clean consumed / superseded seeds from `grc_library_scratch` per the maintainer's "clean up scratch as consumed / if unusable due to being completed so long ago" directive. Do NOT auto-apply misnumbered or drifted deliveries unattended (trades Integrity for Speed under the AIQT tier).

### 3.59 Codify the read-only-git rule for validation/verifier subagent briefs (surfaced 2026-07-13 overnight, S)

A subagent that runs `git checkout` / `switch` / `reset` / `stash` in the SHARED working tree collides with the orchestrator's concurrent feature-branch work: at #866 the #865 `/validate-pr` subagent checked out the merge commit to judge it, which switched the orchestrator off its branch, so the next commit landed on local `main` (caught fail-loud at PR-create, repaired, remote `main` never polluted). The in-flight fix is to brief each `/validate-pr` and verifier subagent to inspect history READ-ONLY (`git show <sha>:<path>`, `git diff <a> <b>`, `git log`; never a checkout that moves the working-tree HEAD/branch). Codify it durably so it is not re-remembered each dispatch. **PROJECT surfaces DONE (#870):** the read-only-git constraint was added to the `/validate-pr` and `/validate` command stubs' subagent-dispatch steps and a CLAUDE.md standing note at PR-workflow step 5a. **REMAINING (pack-distribution half):** add the read-only-git rule to the skeptical-verifier brief in [`ai-assistant-workflow-disciplines.md`](.claude/rules/governance/ai-assistant-workflow-disciplines.md) (both trees, gate-37 parity, pack version bump) and, optionally, the `validation-sweep` / `validation-sweep-pr-scoped` skill bodies, so a fork adopter inherits it. Protected-file class. A secondary observation from the same incident: two subagents running `run_all_audits.sh` (which builds/removes `tests/tmp/` fixtures) concurrently race on those fixtures and produce spurious gate-36 failures, so verification runs should be serialized or the fixture tempdir made per-process (assess as part of this item).
