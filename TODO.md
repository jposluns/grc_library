...
- **(r4 D-F3, [guardrails], L, XS) Evidence-grounded-completion one-liner corollaries: remaining project-CLAUDE.md clause.** The three PACK surfaces (pack `dev-security/claude-rules/CLAUDE.md` rule-index one-liner, pack README tree blurb, pack README scope row) shipped in #656 (pack `1.54.6`), each gaining a condensed clause naming the rule's un-observable-state / inventory / external-version-currency corollaries. The remaining surface is the project [`.claude/CLAUDE.md`](.claude/CLAUDE.md) index bullet's matching clause; it rides the #657 r5 `.claude/CLAUDE.md` bundle (grouped with the r5 close-out clauses so all the project-CLAUDE.md touches land in one PR).
