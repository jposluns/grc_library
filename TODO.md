...
section stays open on (c) plus the next-session hook-firing validation noted below, and on
...
- **(c) (L, XS) CLAUDE.md widening (authorized-touch bundle).** Widen the RM-10 clause from
  ...
  retro's enumeration-grep checklist example and the #619 retro's handoff-refresh
  reconcile line (restored at r4: the session handoff asserted this bundle carried the
  #619 line, but the bullet had dropped it) on the same authorized CLAUDE.md touch. (The
...
- **(GR-8, M, S; half (a), the removal-ledger disposition pass, is COMPLETE: 14 of 15 entries dispositioned in the small-fixes PR and RM-10 dispositioned-codified in the section-1.6 D5 PR) Close the remaining watch-loop.** Retro proposed-improvements closure: [`.working/improvement-log.md`](.working/improvement-log.md) proposals accumulate un-codified and their classes recur (#446 then #450; #471 then #472); adopt a closure discipline so each proposal is codified, rejected, or expired. Pending-candidate worklist (the r4 G-1 enumeration; the closure build's inputs): the #625 grep-result-characterization fidelity candidate (third occurrence: #603, #606, #625), the #621 verifier-fix class-width candidate, the #620 count-reflex candidate, and the #624 pair, whose fixture pre-change-FAIL half now has its durable home (the tests/README authoring paragraph, shipped with gate 65) while its boundary prose-after-merges ordering half rides here per r4 G-2.
...
- **(r4 D-F3, [guardrails], L, S) Evidence-grounded-completion one-liner corollaries.** All four enumeration surfaces (pack CLAUDE.md, pack README tree blurb and scope row, project CLAUDE.md index bullet) omit the rule's un-observable-state / inventory / external-version-currency corollaries, which other project surfaces cite by name; the r3 D-3 fix shape (one clause per surface), extended to this rule. Pack-surfaces PR; the project-CLAUDE.md clause rides the authorized-touch bundle.
- **(r4 D-F5, [guardrails], L, XS) Change-tracking stub-form description vs operated form.** The pack rule says the overnight stub holds "only the protocol description plus the Status line" while the operated file also retains closure notes; either admit the single latest-run closure note in the rule (pack touch) or prune the file to one note at the next morning processing.
- **(r4 D-F4+F6, [guardrails], L, XS, protected-file) CLAUDE.md operating-mode rewords for the next authorized touch.** (F4) The attended-autonomous section's "logging stays normal" differentiator implies overnight logging is abbreviated, contradicting the operated overnight convention (per-PR QA, no abbreviation); reword so the distinguishing property is the skip-to-morning rule alone. (F6) The lease-lifecycle line and the resume command never mention the `winding-down` status gate 63 validates; one clause naming its treatment. (Bundled, #627 validate-pr F2) The worker-brief coverage-pairing checklist line still describes close-time brief-directory removal against the maintainer's 2026-07-04 delivery-time seed-removal convention the runbook now records; reword to the delivery-time form.
- **(r4 G-3, [guardrails], L-M, S) D-check prose-presence coverage.** No gate enforces section-6.1 presence for delta checks (gate 64 is numeric-gate-only; the parity registry is exclusion-only, so a stale entry for a removed Dn dies silently); either extend gate 64 to parse the D-row table and require the per-check narrative opener plus a registry-to-table cross-check, or adjudicate accepted-residual (D checks are rare and D6 landed with its prose). Maintainer scopes.
- **(r4 G-4, [guardrails], L, S) Binding-sentinel stability.** The `BINDING_SENTINEL` sentence gates 62 and 65 both depend on is enforced nowhere and duplicated across the two linters; options: a near-miss guard in both gates, hoisting the shared constants into `lint_common.py`, or a recorded accepted-residual. Maintainer picks (cross-references the GR-3 wave-3 shared-parser decision, different scope).
- **(r4 G-7, [guardrails], L-M, S) QA-report intake shape needs a durable carrier.** The three-layer intake (worker report, independent validation subagent, transcription-fidelity verifier) caught defects at each seam in its first run but lives only in the #626 records; codify as a runbook section-5 subsection (or a skill if the channel recurs). Maintainer scopes.
