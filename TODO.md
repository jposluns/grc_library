...
Policy and standard documents used three H2 numbering patterns (fully-numbered `## N.`, fully-unnumbered `## Title`, and `## Section N:` labels). Maintainer decision 2026-07-01: normalize to **fully-numbered** (`## N.` H2 + hierarchical `### N.M` H3), taking the **safe subset first and deferring the entangled docs**. The safe subset (28 docs with no inbound `§N`/"Section N" citation, no anchor link, no intra-doc section self-reference, and no inline prose clause numbering coupled to the headings) was normalized via deterministic scripted apply + re-parse in this PR. **Residual (deferred, this item):** the 3 remaining entangled docs (of the originally-deferred 38; 25 renumbered through #548, the already-canonical internal-audit standard confirmed in #594, and the exception-management, service-level-management, acceptance-into-service, encryption-policy, identity-and-access-management, data-classification, logging-and-monitoring, compliance-and-audit-management, and legal-and-regulatory-compliance renumbers in #596 through #604) whose sections ARE cited elsewhere, self-referenced, or coupled to inline clause numbers and would require a lockstep citation/clause remap (the full enumerated set, with each doc's entanglement reason, is the worklist in [`.working/fr48-deferred-worklist.md`](.working/fr48-deferred-worklist.md)), across four reference forms: (a) `§N`/`§N.M` subsection citations, markdown-link or bare-parenthetical (e.g. `policy-compliance-and-audit-management` was cited `§4.3` by `procedure-capa.md`, done in #603; `standard-logging-and-monitoring` was this class, done in #602); (b) "Section N" prose citations (e.g. `standard-mobile-application-security`, which was cited by 6 `dev-security/claude-rules/languages/*.md` pack files plus the pack `README.md` (7 pack files total), all renumbered in #548; done); (c) intra-doc "Section N" self-references (`architecture/standard-technology-radar` and `architecture/standard-integration-architecture` were this class; both done, so the open set is class (d) only); (d) inline prose clause numbering keyed to the old headings (`policy-secure-development-and-engineering`, `policy-network-communications-security`, `policy-information-security`; `policy-compliance-and-audit-management` (done in #603) and `policy-legal-and-regulatory-compliance` (done in #604) round out the original five; a pre-push verifier caught all five mid-PR, so they were reverted). Each needs its old-section-number to new-`§N` map applied to every citer and inline clause in the same PR; the numbering itself is gate-blind (gate 38 strips numbering), so this is editorial-consistency, not error-prevention. A dedicated effort (**one document per PR**, deterministic apply + citation remap + re-parse) is the shape being worked, per the maintainer's 2026-07-01 directive: one document at a time with maximal QA, no mass change until certain the renumber leaves zero broken links or references anywhere (this supersedes the earlier "one domain-batch per PR" framing, chosen for its smaller per-PR blast radius on a high-risk structural change).
