...
- **(from item 4) Extend gate 55 `RETENTION_CHECKS`.** [`tools/lint-retention-consistency.py`](tools/lint-retention-consistency.py) tracks only three curated register/procedure retention pairs; extend it to the AI-audit, supplier-audit, privacy-breach, PIA, and AI-IA rows, whose canonical values the retention reconcile set (7-year audit floor; breach 7 years; PIA and AI-IA 7-or-system-decommission-plus-5 whichever is longer): widen the gate so it locks the reconciled values. Tooling note (#563 verifier): the D5 closure detector also misses the "section-N.M closed" phrasing AND the "items N-M and X-Y closed" multi-item form (#567 passed it vacuously); add both to the closure forms when D5 is next touched (same detection-width family as GR-12). Same family, gate 50: its maintainer-exception marker regex matches only the Commonwealth `authoris` spelling while the flipped CLAUDE.md convention prose now says "maintainer-authorized", so a future history row following the house spelling would be missed; widen to `authori[sz]` with a fixture when gate 50 is next touched (the batch-A docstring flips were reverted to the -ise word-reference form meanwhile).
...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half (tools-prose flips, the TLS caveat, the gate-55 section-5 listing, the protected D1-D5/closure-form lines, the COBIT APO12 trio, L-b/L-c/L-e) shipped in the batch-A PR; the remaining bullets are the judgment tier (value compositions, genericizations) plus the maintainer-gated items. (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)
