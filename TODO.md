...
### 1.6 D5 space-separated closure form + CLAUDE.md D5/RM-10 codifications (authorized 2026-07-03) (S)

The D5 PR-time check missed #607's root-CHANGELOG phrasing "TODO section 1.1 closed" (space-separated `section N.M ... closed`, a seventh observed variant outside the six shipped forms; the rotation was present so nothing escaped). One PR, all maintainer-authorized 2026-07-03: (a) widen [`tools/check-todo-rotation-on-pr.py`](tools/check-todo-rotation-on-pr.py) with the space-separated section-closure form plus a regression fixture and the audit-spec D5 narrative update; (b) refresh the `.claude/CLAUDE.md` PR-workflow step-7 form descriptions to match the final form set (the #595 sweep L-1 staleness on forms 3 and 6); (c) add the RM-10 unpiped-guard convention sentence to the CLAUDE.md PR-workflow steps (run the pre-push guard standalone and read its exit before the chained push; two pipe-masked incidents, #569 and #583), dispositioning the last open removal-ledger entry.
...
- **(GR-8, M, S; half (a), the removal-ledger disposition pass, executed 2026-07-03 in the small-fixes PR, 14 of 15 entries dispositioned; RM-10's codification was maintainer-AUTHORIZED 2026-07-03 and ships in the D5 PR, dispositioning the last entry) Close the remaining watch-loop.** Retro proposed-improvements closure: [`.working/improvement-log.md`](.working/improvement-log.md) proposals accumulate un-codified and their classes recur (#446 then #450; #471 then #472); adopt a closure discipline so each proposal is codified, rejected, or expired.
...
- **P1 (fix errors and prevent recurrence)**: 3 items (1.4 audit-gate candidates, 1.5 reference version-currency, 1.6 the D5/CLAUDE.md codifications authorized 2026-07-03; the 2026-07-02 audit P1 cluster is fully closed; FR-48 completed in #596 through #607).
...
- **P3 (clean up and tooling)**: 7 items (3.1, 3.4, 3.6, 3.7, 3.12, plus 3.13 audit tooling extensions and 3.15 the 2026-07-02 guardrail-review machinery extensions; sections 3.8, 3.10, and 3.14 fully closed).
