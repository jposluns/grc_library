...
### 1.3 DPO role-separation sweep (maintainer-directed 2026-07-02, H, M)

The "CIO (acting DPO)" composite reflects the maintainer's own current state from source material used when creating the initial documents, and must not be reflected in the corpus. Maintainer direction (2026-07-02 overnight round, verbatim intent): go through every document looking for DPO and ensure that the DPO is separate, not a composite with another role. Sweep every corpus document for DPO mentions; wherever a composite ("CIO (acting DPO)", "acting DPO", or an equivalent) appears, separate the roles (the DPO advises and signs privacy reviews; acceptance and other authorities sit with their canonical owners); re-verify each edit against the document's own authority model. The PIA high-residual signatory answer (DPO advises, EC accepts) is the first instance and ships with this sweep. Corpus-wide sweep: single-session, never a worker task.

### 1.4 Audit-gate candidates from the 2026-06-22 review (M, S each) (was 4.5)

Decided 2026-06-23 (maintainer triage): **build them all** from the 2026-06-22 review. **S3 remains** (the only open item in this section); the original S1 retention-consistency gate shipped in #462, S2 was closed in #463 as a register consolidation (the role-consistency check already existing as gate 8 `lint-roles.py`), and **S4 (no-bare-normative-`shall`) shipped in #466** (gate 56). Each is a `lint-*.py` + 4-surface wiring + regression fixture; one gate per PR to keep diffs reviewable. **Standing state (maintainer, 2026-06-29 park reaffirmed 2026-07-02): S3 stays DEFERRED until a new process/design is ready** (gates 48/49/54/58/61 already cover control-code existence; the semantic-precision half needs a design round before any build); the P1 focus is the section-1.1 FR-48 completion.
...
### 2.13 Cross-document control and timing contradictions (audit 2026-07-02, two routed notes remaining)

Items 12-14 and 16-19 closed in the C8 PR (stricter-safe or canonical-source resolutions, each logged in its CHANGELOG entry); item 11 (the GDPR DSR clock wording) closed in the DSR-clock PR. Remaining: the routed acceptance-authority note carrying three carriers (blocked on the GDPR-Article-36-to-High-band mapping decision, a morning item), and the routed composed-retention-vs-approval-blockquote tension below (the #580 verifier's N-4).

- **(routed, #580 verifier N-4, L, S; maintainer ANSWERED 2026-07-02 at the overnight round: exempt schedule-composed rows, reword the standard so schedule-defined composed values need no per-instance approval, the Legal Counsel gate keeps its real target, ad-hoc extensions beyond the schedule) Composed retention values vs the 7-year approval blockquote.** [`governance/standard-records-retention-and-destruction.md`](governance/standard-records-retention-and-destruction.md) Section 5 carries "Retention periods longer than 7 years require written approval from Legal Counsel", in latent tension with the register's composed whichever-is-longer rows (which can exceed 7 years for long-lived systems; the tension dates from the #574 compositions). Decide whether the composed register rows count as the written approval's standing instrument or the blockquote needs a composed-value carve-out.
- **(routed note, #561 verifier, L, S; maintainer ANSWERED 2026-07-02 at the overnight round: DPO advises, EC accepts; ships with the section-1.3 DPO role-separation sweep, its first instance) PIA high-residual acceptance signatory.** [`privacy/procedure-privacy-impact-and-cross-border-transfer.md`](privacy/procedure-privacy-impact-and-cross-border-transfer.md):220 has the CIO (acting DPO) signing the residual-risk acceptance for high-residual-risk processing, which sits uneasily beside the canonical High-acceptance authority (Executive Committee or Board Risk Committee). Not a retired-authority carrier; the open question is whether privacy "high-residual-risk processing" (the GDPR Article 36 prior-consultation threshold) maps 1:1 to the risk matrix's High band before the signatory is judged misaligned. Decide the mapping, then align or annotate. Same-class siblings from the #561 `/validate-pr` (word-form "accepted", invisible to the "approv" scan): [`governance/procedure-grc-programme-management-and-annual-review.md`](governance/procedure-grc-programme-management-and-annual-review.md):262 ("formally accepted in writing by the CIO or above", satisfiable by a CIO-alone signature for a High or Critical residual) and :266 ("Residual risks accepted at ERC level", asserting an ERC acceptance level the role register does not grant). Align all three to the canonical chain in one pass once the mapping question is decided.
...
Cheap, mostly-mechanical corrections re-validated at source; bundle into one or a few small PRs. The mechanical half shipped in the batch-A PR and the judgment tier (the rollout-phasing note, the two retention resolutions, the two genericizations) in the batch-B PR; the original batch's items have all shipped (batch A, batch B, the coverage-gaps re-grade, L-j, and L-k, each in its own PR); the remaining bullets are the routed COBIT fit-pass recode PR, the third coverage-gaps triage batch, and the routed #585 pre-push verifier finding below (the coverage-gaps second batch and the lifecycle-vocabulary fork each shipped in their own PRs). (One audit low-sev candidate, the glossary CalVer/PR, was REFUTED: the glossary carries the standard 13-field block like its peers, and no document carries a Library Version/PR in its metadata, so it is not a finding.)

- **(routed, the 2026-07-02 maintainer-authorized ad-hoc COBIT fit pass, M, S; pre-authorized: "any recode ships as its own small PR") COBIT semantic-fit recodes, seven verdicts, one PR.** The fit pass judged the four routed APO12 placements plus three carriers surfaced while building gate 61, every verdict RECODE with held-text grounding: cross-framework matrix row 64 APO12.01 to EDM03 (framework-and-governance row; EDM03.02 the practice-level alternative); row 66 APO12.02 to EDM03.01 AND its ISO 31000 cell "Clause 5.4; Clause 5.6" to Clause 6.3.4 Defining risk criteria (the appetite home); row 65 APO12.03 to APO12.02 (pair form APO12.01; APO12.02 available once row 64 recodes); the exception policy's three APO12.03 carriers (the section-2.1 sentence, the titled framework bullet, the matrix row) together to APO12.02 Analyze risk; and the four BAI05.02 supplier carriers (three in the supplier-audit procedure, one in the compliance-management policy) to APO10.05 Monitor vendor performance and compliance. Items 5 and 6 of the pass (the nonexistent MEA01.06 and DSS01.06) already shipped with gate 61. Two additions from the #587 pre-push verifier: the ISO 31000 clause/title swap "§6.7 (Monitoring and review)" on three carriers that move together (the GRC-programme procedure's alignment sentence and table row, and the document-index register's row 77 cell; per the held extract 6.6 is Monitoring and review and 6.7 is Recording and reporting, and the row content is 6.6 semantics, so the NUMBER is the error; gate 61 is title-blind to it by design); and the two adjacent mis-titled MEA01 cells in the compliance-management policy (MEA01.02 titled "Conduct assessments", canonical "Set performance and conformance targets"; MEA01.05 titled "Review and report", canonical "Ensure the implementation of corrective actions"). Apply with the three-exception-policy-carriers-move-together and four-supplier-carriers-move-together constraints; re-verify each held-text quote at apply time.
- **(routed, #586 sweep M-1 plus M-2 plus L-2 plus L-3, M, S-M) Coverage-gaps triage, third batch: rows outside the batch-2 enumeration that its own precedents now contradict.** Each claim re-verified at file at routing time: Mexico NEEC / OEA graded None while the supply-chain alignment matrix carries a full NEEC programme-profile row, a NEEC domain column, evidence-checklist rows, and roadmap and authority rows (the same columns-and-rows class that re-graded EU AEO; Brazil OEA has a profile row plus mutual-recognition rows and no register row at all, part of the triage); China generative AI rules graded None while the China privacy annex carries a dedicated described-obligations bullet for the Interim Measures and the jurisdiction index carries Generative-AI cells (stronger than the Colorado-only basis that re-graded US state-by-state AI; Korea AI framework None stays, the Act is uncited); the five section-2.2 rows Australia ATT, Singapore STP/STP-Plus, Japan AEO, Korea AEO, and NZ SES graded None on exactly the logistics-README candidates-list class that re-graded My Health Records (Japan and Korea AEO also sit in the matrix's mutual-recognition cell; China AEO None stays, unnamed anywhere); and the section-5 post-quantum migration-playbook row graded None while the shipped PQC roadmap carries migration-scope and phased-migration sections (the Manufacturing adjacent-dedicated pattern; its sibling readiness row already grades that roadmap Substantive).
- **(routed, #585 pre-push verifier SF-1, L, XS) Dangling lifecycle pointer in the quality-cadence procedure.** The Supersede row of [`governance/procedure-library-quality-and-review-cadence.md`](governance/procedure-library-quality-and-review-cadence.md) says the predecessor's status changes "per the lifecycle in the document architecture framework", but [`governance/framework-document-architecture-and-interrelationship.md`](governance/framework-document-architecture-and-interrelationship.md) carries no lifecycle stage enumeration; the stage list lives in the charter's "Document lifecycle" section. Re-point the pointer at the charter, or move the stage list into the framework if that is the better home (a small naming call). Pre-existing; surfaced by the #585 pre-push verifier while the lifecycle vocabulary was being aligned.
...
- **(routed, #581 verifier N-4, L, XS) D5 short-form closure phrasings.** "Closes the section-N.M ... bullet (rotated to DONE)" matches none of D5's six forms (form 3's noun set lacks "bullet"; form 4 wants the closure word AFTER the section token; form 6 wants the full "rotated to the DONE ledger" phrase), so a forgotten rotation behind that phrasing would pass vacuously. Candidate widenings: add "bullet" to form 3's noun set and a short "rotated to DONE" variant of form 6, each subject to the gate's documented full-history FP census before shipping.
...
- **(routed, #587 pre-push verifier S-4, L, XS) verify-reference-modules parity coverage for the new reference module.** [`tools/verify-reference-modules.py`](tools/verify-reference-modules.py) backstops module-vs-scratch parity for the CCM/AICM/CSF reference modules but not for [`tools/cobit_iso31000_reference.py`](tools/cobit_iso31000_reference.py); extend it (and widen its own gates-48/49/54 docstring line while there). The HA register row's refresh-on-release follow-up is prose-only until this lands.
