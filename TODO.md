...
- **Working-state relocation R2 (M, S) — deferred, needs a maintainer decision**: citation-verification cluster (6 files) — `register-citation-verifications.md` + `register-citation-verification-bundle.md` + four `worklist-citation-verification-batch-*.md` → `.working/citation-verifications/` (project-specific in-flight verification campaign). **Convention-conflict surfaced 2026-06-24 (overnight R2 assessment):** the 6 files are cross-cited by *active-corpus* governance docs (`register-canonical-citations.md`, `register-coverage-gaps.md`, `register-ai-security-tooling-landscape.md`, `specification-citation-verification.md`, `template-citation-verification-worklist.md`) and by two linters that hardcode their paths (`tools/lint-citation-verification-freshness.py`, `tools/lint-citations.py`, plus `tests/test_linters.py`). Moving them to `.working/` would force active docs to either link into `.working/` (violating the "don't link `.working/` from adopter-facing files" convention) or drop those citations, and would change the two linters' load-path constants. Options: (a) keep in `governance/` and close R2 won't-move (parallels the R1 won't-move decision; the citation campaign arguably IS corpus governance content, not transient working state); (b) move and re-point everything (heavy, convention-conflicting); (c) move only the four transient `worklist-*` batch files and keep the two registers in `governance/`. Recommend (a) or (c); needs maintainer judgment. (R1, the sweep-preflight-exemptions.json relocation, was closed won't-move 2026-06-24; R3 shipped in #304.)
