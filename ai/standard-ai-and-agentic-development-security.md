...
Absolute prohibitions. No exception without approval through the §4.2.2 risk-tier pathway of the [Exception and Risk Acceptance Management Policy](../governance/policy-exception-and-risk-acceptance-management.md) (with CISO co-approval for security-related exceptions), except where a pattern restates a Section 4 architecture principle that itself declares no exception: human approval for irreversible actions (the Section 4 P-05 principle) is absolute, so P-13 is not subject to this exception pathway.
...
| P-08 | MCP servers (organization-hosted or supplier-hosted) used in production without registration in the MCP server register and the security review and approval their tier requires | Tool poisoning |
...
**Framework anchors.** These controls align with NIST SP 800-218A PW.4.4 (verify the integrity, provenance, and security of acquired AI components, and scan them for vulnerabilities and malicious content before use; DEVSEC-AI-04) and RV.1.1 (weekly CVE scanning of pinned AI dependencies) and with ETSI EN 304 223 V2.1.1 Provision 5.2.2-3 (dedicated development and model-tuning environments backed by technical controls for separation and least privilege; Principle 6).
...
**MCP-SEC-01:** Every MCP server used in production, whether organization-hosted or supplier-hosted, must be recorded in the [MCP server register](register-mcp-server.md) and pass the security review and approval its tier requires (the register's approval categories) before any integration. Unregistered (shadow) MCP servers are prohibited in production.
...
**Framework anchors.** These controls align with NIST SP 800-218A: PW.4.4 (acquired-model provenance, checksum verification and adoption scanning; SUPPLY-SEC-04, SUPPLY-SEC-05 and SUPPLY-SEC-07), RV.1.1 and RV.2.2 (known-CVE scanning of AI dependencies on every build and formal risk acceptance for an unresolved High; SUPPLY-SEC-02), PS.3.2 (provenance data via a software bill of materials and Supply-chain Levels for Software Artifacts; SUPPLY-SEC-03), and PW.3.2 (track data provenance; SUPPLY-SEC-06); and with ETSI EN 304 223 V2.1.1 Provision 5.2.3-1 (secure software supply-chain processes; Principle 7) and Provision 5.2.4-1.2 (cryptographic hashes for released model components; Principle 8). Where the organization holds model weights it has not publicly released, the EU GPAI Code of Practice (Safety and Security Chapter, July 2025), the Article 56 co-regulatory instrument for demonstrating Article 55 compliance, provides an authoritative control catalogue in its Commitment 6 and Appendix 4: encryption of unreleased model parameters at rest and in transit (at least 256-bit) with keys stored on a Trusted Platform Module (TPM) and decryption only to non-persistent memory (Appendix 4.2; anchors SUPPLY-SEC-05 and P-07), hardened and rate-limited interface access (Appendix 4.3), and insider-threat controls including model sandboxing and training-data tamper checks (Appendix 4.4; anchors SUPPLY-SEC-06). Adherence to the Code is voluntary for providers of general-purpose AI models with systemic risk, but it is the primary co-regulatory route to demonstrate Article 55 compliance (a non-adhering provider must show alternative adequate means); the organization applies these as the expectation it holds a systemic-risk provider to, and as direct obligations if its own model reaches that scale.
...
The adversarial test suite must cover the categories defined in the AI Adversarial Test Reference on every system in scope; the required categories and their test cases are specified there.
...
| Excessive agency | LLM03:2026 | N/A | IAM-18, IAM-05 | MANAGE 1.3 | Least model privilege | Limit model behavior (focused functionality) | ASI02 |
| Overreliance | LLM07:2026 Misinformation | N/A | GRC-15, GRC-13 | MANAGE 4.1 | AI transparency | N/A | ASI09 |
