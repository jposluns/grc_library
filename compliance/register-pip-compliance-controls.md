...
| PIP-IT-6 | **Incident Reporting and Response** — Maintain documented cyber incident response procedures. Report security incidents that affect supply chain integrity or border data to CBSA in accordance with the PIP partnership agreement. | [`resilience/procedure-incident-response.md`](../resilience/procedure-incident-response.md), [`security/sop-incident-escalation-matrix.md`](../security/sop-incident-escalation-matrix.md) | Incident response plan; CBSA notification runbook; incident log | Implemented | |
