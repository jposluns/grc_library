...
- [`governance/session-lifecycle.md`](governance/session-lifecycle.md): the session-lifecycle and operating-modes discipline for multi-session AI-assisted work. Six disciplines across the RESUME / WORK / CLOSE phases: a durable handoff record (reconciled not appended, pruned keep-current-plus-one-prior); explicit operator-set operating modes (fully attended, attended-autonomous with green-CI merge authority as the default, unattended with no-idle-stop) that are named never inferred; graceful degradation for blocked operator decisions (a short-timer surface with either a reversible stricter-safe default or defer-and-skip, an absolute reversibility gate so a timeout never auto-executes a destructive action); evidence-gated wind-down (continue is the default; the only valid close trigger is a named externally-observable signal, never felt degradation or work shape); the closing green-merge and its loop-break compensating control (the closing change may skip its own trailing QA only where a stronger next-resume whole-scope validation, cross-checked against asserted-clean claims, pairs with it); and an advisory concurrency lease. Distilled from the session apparatus of the project this pack was developed in; project-agnostic, so the consuming project wires the concrete files (its own `CLAUDE.md` or equivalent carries the project-specific operationalization).
