...
- Reference base: the in-repo validator modules (`tools/ccm_aicm_reference.py`,
  `tools/nist_csf_reference.py`, `tools/cobit_iso31000_reference.py`) and the
  `grc_library_ref` repository's source-text extracts.
...
- Judging fit from the code's familiarity or a remembered meaning instead of reading the source control TITLE. The pre-filter's founding design lesson is that the title is the only evidence; a remembered meaning is the failure mode that produced the valid-but-wrong codes in the first place.
