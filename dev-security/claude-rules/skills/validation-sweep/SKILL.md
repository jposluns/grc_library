...
**Rule 5.6, subagent dispatch must be declared in the register entry.** Every sweep entry in [`governance/register-sweep-history.md`](../../../../governance/register-sweep-history.md) names which subagents were dispatched (e.g. `Subagents dispatched: A, B, C` for a full sweep; `Subagents dispatched: A only; B and C scope-skipped per maintainer authorisation` for a thin sweep). If a subagent was skipped without explicit maintainer authorisation in the same sweep cycle, the next sweep entry's "Sweep value" section records the discipline failure as a corrective action. The mechanism is the auditable trail: a subagent's silent absence in a sweep register cannot be reconstructed later, so the dispatch declaration is the only point at which the discipline can be enforced.
