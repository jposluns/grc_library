...
#### 4.5.2 Source and build integrity (SLSA v1.2 alignment)

The organization targets the following SLSA v1.2 Build track levels by system type:

| System Type | Target Build Level | Rationale |
|---|---|---|
| Externally accessible applications | Build L2 | Signed provenance generated by a hosted build platform; build-as-code as an organizational requirement |
| Internal tooling | Build L1 | Provenance showing how the package was built |
| Critical infrastructure code | Build L3 | Hardened build platform; hermetic, reproducible builds as an organizational requirement beyond Build L3 |

Build L2 minimum requirements (SLSA v1.2 Build L2, plus version-controlled build scripts as an organizational requirement):
- Build process defined in version-controlled build scripts (organizational requirement)
- Build runs on a hosted build platform that generates signed provenance (build inputs, outputs, timestamps)
...
|---|---|
| **NIST SSDF (SP 800-218)** | PW.4.1, PW.4.4, Acquire and verify third-party components; PS.2, Provide integrity verification information; PS.3.2, SBOM provenance; RV.1.1, Gather information on vulnerabilities in third-party components; RV.2.1, Analyze each vulnerability to plan its risk response; RV.2.2, Plan and implement risk responses for vulnerabilities |
| **NIST SP 800-53 Rev. 5** | SR-3 Supply Chain Controls; SR-4 Provenance; SR-11 Component Authenticity |
...
| **SLSA v1.2 Framework** | Build track levels L0 to L3 and Source track levels L1 to L4 (SLSA v1.2); build-platform, provenance, and source-control requirements |
