...
## Step 3: apply the organization-neutrality rules

Apply the organization-neutrality rules in Appendix A of [`specification-ingestion.md`](../specification-ingestion.md): replace any organization-specific product, vendor, system, or programme name with a generic category description. For example, a specific identity-provider product becomes "an enterprise identity provider", a specific SIEM product becomes "a SIEM", and a specific collaboration platform becomes "a collaboration and file storage platform".

Quebec is a legitimate jurisdictional reference, not organization-specific detail. It stays.
...
The script runs the full audit programme defined in [`governance/specification-audit-programme.md`](../governance/specification-audit-programme.md) §6. All gates must exit clean before commit.
...
- Leaving a specific vendor-product name in place instead of its generic category. Manual review is required.
...
The end state is a self-contained, role-based, framework-aligned, CC BY-SA 4.0-licensed procedure that any organization can fork and adapt. The path to that state is short: type selection, domain placement, canonical filename, generalization, role substitution, metadata block, canonical body structure, index and README updates, audit pass, commit.
