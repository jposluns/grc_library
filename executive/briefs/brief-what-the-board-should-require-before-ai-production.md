...
Putting an AI system into production is a management decision. The board's role is oversight: to satisfy itself, before the system goes live, that the organization is ready to operate the system and not only that the system works. The failure pattern the corpus is designed to address is a system going live while the governance around it (an owner, a risk classification, tested controls, monitoring, an incident path, a way to retire it) is assembled after the fact or not at all, so that the gap becomes visible only once something goes wrong. By then the board's options are narrow.

The board's oversight sits before production and is different in kind from management's approval. The board does not build the system and does not approve it into production; it sets the risk appetite the organization operates within, asks the questions that expose an unready deployment, and states what it expects to receive. This brief points to where the corpus defines each of those, so the board can call for them rather than reconstruct them after an incident. (The division of authority this brief draws between the board and management is synthesized across several corpus sources; see the limitations.)

## What the corpus establishes

The corpus defines what an organization needs in place before an AI system is put into production, and it separates the board's role from management's.
...
- **Human oversight is over the system's outputs.** The same standard requires that human reviewers receive enough context to challenge, override, or reject the system's outputs (its section 4.8). This is an operating control on the AI itself, and it is not a substitute for the board's governance role.
...
- **The board sets appetite and asks the questions.** The [board oversight guide](../../ai/guide-ai-board-oversight.md) establishes that the board sets the organization's AI risk appetite and approves material AI policy direction while management operates within it (its section 4); it names structured interrogation as the board's primary oversight instrument and provides a director question set (its section 5); and it sets out governance red flags (its section 6) and what the board should expect to receive (its section 7).
- **High residual risk escalates above the management gate.** The [exception and risk acceptance policy](../../governance/policy-exception-and-risk-acceptance-management.md) routes acceptance of high or critical residual risk above a routine management approval, to the higher authority it designates (its section 4.2). Where a system would enter production carrying such risk, that acceptance follows this route rather than passing as an ordinary production sign-off.
...
For the board and executive leadership, the corpus position sets a division of labour before an AI system goes live. (This division is a composite reading of the sources named above.)

- **The board does not approve the system into production; it sets the appetite and policy that management's decision operates within.** The board's contribution beforehand is to have an approved AI risk appetite and material policy direction in place, so that management's Step 9 decision is exercised within limits the board has set. A board that has set neither an appetite nor its questions has little left to exercise before production.
- **The board should expect evidence, not assurances.** The controls the corpus calls for are visible to the board only if they can be shown. The board's oversight before production is a dependency on management producing the evidence that each control exists and operates, which the next section expresses as evidence classes.
- **High residual risk does not pass as a routine sign-off.** Where a system would enter production carrying high or critical residual risk, the corpus escalates that risk-acceptance decision above the routine management approval, to the authority the exception and risk acceptance policy designates (its section 4.2). Treating it as an ordinary production approval bypasses a control the corpus deliberately places above the management gate.
- **Some answers are weak answers.** The board oversight guide names governance red flags (its section 6); one bears directly on a production decision, namely accountability attributed to a system or a supplier rather than to a person. Two further weak answers are worth naming against the security standard rather than the red-flag list. "We will monitor it once it is live" treats monitoring as a replacement for the pre-production controls, when the security standard places monitoring among the controls to have in place before production rather than in place of testing and oversight. And a passing internal test is evidence that a control was exercised once, not evidence that the control operates in production.

## Evidence to request

Each item below is an evidence class the board can call for from management, and each is evidence for a named control or outcome. The board oversight guide's director question set (its section 5) frames the questions to ask; this list frames what a strong answer produces.

- **An entry in the AI system register** for the system, as evidence of inventory and ownership. The [register template](../../ai/template-ai-system-register.md) shows the structure; the requirement to record each AI system in the register before production lives in the [security standard](../../ai/standard-ai-security-and-risk.md).
...
- **The incident-response linkage**, as evidence that the AI incident classes in the [AI incident response plan](../../ai/plan-ai-incident-response.md) are covered and its evidence-preservation requirements are met.
