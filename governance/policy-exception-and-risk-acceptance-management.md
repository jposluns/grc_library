...
The `renewal_count_limit` field defaults to 2 (the value at which the 3rd-renewal prohibition takes effect, and the maximum number of 180-day renewals that fits inside the 540-day `max_duration` ceiling in §4.3.4). A lower limit may be recorded on individual entries by the approving authority where the risk profile warrants a tighter cap. A higher limit must not be recorded; the absolute prohibition on a 3rd renewal is policy-wide and cannot be raised through the entry field.
