...
4.3.8 **Anchoring the 90-day baseline.** The 90-day initial term itself is a library convention rather than a number quoted from any single normative source. Neither NIST SP 800-53 Rev. 5 CA-6 (ongoing authorization, whose monitoring frequency is organization-defined) nor ISO/IEC 27001:2022 Clause 9.2 (internal audit "at planned intervals") prescribes a fixed interval; 90 days is chosen to align with the quarterly refresh cycle these controls' typical implementations use, so an exception's renewal point coincides with a scheduled control-evidence refresh. Adopters whose monitoring cadence is monthly or semi-annual may tune the initial term downward or upward respectively (the cumulative `max_duration` ceiling in §4.3.4 then tunes proportionally as four multiples of the chosen base); the renewal-ceiling pathway in §4.3.5 remains in force regardless of the chosen base term.
