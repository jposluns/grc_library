...
| US federal (executive branch) | Substantive | In library | n/a | Dedicated jurisdiction annex [`ai/jurisdictions/annex-ai-us-federal.md`](../ai/jurisdictions/annex-ai-us-federal.md) (OMB M-25-21 agency AI governance and high-impact minimum practices, M-25-22 AI acquisition contract terms, M-26-04 Unbiased AI Principles in LLM contracts; binding on agencies, reaching vendors through contracts), with America's AI Action Plan (July 2025) as strategy context and Executive Order 14365 on state AI laws |
...
| AI model routing and selection governance | Partial | Out of scope | n/a | Model-selection and tiered-routing are addressed by shipped standards ([`standard-ai-inference-cost-governance.md`](../ai/standard-ai-inference-cost-governance.md) records model-selection rationale and cheaper-model/tiered-routing fallback; [`standard-ai-access-and-agent-permissions.md`](../ai/standard-ai-access-and-agent-permissions.md) records routing decisions with the chosen model and rationale; model selection is an ADR); dedicated routing/selection governance (consolidated selection criteria, fallback-quality governance, routing auditability) remains the gap. Assessed 2026-08-03 (PR #1974) and dedicated build declined as a durable, revisitable decision |
| AI batch inference governance | Partial | Out of scope | n/a | Provider batch-API usage is addressed operationally in [`standard-ai-inference-cost-governance.md`](../ai/standard-ai-inference-cost-governance.md) (batch APIs for non-interactive workloads); dedicated batch-inference governance (scheduling, cost/latency SLOs, failure handling) remains the gap. Assessed 2026-08-03 (PR #1974) and dedicated build declined (durable, revisitable) |
| Self-hosted model serving architecture | Partial | Out of scope | n/a | Self-hosted models are addressed in shipped documents ([`procedure-foundation-model-lifecycle.md`](../ai/procedure-foundation-model-lifecycle.md) secure-hosting plan; [`standard-ai-inference-cost-governance.md`](../ai/standard-ai-inference-cost-governance.md) self-hosted hosting cost; [`standard-ai-and-agentic-development-security.md`](../ai/standard-ai-and-agentic-development-security.md) self-hosted security review); a dedicated serving architecture (serving, capacity, and isolation model) remains the gap. Assessed 2026-08-03 (PR #1974) and dedicated build declined (durable, revisitable) |
| AI use-case portfolio schema | Partial | Out of scope | n/a | Idea-stage governance exists ([`framework-ai-value-and-decision-governance.md`](../ai/framework-ai-value-and-decision-governance.md)) and the AI System Register ([`template-ai-system-register.md`](../ai/template-ai-system-register.md)) already admits proposed use cases (Business Purpose, Lifecycle Status of Proposed); a fuller opportunity-portfolio and value-hypothesis schema remains the gap. Assessed 2026-08-03 (PR #1974) and dedicated build declined (durable, revisitable) |
...
| Data mesh / data product governance | Partial | Deferred | Backlog candidate: data mesh governance guide (not currently scheduled in TODO) | Re-graded None to Partial 2026-09-25: [`architecture/standard-data-architecture.md`](../architecture/standard-data-architecture.md) sets domain ownership, data-product owners, contract-based internal sharing, per-product quality SLOs where appropriate, and a federated data council for cross-domain concerns, in outline; a data-mesh operating model (self-serve data platform, computational federated governance, global interoperability policies) is not covered |
| Semantic layer and business-definition governance | None | Planned | TODO backlog: semantic layer and business-definition governance standard | Row added 2026-09-25 (external review): the lifecycle governance of shared business definitions, glossary terms, ontologies and metric definitions that centralized context and semantic layers (including AI retrieval context) depend on: ownership, change control, versioning and cross-domain reconciliation. Adjacent controls exist but do not cover this lifecycle: data stewards maintain data dictionaries ([`privacy/policy-privacy-and-data-governance.md`](../privacy/policy-privacy-and-data-governance.md)), reference data are centrally curated, master entities have designated master systems, and hierarchies are versioned ([`architecture/standard-data-architecture.md`](../architecture/standard-data-architecture.md)), and metric registers require an owner and a measurement or interpretation rule per metric |
