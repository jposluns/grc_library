...
| AI incident records | 5 years, or 5 years after the associated system's decommission, whichever is longer | The 5-year figure and its decommission anchor are BOTH the organization's own floor, not the Act's: the EU AI Act prescribes no retention period for incident records, and no provision of it anchors any retention obligation to decommission, end of life, or withdrawal (the same finding that corrected the two rows above). The Act's Article 73 creates the serious-incident REPORTING duty, with reporting deadlines rather than a keeping period, and the log-retention periods it prescribes in this area are Article 19(1) (provider-held logs) and Article 26(6) (deployer-held logs), each requiring automatically generated logs under the relevant operator's control to be kept for a period appropriate to the intended purpose and in any event AT LEAST SIX MONTHS; the figure here exceeds that floor. Composed with the AI-Systems domain minimum in [`governance/standard-records-retention-and-destruction.md`](standard-records-retention-and-destruction.md) Section 5 (5 years post-decommission), the adjacent composed rows' proven shape, so an incident record on a long-lived system is not destroyed while the system still runs |
...
| Personnel security screening (trade) | 7 years after employment, contract, or engagement ends, or 5 years from the screening outcome, whichever is longer | BASC v6 Chapter 6; the base HR 7-year-after-separation floor governs where longer, per the records-retention standard sector-overlay rule |
