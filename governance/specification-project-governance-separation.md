...
The directional-dependency gate that enforces §4 is built as **gate 53** ([`tools/lint-directional-dependency.py`](../tools/lint-directional-dependency.py)): it flags any deliverable-corpus document that contains a markdown link whose resolved target is inside `.project-governance/`, the mechanical enforcement of the one-way dependency rule. The deliverable-corpus scan set is derived from `AUDITED_DOMAIN_DIRS` minus `.project-governance` plus the root deliverable specifications; the non-deliverable surfaces §4 permits to link into the directory (the pack under `dev-security/claude-rules/`, the repository backlog, the root `CHANGELOG.md`, the generated indexes, the `.working/` and `.claude/` trees) are out of scope, and so is `.project-governance/` itself (links within it are allowed). Gate 53 was widened in PR #1221 to guard a second deletable target: a deliverable-corpus document must not hard-link into the `.working/` maintainer-working-state tree either, because an adopter may delete `.working/` and such a link would then dangle. `.working/` remains a permitted *source* of links into `.project-governance/` (the one-way rule and this widening both constrain only where a deliverable-corpus document links *to*, not what links into these trees). Before the gate existed the rule rested on the migration discipline (§8.2) and the broken-link gate; the gate makes it a standing mechanical check, the §4 counterpart to the §7.4 scan-scope parity gate (gate 52).
...
- **Explicit allow-list** (the enumerated shape): the linter selects scan roots from a list of domain directories plus repository-root meta files. Historically each such linter hardcoded the domain run inline (in a `DEFAULT_SCAN_PATHS` tuple, a `DOMAINS` list, or a function-local list), so a new top-level directory was picked up only when `.project-governance` was added to every one of those copies by hand, and the directory was silently skipped wherever a copy was missed.

The audited-domain run now has a **single source of truth**: `AUDITED_DOMAIN_DIRS` in [`tools/lint_common.py`](../tools/lint_common.py) (the eleven domain directories plus `.project-governance`). Every explicit-allow-list content linter that implements a §6.3-named obligation (metadata-block completeness, per-document version and date currency, language, citation accuracy, link coverage) splats that constant rather than hardcoding the run, so a future top-level audited directory propagates to every such linter from one place. The same applies to any further gate the maintainer classifies as part of the "full corpus audit sweep" beyond §6.3's enumerated list (the per-document review-cadence gate is one such gate, included by maintainer decision so the directory's stated review cadences are scheduled, not merely asserted). The **directory-scan-scope parity gate** (gate 52, [`tools/lint-scan-scope-parity.py`](../tools/lint-scan-scope-parity.py)) is the mechanical backstop for this subsection: it forbids any content linter from hardcoding the domain run (it matches the literal directory names, so it is robust to all three historical declaration styles), with a documented `EXEMPT` set for the files that legitimately scan a *different* domain set (the section-placement gate and the taxonomy generator, which scan the corpus-document set that excludes `.project-governance` because those documents are outside the corpus document model, and the collection-candidate helper). The Phase-1 migration originally enumerated the explicit-allow-list set incompletely (the date-staleness and review-cadence gates were missed and silently skipped the directory until a validation sweep caught them); the single-source-of-truth refactor plus gate 52 makes that gap class structurally impossible rather than relying on each addition being remembered.
