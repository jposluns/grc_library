...
Two worker primitives serve the default: **in-session subagent fan-out** (workers launched inside the coordinating session and operating within its authorization boundary, the lowest-setup form, available wherever the harness offers a subagent primitive) and **separate-session delegated workers** (workers operating in isolated execution contexts, with least-privilege access limited to a designated handoff surface and no access to the protected repository). A project that runs multi-session or multi-worker work should keep a project-local runbook documenting its handoff mechanism, its durable work-ownership record (consulted at launch, never through a poll loop), and its partitionability checklist; the pack rule states the discipline, the runbook states the project's how-to.
...
- **Substantive tier** (a corpus-document body change, a new or edited gate or linter, a multi-surface change, a control-code / citation / normative-value change): a MULTI-FAMILY panel of skeptical verifier subagents (two or more independent model families, for example one Claude-family and one GPT/Codex-family, plus a third such as a Gemini-family where a project has one), pre-push, scoped to the diff, each briefed to refute. This panel catches defects before CI and merge (cheaper than a post-merge catch plus a hot-fix) and is the mechanism that sustains quality across a long session, which is why work size or session length is never itself a reason to wind down (see the consuming project's wind-down framework). The families surface systematically different failure classes, so a single-family verifier shares the orchestrator's own blind spots; running the identical refute-brief to EACH family and reconciling the verdicts is the DEFAULT for every substantive validation and verification pass, not reserved for consequential changes (per the multi-family standard in [`high-assurance-verification.md`](high-assurance-verification.md)'s stage 3), and each additional independent family widens the blind-spot coverage a same-family pass shares. A consuming project may adopt this multi-family panel as the PERMANENT standard for EVERY QA pass, running to as many independent families as have tokens (a project with three available families runs all three). The GRADUATED FLOOR applies only on token or tooling unavailability on a family (a limited or exhausted account, or a family whose worker cannot deliver its result): the panel drops to the families that can run, the gap is noted, and the missing family re-runs when it returns; a reduced-family pass is the floor only then, never a discretionary downgrade of a substantive QA.
- **Sensitive tier** (gate-blind correctness AND delicate scale AND high escaped-error cost, all three): the full [`high-assurance-verification.md`](high-assurance-verification.md) harness (independent adversarial verifiers from two or more families, deterministic apply). Unchanged; the substantive tier is its lighter sibling, not a replacement.

When genuinely in doubt between tiers, run the verifier: a false escalation costs a little extra verification, a false de-escalation ships a defect.
...
3. **Loop cap: six iterations, extended to nine when unattended and still converging.** If a finding is not resolved after the SIXTH verify-fix iteration, STOP: do not force the change through. Surface the unresolved finding to the responsible authority for a decision, recording it where the project keeps its decision queue. In an UNATTENDED mode the ceiling extends to NINE iterations, but only while the loop is CONVERGING, defined unambiguously: each round resolves the prior round's findings, and no single finding survives its own fix attempt unchanged. The early stop is a STUCK finding, one that re-surfaces unchanged after a fix (the fix did not work); that ends the loop and defers regardless of the count. NEW distinct findings each round are the loop working (it is finding and fixing real defects) and are NOT a reason to stop, even when the per-round finding count does not fall. The cap counts distinct verify-fix rounds and is a ceiling on churn, never a target.
...
to be more accommodating. Where a project runs a multi-family panel, expect the families to differ
