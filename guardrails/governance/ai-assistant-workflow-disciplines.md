...
3. **Loop cap: six iterations, extended to nine when unattended and still converging.** If a finding is not resolved after the SIXTH verify-fix iteration, STOP: do not force the change through. Surface the unresolved finding to the responsible authority for a decision, recording it where the project keeps its decision queue. In an UNATTENDED mode the ceiling extends to NINE iterations, but only while each round makes PROGRESS (distinct findings resolving, or the finding set demonstrably converging round over round); a round that makes no progress (the same finding survives unchanged, or the finding count stops falling) ends the loop early and defers, regardless of the count. The cap counts distinct verify-fix rounds, and a fresh, converging finding each round is the loop working rather than a stuck finding, so the number is a ceiling on churn, never a target.
