...
- Per-run record path: a dated per-run record in the consuming project's working state, with a row appended to the review history beside it.
- Failure-mode ledgers the gap lens reads: the improvement-log register and the worker-hallucination-metrics ledger, both in the consuming project's working state.
...
- Inventory surfaces: governance rules under `guardrails/governance/` with their `.claude/rules/governance/` mirror and four enumeration surfaces (the pack README tree, the pack `CLAUDE.md`, the project `.claude/CLAUDE.md`, and the `rule-provenance.md` register); skills under `guardrails/skills/` with the PAIRS registry; gates declared across the four parity surfaces (the CI workflow, the local runner, the pre-commit config, and the audit-programme specification's gate-inventory section).
