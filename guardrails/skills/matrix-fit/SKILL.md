...
- Recording a fit verdict, or a held-status claim for a cited framework, from a note or a prior run's record instead of an executed `ref-holds` + index read with the held-source title `path:line` cited THIS run (the 2026-08-31 stale-note incident this cadence's held-source-citation requirement exists to prevent).
...
- Any worklisted or in-scope row not judged (deferred, or routed to the pending decision, e.g. a representative-vs-strict-reproduce boundary) was NAMED with its blocker; none was silently dropped.
