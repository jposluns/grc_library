...
1. Identify the document type. The current allowed types are: Charter, Framework, Policy, Principle, Standard, Procedure, SOP, Plan, Roadmap, Guideline, Guide, Register, Matrix, Specification, Template, Annex, Checklist, Worklist. Apply the type selection guidance in the Master Project Specification §4.4 to distinguish Procedure from SOP, Plan from Roadmap, Guideline from Guide, and Template from Worklist.
2. Generate the canonical filename using the type prefix (`charter-`, `framework-`, `policy-`, `principle-`, `standard-`, `procedure-`, `sop-`, `plan-`, `roadmap-`, `guideline-`, `guide-`, `register-`, `matrix-`, `specification-`, `template-`, `annex-`, `checklist-`, `worklist-`).
...
16. After updating any artefact metadata or adding a new artefact, regenerate the machine-readable taxonomy (`python3 tools/build-taxonomy.py`) and the adopter portal and maturity scorecard (`python3 tools/build-portal.py`); an ingestion that touches `executive/` narrative pages also regenerates the narrative registry (`python3 tools/build-narrative-registry.py`).
