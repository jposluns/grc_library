...
The enterprise identity provider is the authoritative enforcement point for Conditional Access policies. Application and infrastructure teams must not implement workarounds or exceptions to Conditional Access without written CISO approval.

### 6.2 Conditional access policy governance

Conditional Access policies are defined and owned by the CISO. All policy changes require a High-risk change request per the Change Management and Configuration Control Procedure. Changes to Conditional Access are logged, reviewed at CAB, and subject to post-implementation review within 48 hours.

### 6.3 Grace period for newly enrolled devices

Newly enrolled devices are granted a grace period of up to 8 business hours from enrolment to achieve full compliance status before Conditional Access restrictions are applied. Grace period extensions require IT Operations Manager approval.
...
The endpoint management platform generates a weekly patch compliance report. IT Operations reviews the report within 48 hours. Devices that have not received a critical or high patch within the applicable SLA are flagged as non-compliant and subject to Conditional Access restriction until patched.
...
- Access to corporate resources governed by Conditional Access.
