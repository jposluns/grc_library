...
| --- | --- | --- |
| **Generation** | Securely create cryptographic keys using approved algorithms and entropy sources. | HSM-based key creation; PQC hybrid generation (ML-KEM + ECC). |
...
| Algorithm | AES-256, RSA-4096, ECC P-384, ML-KEM, etc. |
...
Where the organization operates trade and customs systems under sector programmes such as BASC and WCO SAFE, it applies its own additional cryptographic requirement of AES-256 with PQC hybrid encryption by 2027 for those systems; the keys used for sector-programme communications must:
...
ML-KEM (key encapsulation; formerly CRYSTALS-Kyber) and ML-DSA (digital signatures; formerly CRYSTALS-Dilithium) are the designated primary PQC algorithms per NIST FIPS 203 (August 2024) and FIPS 204 (August 2024). The "ML-KEM" / "ML-DSA" names are the official NIST designations; the older "Kyber" / "Dilithium" names from the NIST PQC competition remain in common informal use.
