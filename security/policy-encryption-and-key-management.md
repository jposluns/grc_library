...
| Hashing | SHA-512, BLAKE2b-512, for integrity hashing where the organization selects the algorithm (for example log, document, evidence and data integrity). Where an external specification or ecosystem interface mandates a digest algorithm or format (for example OCI image digests, SLSA and in-toto provenance, Sigstore signatures, SBOM component hashes, or git object identifiers), that algorithm is used only for interoperability within that interface, and the governing specification is documented. Password-based encryption (its own row in this table), and key derivation and TLS (section 4), are governed separately; message-authentication and signature algorithms are not integrity hashing and are outside this row. Integrity values already recorded keep the algorithm they were recorded with; new values use SHA-512 or BLAKE2b-512. |
