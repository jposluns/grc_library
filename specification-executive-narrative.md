...
- Source pinning, staleness detection, and human revalidation.
...
**Review Frequency:** Upon any version change to a pinned corpus source, and annual\
...
**Corpus Sources:** [`risk/annex-ai-risk-methodology.md`](../risk/annex-ai-risk-methodology.md)@1.0.6, [`supply-chain/register-concentration-risk.md`](../supply-chain/register-concentration-risk.md)@1.0.6\
...
4. **Corpus Sources:** the pinned corpus source list. Each entry is a markdown link to the corpus document followed by `@` and the source's `Version` metadata value as of the page's last human revalidation. Every corpus document the page relies on for any claim must be pinned here; a corpus link in the body absent from this field is a defect. At least one pin is mandatory. Exactly one pin per distinct corpus dependency (no duplicate pins on the same target).
...
8. **Last Revalidated:** ISO 8601 date of the most recent human revalidation against the pinned source versions.
...
- **Explanatory.** The page restates, in executive language, what the pinned corpus sources establish, and claims nothing beyond them. Executive Briefs, Journeys, and Outcome Maps are Explanatory: their value is faithful translation, and any statement in them that cannot be traced to a pinned source is a defect.
...
- **citation.** The claim is traceable to a single pinned corpus source. On-page form: the claim links the corpus document inline, and that document appears in `Corpus Sources`.
...
## Source pinning, staleness, and revalidation

Currency is detected by pinning, not by review cadence alone. A hard pin-resolution failure is enforced (it blocks the page); a version drift is detected and surfaced as an advisory (item 5).

1. **Pinning.** Every corpus source a page relies on is pinned in `Corpus Sources` as `path@document-version`, where `document-version` is the source's `Version` metadata value as of the page's last human revalidation. The same value appears per document in `taxonomy.yml`, so pin checking requires no document parse. Exactly one pin per dependency.

2. **Pin resolution (fail-closed).** The staleness gate resolves every pin path into the live corpus set. It fails on any pin that is missing, moved, duplicate, malformed, or whose target is not a corpus document (a pin must not target another narrative page or an out-of-corpus path). A page with any failing pin is not eligible to be `current`. Pin resolution failure is a hard defect (a malformed or non-resolving pin is a broken page), distinct from the version-drift signal in item 5.

3. **Version-drift trigger.** A page is due-for-revalidation the moment any pinned source's current version differs from the pinned version, including a patch-level change. A pinned path that no longer resolves is the separate `invalid` hard defect (item 2), not a version drift. There is no materiality threshold for a drift: version semantics live with the source document, and the narrative layer does not second-guess them. This binds to the corpus version-bump discipline (the corpus content gate that makes an unbumped content change near-impossible is the mitigation for the content-change-without-version-change blind spot).

4. **State machine.** Each page occupies exactly one of three STABLE states, computed and recorded in the generated narrative registry (not an on-page field):
   - **current.** Every pin resolves to a live corpus document AND matches its live version. The page is pin-current (valid with respect to its source pins; the non-currency defect classes are covered by the other narrative gates, not this state).
   - **due-for-revalidation.** Every pin resolves, but at least one pinned version differs from the live corpus version (a version drift). This is the advisory fail-closed state (see item 5): the page enters it automatically on any drift.
   - **invalid.** At least one pin fails resolution (missing, moved, duplicate, malformed, or a non-corpus target). This is a hard defect (item 2) and blocks the page; it is not a currency lag.

   **Revalidation** is the human TRANSITION that returns a `due-for-revalidation` page to `current` (an `invalid` page is first repaired to resolve, which moves it to `due-for-revalidation` or `current`). It is not itself a state. The transition completes only by the atomic condition that all three legs hold together: (a) every pin updated to the exact current corpus version, and (b) a referenced review record (the `Review Record` field), and (c) a recorded version and revalidation bump on the page (the `Version` and `Last Revalidated` fields advanced in the same change). The three stable states above are computed from pins alone, so a pins-only edit would read as pin-current and silently skip the review. To prevent that, the staleness gate adds a diff-level check: whenever a change touches a page's `Corpus Sources` pins, it requires the `Version` and `Last Revalidated` fields to advance AND the `Review Record` to name a fresh record (a different identifier from the prior revalidation, since an unchanged, already-mandatory identifier evidences no new review) in the SAME change, so legs (b) and (c) cannot be omitted. That diff-level enforcement is a Phase-1.4 gate-design obligation, recorded here as the mechanism that binds the three legs; the state computation itself remains pin-only. No generator or tool ever writes a pin, edits page prose to match a changed source, or performs the transition; revalidation is a human act, and the quality of that review is a procedural control, not a mechanical guarantee.

5. **Enforcement posture (advisory plus scheduled audit).** A version-drift `due-for-revalidation` is enforced as a PR-time advisory and a scheduled-audit finding, not as a hard CI block on unrelated pull requests. A page in `due-for-revalidation` remains published but is marked in the narrative registry. This posture is deliberate: hard-failing every pull request that bumps a pinned corpus source would couple corpus velocity to narrative upkeep (a routine corpus version bump would block until each affected narrative page is revalidated), which fits neither the never-auto-update principle nor a healthy corpus cadence. The advisory surfaces the drift at the moment it is introduced; the scheduled audit is the backstop that catches any drifted page still outstanding. Pin-resolution failure (item 2, a malformed or non-resolving pin) is the exception: it is a hard defect and blocks the page, because a broken pin is a broken page rather than a currency lag.
...
- **Versioning.** A narrative page follows the corpus version-numbering scheme. Any prose change, any pin update, and any revalidation records a version and date bump.
...
- **Retirement on lost corpus support.** When the corpus support a page depends on is removed (a pinned source retired with no successor), the page's pin no longer resolves, so it enters `invalid` (item 4). If it cannot be re-pinned to a live corpus source, it is retired. A narrative page must never outlive the corpus it explains.

---

## The narrative registry

`narrative.yml` is the generated, machine-readable registry of the narrative layer, the analogue of `taxonomy.yml` for `executive/`. It is regenerated from page metadata, never hand-edited, and carries per page: path, title, narrative type, narrative status, version, date, corpus source pins, external sources, claim classes present, review record identifier, last-revalidated date, staleness state, and any derived tags.
...
The registry generator provides a `--check` mode that fails loud on an unreadable or malformed candidate page and never discovers pages from the committed registry. Registry completeness is proven by a three-way bijection among the independently enumerated `executive/` files, the registry rows, and the generated listing routes, with named exclusions for entry points and redirects. The generator's placement and internal design, including whether staleness computation lives in the generator or in a separate check, are settled when the generator is built.
...
This section records the gate intent this specification creates. The narrative gate family does not exist yet: the gate-scope manifest's `narrative-only` class is empty at the specification's authoring point. Every item below marked new is design work for later phases; the specific wiring (which existing gate is extended versus which new gate is created, gate numbering, parity registration, and the four-surface obligation) is settled when each gate is built. Items marked existing are verified mechanisms this specification relies on.
...
2. **Scan-scope split (existing, one residue).** The narrative-directory set and the gate-scope manifest exist and are regression-checked; the safety root-walkers cover `executive/` and the link gate already includes it. The residue is the by-name inclusion of `specification-executive-narrative.md` in the intended scan roots of the gates meant to govern this specification; that wiring is a later-phase item. No prefix-exemption is needed (the file is `Document Type: Specification` with the matching `specification-` prefix, so it passes the prefix check normally).
3. **Symmetric narrative-boundary gate (new).** Outside `executive/`: reject narrative document types or narrative-extension fields anywhere, README paths included (the README skip becomes path-scoped, not basename-keyed). Inside `executive/`: require the `Executive Narrative` type and the narrative-extension block on every page, and reject corpus document types. Honest residue: a file stripped of both its type marker and all narrative-only fields is not detectable by repository-state rules and is covered by review only.
4. **Narrative metadata gate (new).** Validates every file under `executive/` (except the named `executive/README.md` entry point): the 13 canonical fields plus the 8 extension fields, closed vocabularies for Narrative Type and Narrative Status, the type-to-prefix parity from the subtype table, the fixed subtype-to-status mapping, `Document Type: Executive Narrative` exactly, at least one `Corpus Sources` pin, and pin syntax `path@semver`. It also enforces body-link/pin completeness: every corpus document linked in the page body must appear in `Corpus Sources` (a body corpus link absent from the pin list is a defect). The shared line-break gate does NOT scan `executive/`, so this narrative metadata gate owns the backslash hard-break validation for narrative pages (the append itself needs no change to the shared checker, whose generic last-line exemption already tolerates the extended block; the coverage of `executive/` is the new gate's obligation).
5. **Disclaimer presence gate (new).** The universal authority disclaimer, verbatim, in the required position on every page. String-level only.
6. **Vocabulary gate (new).** No `guarantee` or the listed absolutes anywhere in narrative prose (page-wide, no causal-statement classification needed); no unqualified `shall` (with the qualified-shall definition as the sole exception); no em or en dashes. The existing bare-shall, language, and dash gates do not cover `executive/`, so narrative coverage needs its own wiring, not an `AUDITED_DOMAIN_DIRS` edit, which would wrongly pull `executive/` into every content gate.
7. **One-way authority boundary gate (new).** Corpus documents must not reference `executive/` in any field; the taxonomy generator must reject `executive/` targets in corpus relationship metadata. Modelled on the existing directional-dependency gate. Typed-edge endpoint validation enforces the node-class and direction matrix.
8. **Staleness gate (new).** Implements the fail-closed state machine: resolve every pin against `taxonomy.yml`, fail hard on missing, moved, duplicate, malformed, or non-corpus pin targets, and flag any version mismatch as an advisory `due-for-revalidation` that feeds the scheduled audit rather than blocking unrelated pull requests; and, on any change that touches a page's `Corpus Sources` pins, require the `Version` and `Last Revalidated` fields to advance and the `Review Record` identifier to change (the diff-level revalidation-evidence check that binds legs (b) and (c)). Never auto-fixes.
9. **Registry and completeness gate (new).** `narrative.yml` matches a fresh regeneration (`--check`), fails loud on unreadable or malformed candidates, and never discovers from the committed registry; a three-way bijection among independently enumerated `executive/` files, registry rows, and generated listing routes, with named exclusions for entry points and redirects.
...
11. **Collision-ledger and mixed-diff gate (new).** A ledger-to-content bijection check, and a pull-request-delta mixed-diff gate that rejects a narrative-content pull request touching a corpus-document path absent an accountably-reviewed override marker (the marker subject to a review rule, never mere presence).
