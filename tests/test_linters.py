...
class VerificationGuardrailSelfTests(unittest.TestCase):
    ...
    def test_block_repeated_tool_failure_hook_self_test(self) -> None:
        result = self._run_selftest(
            [sys.executable,
             str(REPO_ROOT / ".claude" / "hooks" / "block-repeated-tool-failure.py"),
             "--self-test"]
        )
        self.assertEqual(
            result.returncode, 0,
...
class GateCountConsistencyTests(LinterTestCase):
    ...
    def test_stale_wordform_audit_gates_flagged(self) -> None:
        # P9 (word-form): "ninety-eight audit gates" can never match the
        # canonical gate count, so the word-form gate-count check must
        # flag it. (The gate-39-blind word-form class; the backlog item that queued it has rotated out.)
        fixture = self.make_fixture(
            "standard-bad-wordform-gates.md",
            "# X\n\nThis stub mentions the ninety-eight audit gates in the suite.\n",
        )
        result = run_linter("tools/lint-gate-count-consistency.py", fixture)
        self.assertLinterFails(result)

    def test_stale_wordform_growth_narrative_flagged(self) -> None:
        # P12 (growth-narrative): "gates to two hundred" resolves the
        # collection keyword (gates) and the TO-target word-number; 200 never
        # matches the canonical count, so it is flagged. The rounded FROM value
        # ("a dozen gates") is deliberately NOT matched.
        fixture = self.make_fixture(
            "standard-bad-wordform-growth.md",
            "# X\n\nThe machinery grew from a dozen gates to two hundred over time.\n",
        )
        result = run_linter("tools/lint-gate-count-consistency.py", fixture)
        self.assertLinterFails(result)
...
class ClaudeRulesSyncCompilerOwnershipTests(LinterTestCase):
    """tools/lint-claude-rules-sync.py: compiler-owned recognition (compile PR-2).

    The completeness check recognizes a local rule file as compiler-owned ONLY
    through a valid entry in the Corpus-Management ownership register (never a
    directory skip): a covered file passes (its byte-integrity is gate 99's
    job), an uncovered file under the reserved subdirectory still fails as
    unmapped, and an ownership entry that contests a mirrored path, sits
    outside the reserved subdirectory, or claims a block under .claude/rules/
    is itself a finding. An unparseable register is an internal error (2).
    """

    OWNED_FILE_ENTRY = (
        "schema_version = 1\n\n[[owned_targets]]\n"
        'rule = "gen-rule"\nkind = "file"\n'
        'target = ".claude/rules/corpus-management/generated-rule.md"\n'
    )
    OWNED_TREE_ENTRY = (
        "schema_version = 1\n\n[[owned_targets]]\n"
        'rule = "gen-tree"\nkind = "tree"\n'
        'target = ".claude/rules/corpus-management"\n'
    )
    ...
    def _make_root(self, ownership_toml, owned_local):
        ...
        (root / ".claude" / "rules" / "secrets.md").write_text(
            "# Title\n\nIdentical body.\n", encoding="utf-8")
        (root / "guardrails" / "core" / "secrets.md").write_text(
            "# Title\n\nIdentical body.\n", encoding="utf-8")
        if ownership_toml is not None:
            reg_dir = root / ".corpus-management" / "core"
            reg_dir.mkdir(parents=True)
            (reg_dir / "ownership.toml").write_text(ownership_toml, encoding="utf-8")
        if owned_local:
            d = root / ".claude" / "rules" / "corpus-management"
            d.mkdir(parents=True)
            (d / "generated-rule.md").write_text(
                "<!-- GENERATED by corpus-management -->\n\n# Generated rule\n",
                encoding="utf-8")
        return root, {".claude/rules/secrets.md": "guardrails/core/secrets.md"}

    def _run_patched(self, ownership_toml, owned_local):
        mod = ClaudeRulesSyncTests._load_module()
        root, fake_map = self._make_root(ownership_toml, owned_local)
        saved = mod.MIRROR_MAP
        try:
            mod.MIRROR_MAP = fake_map
            import contextlib, io
    ...
    def test_compiler_owned_file_recognized(self):
        rc, out = self._run_patched(self.OWNED_FILE_ENTRY, owned_local=True)
        self.assertEqual(rc, 0, f"a register-covered file must not be unmapped:\n{out}")

    def test_owned_tree_coverage_recognized(self):
        rc, out = self._run_patched(self.OWNED_TREE_ENTRY, owned_local=True)
        self.assertEqual(rc, 0, f"a file inside a register-covered tree must pass:\n{out}")

    def test_uncovered_file_fails(self):
        # The reserved subdirectory is NOT a skip: with no covering register
        # entry, a hand-dropped file there still fails loud.
        rc, out = self._run_patched(self.EMPTY_REGISTER, owned_local=True)
        self.assertEqual(rc, 1, out)
        self.assertIn("unmapped local rule file", out)

    def test_mirror_map_overlap_fails(self):
        entry = self.OWNED_FILE_ENTRY.replace(
            ".claude/rules/corpus-management/generated-rule.md",
            ".claude/rules/secrets.md")
        rc, out = self._run_patched(entry, owned_local=False)
        self.assertEqual(rc, 1, out)
        self.assertIn("ownership overlap", out)

    def test_block_kind_under_rules_fails(self):
        entry = self.OWNED_FILE_ENTRY.replace('kind = "file"', 'kind = "block"')
        rc, out = self._run_patched(entry, owned_local=False)
        self.assertEqual(rc, 1, out)
        self.assertIn("block-kind", out)

    def test_outside_reservation_fails(self):
        entry = self.OWNED_FILE_ENTRY.replace(
            ".claude/rules/corpus-management/generated-rule.md",
            ".claude/rules/governance/new-generated.md")
        rc, out = self._run_patched(entry, owned_local=False)
        self.assertEqual(rc, 1, out)
        self.assertIn("outside the reserved", out)

    def test_unparseable_register_exits_2(self):
        mod = ClaudeRulesSyncTests._load_module()
        root, fake_map = self._make_root("owned_targets = [broken", owned_local=False)
        saved = mod.MIRROR_MAP
        try:
            mod.MIRROR_MAP = fake_map
            rc = mod.main(["--root", str(root)])
            self.assertEqual(rc, 2, "an unparseable ownership register is an internal error")
        finally:
            mod.MIRROR_MAP = saved
...
class WebGeneratorV2StagingTests(unittest.TestCase):
    ...
    def test_v3_narrative_domain_rail_matches_registry(self):
        ...
        set. Guards the per-narrative-rail fix across all 25 pages, not one."""
        import re
        figures = self.mod.compute_figures()
        with tempfile.TemporaryDirectory() as tmp:
            out = Path(tmp)
    ...
    def test_v3_no_stale_executive_blob_links(self):
        ...
        the allowed map from the narrative registry (each of the 25 pages links
...
class UnwiredToolSelfTests(LinterTestCase):
    ...
    never exercised in CI (the same gap ``TokenSpendToolTests`` closed for the token-spend tool). Each ``--self-test`` below was verified to run on inline fixtures with no sibling /
...
class WebGeneratorNarrativeJoinTests(unittest.TestCase):
    ...
    def test_real_registry_pins_known_join_values(self):
        ...
        figures = self.mod.compute_figures()
        self.assertEqual(len(figures["narratives"]), 32)
        by_domain = {
            dp["domain"]: len(dp["narratives"]) for dp in figures["domain_pages"]
        }
        self.assertEqual(by_domain["risk"], 17)
...
class WebGeneratorReadingRoomTests(unittest.TestCase):
    ...
    decisions/<subtype>/<slug>/), the on-site route generation (PR-2b; extended to /v3 in PR #1638: ALL 25
    ...
    def test_v2_renders_all_twenty_two_narrative_routes(self):
        """PR-2b: EVERY registry-listed narrative page (all 25, across the
        ...
        self.assertEqual(len(narratives), 32)
        self.assertEqual(
...
class CorpusManagementScanScopeTests(unittest.TestCase):
    ...
    def test_positive_twins_and_repo_wide_boundary(self):
        ...
        for name, attr, expected in (
            ("lint-stdlib-only-imports.py", "SCAN_DIRS", ("tools", "tests", ".web", ".corpus-management/tools", "vendor/aiqt/tools")),
            ("lint-unused-imports.py", "DEFAULT_DIRS", ("tools", "tests", ".web", ".claude/hooks", ".corpus-management/tools")),
        ):
...
class CorpusManagementPackActivationTests(unittest.TestCase):
    ...
    pack Python would leave the audit green. Runs in the regression suite (gate 36)."""

    PACK = REPO_ROOT / ".corpus-management"

    def _load(self, rel):
        import tomllib
        with open(self.PACK / rel, "rb") as fh:
            return tomllib.load(fh)

    def test_pack_active_and_manifest_wellformed(self):
        self.assertTrue(self.PACK.is_dir(), ".corpus-management/ pack must be present")
    ...
    def test_generation_enabled_and_summary_matches_ruleset(self):
        man = self._load("core/manifest.toml")
        self.assertTrue(man["generation"]["enabled"], "compile PR-2 enables generation")
        rules = self._load("gensrc.toml")["rules"]
        self.assertTrue(rules, "compile PR-2 installs at least one generation rule")
        self.assertEqual(
            sorted(set(man["generation"]["owned_targets"])),
            sorted({r["target"] for r in rules}),
            "manifest [generation].owned_targets must equal the ruleset's target set",
        )
    ...
    def test_register_paths_resolve_and_shapes(self):
        ...
        # id-history); the gate and hook registers stay empty until a transfer wave fills them.
        expected = {
        ...
        for key, rel in man["registers"].items():
            ...
            coll, non_empty = expected[Path(rel).name]
            self.assertIn(coll, reg, f"register {key!r} missing its {coll!r} collection")
            self.assertIsInstance(reg[coll], list, f"register {key!r} {coll!r} must be a list")
            if non_empty:
                self.assertTrue(reg[coll], f"register {key!r} {coll!r} must be non-empty in PR-2")
            else:
                self.assertEqual(reg[coll], [], f"register {key!r} {coll!r} must stay empty in PR-2")

    def test_gensrc_ownership_parity(self):
        rules = self._load("gensrc.toml")["rules"]
        entries = self._load("core/ownership.toml")["owned_targets"]
        rule_triples = {(r["id"], r["kind"], r["target"]) for r in rules}
        own_triples = {(e["rule"], e["kind"], e["target"]) for e in entries}
        self.assertEqual(rule_triples, own_triples,
                         "gensrc rules and the ownership register must agree exactly")

    def test_clauses_wellformed(self):
        clauses = self._load("core/clauses.toml")["clauses"]
        ids = [c["id"] for c in clauses]
        self.assertEqual(len(ids), len(set(ids)), "clause ids must be unique")
        for c in clauses:
    ...
    def test_pack_python_only_under_tools(self):
        ...
        self.assertTrue(pys, "compile PR-2 ships the compiler inside the pack")
        for rel in pys:
            self.assertEqual(rel.parts[0], "tools",
                             f"pack Python must live only under .corpus-management/tools/: {rel}")


class CorpusManagementCompilerTests(LinterTestCase):
    """tools/build-corpus-management.py -> .corpus-management/tools/corpus_mgmt_compiler.py
    (gate 99).

    Exercised through the wrapper as a subprocess against synthetic --root fixtures (the
    same shape gates 35/37 use), with a true-positive control (a clean fixture passes) and
    engineered drift and configuration defects. The live tree must also pass, since that is
    what every other gate run assumes.
    """
    ...
    NOTE_SOURCE = "- Never hand-edit generated files.\n"
    HANDBOOK_IN_SYNC = (
        "# Handbook\n\nIntro prose.\n\n"
        "<!-- cm:begin note -->\n"
    ...
    def _make_root(self, *, gensrc, ownership, owned_targets,
        ...
        (pack / "core" / "clauses.toml").write_text(
            clauses if clauses is not None else self.EMPTY_CLAUSES, encoding="utf-8"
        )
    ...
    def _run(self, root, *flags):
        return run_linter("tools/build-corpus-management.py", *flags, "--root", root)

    def _block_root(self, handbook):
        return self._make_root(
            ...
            files={"HANDBOOK.md": handbook},
        )

    def test_live_tree_check_passes(self):
        # The live repository state must be in sync (what every other gate
        # run assumes); this is also the byte-identity proof for the
        # transferred CLAUDE.md slice.
        result = run_linter("tools/build-corpus-management.py", "--check")
        self.assertEqual(
            result.returncode, 0,
            f"gate 99 should pass on the current state.\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}",
        )

    def test_clean_block_fixture_passes(self):
        root = self._block_root(self.HANDBOOK_IN_SYNC)
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 0, result.stdout + result.stderr)
        self.assertIn("OK:", result.stdout)

    def test_stale_block_body_flagged(self):
        stale = self.HANDBOOK_IN_SYNC.replace(
            "- Never hand-edit generated files.", "- STALE body line."
        )
        result = self._run(self._block_root(stale), "--check")
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        self.assertIn("out of sync", result.stdout)

    def test_missing_sentinels_flagged(self):
        result = self._run(
            self._block_root("# Handbook\n\nNo sentinels here.\n"), "--check"
        )
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        self.assertIn("expected exactly one begin", result.stdout)

    def test_duplicated_sentinel_flagged(self):
        doubled = self.HANDBOOK_IN_SYNC.replace(
            "<!-- cm:begin note -->\n",
            "<!-- cm:begin note -->\n<!-- cm:begin note -->\n",
            1,
        )
    ...
    def test_reversed_sentinels_flagged(self):
        ...
        result = self._run(self._block_root(reversed_doc), "--check")
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        self.assertIn("does not precede", result.stdout)

    def test_missing_source_config_error(self):
        root = self._make_root(
            gensrc=self.BLOCK_GENSRC.replace("core/policies/note.md",
                                             "core/policies/absent.md"),
            ownership=self.BLOCK_OWNERSHIP,
            owned_targets='"HANDBOOK.md"',
            files={"HANDBOOK.md": self.HANDBOOK_IN_SYNC},
        )
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 2, result.stdout + result.stderr)
        self.assertIn("source does not exist", result.stdout)

    def test_duplicate_rule_ids_config_error(self):
        gensrc = self.BLOCK_GENSRC + (
            "\n[[rules]]\n"
            'id = "note"\nkind = "block"\n'
            'sources = ["core/policies/note.md"]\n'
            'target = "OTHER.md"\n'
            'begin = "<!-- cm:begin other -->"\n'
            'end = "<!-- cm:end other -->"\n'
        )
        root = self._make_root(
            gensrc=gensrc,
        ...
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 2, result.stdout + result.stderr)
        self.assertIn("duplicate rule id", result.stdout)

    def test_claude_rules_tree_reservation_config_error(self):
        gensrc = (
            "schema_version = 1\n\n[[rules]]\n"
            'id = "grab-rules"\nkind = "tree"\n'
            'sources = ["core/rules"]\n'
        ...
        root = self._make_root(
            gensrc=gensrc,
            ownership=ownership,
            owned_targets='".claude/rules"',
            sources={"core/rules/a.md": "# A\n\nBody a.\n"},
        )
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 2, result.stdout + result.stderr)
        self.assertIn("rejected outright", result.stdout)

    def test_ownership_gensrc_mismatch_config_error(self):
        root = self._make_root(
            gensrc=self.BLOCK_GENSRC,
            ownership=self.BLOCK_OWNERSHIP.replace("HANDBOOK.md", "WRONG.md"),
            owned_targets='"HANDBOOK.md"',
            sources={"core/policies/note.md": self.NOTE_SOURCE},
            files={"HANDBOOK.md": self.HANDBOOK_IN_SYNC},
        )
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 2, result.stdout + result.stderr)
        self.assertIn("no owned_targets entry matching", result.stdout)

    def test_unexpected_file_in_owned_tree_flagged(self):
        root = self._make_root(
            gensrc=self.TREE_GENSRC,
            ownership=self.TREE_OWNERSHIP,
        ...
        (root / "generated" / "rules" / "zz-extra.md").write_text(
            "# Stray\n", encoding="utf-8"
        )
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
    ...
    def test_generate_then_check_green_and_idempotent(self):
        stale = self.HANDBOOK_IN_SYNC.replace(
            "- Never hand-edit generated files.", "- OLD body line."
        )
        root = self._block_root(stale)
        first = self._run(root)
        self.assertEqual(first.returncode, 0, first.stdout + first.stderr)
        self.assertIn("1 updated", first.stdout)
        check = self._run(root, "--check")
        self.assertEqual(check.returncode, 0, check.stdout + check.stderr)
        after_first = (root / "HANDBOOK.md").read_bytes()
        second = self._run(root)
        self.assertEqual(second.returncode, 0, second.stdout + second.stderr)
        self.assertIn("0 updated", second.stdout)
        self.assertEqual(after_first, (root / "HANDBOOK.md").read_bytes(),
                         "generation must be byte-idempotent")
        self.assertIn(
            "- Never hand-edit generated files.",
    ...
    # --- Compile PR-3: tree-child containment (the #2038 routed blocker),
    # --- the inside-root symlink rider, and the first live file-kind rule. ---
    def test_tree_child_symlink_escape_flagged(self):
        # An EXPECTED tree child replaced by a symlink to an OUTSIDE file with
        # the exact rendered bytes: a content compare through the link passes,
        # so containment must flag it (--check drift, generate refusal).
        root = self._make_root(
            gensrc=self.TREE_GENSRC, ownership=self.TREE_OWNERSHIP,
            owned_targets='"generated/rules"',
            sources={"core/rules/a.md": "# A\n\nBody a.\n"},
        ...
        self.assertEqual(self._run(root).returncode, 0)
        child = root / "generated" / "rules" / "a.md"
        outside = root.parent / "cm-child-escape.md"
        ...
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        self.assertIn("outside the project root", result.stdout)
        gen = self._run(root)
        self.assertEqual(gen.returncode, 2, gen.stdout + gen.stderr)

    def test_tree_child_inside_root_symlink_flagged_and_repaired(self):
        # An inside-root symlink child (content-equal) is an unowned mutation
        # channel: --check flags it as a symlink; plain generate repairs it
        # (os.replace swaps the link for a regular file).
        root = self._make_root(
            gensrc=self.TREE_GENSRC, ownership=self.TREE_OWNERSHIP,
            owned_targets='"generated/rules"',
            sources={"core/rules/a.md": "# A\n\nBody a.\n"},
        ...
        self.assertEqual(self._run(root).returncode, 0)
        child = root / "generated" / "rules" / "a.md"
        inside = root / "generated" / "copy.md"
        inside.write_bytes(child.read_bytes())
        child.unlink()
        os.symlink(inside, child)
        result = self._run(root, "--check")
        self.assertEqual(result.returncode, 1, result.stdout + result.stderr)
        self.assertIn("is a symlink", result.stdout)
        gen = self._run(root)
        self.assertEqual(gen.returncode, 0, gen.stdout + gen.stderr)
        self.assertFalse((root / "generated" / "rules" / "a.md").is_symlink())
        self.assertEqual(self._run(root, "--check").returncode, 0)

    def test_clean_file_rule_provenance_and_idempotency(self):
        root = self._make_root(
            gensrc='schema_version = 1\n\n[[rules]]\nid = "r"\nkind = "file"\n'
                   'sources = ["core/rules/conv.md"]\ntarget = "gen/conv.md"\n',
            ownership='schema_version = 1\n\n[[owned_targets]]\nrule = "r"\n'
                      'kind = "file"\ntarget = "gen/conv.md"\n',
            owned_targets='"gen/conv.md"',
            sources={"core/rules/conv.md": "# Conv\n\nBody.\n"},
        )
        first = self._run(root)
        self.assertEqual(first.returncode, 0, first.stdout + first.stderr)
        self.assertEqual(self._run(root, "--check").returncode, 0)
        out = (root / "gen" / "conv.md").read_text(encoding="utf-8")
        self.assertTrue(out.startswith(
            "<!-- GENERATED by corpus-management; source: core/rules/conv.md;"))
        second = self._run(root)
        self.assertIn("0 updated", second.stdout)
