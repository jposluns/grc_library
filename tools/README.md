...
| `lint-filename-title-alignment.py` | Permissive audit. Flags active documents where the filename (after the doctype prefix) and the Document Title share no significant content words. Catches typos and copy-paste mistakes such as duplicated words in titles. Honours an acronym-to-expansion synonym table for legitimate patterns (SBOM/CAPA/FedRAMP/etc.). |
