...
follow-up), not guessed here. WORKER durations are measured separately by the worker dispatcher (`dur=`),
...
def report(repo_root: Path, session: str | None, idle_threshold_s: int, oneline: bool) -> int:
    ...
    print("  separately by the worker dispatcher (dur=) and kept in their own column, never summed with these.")
    return 0
