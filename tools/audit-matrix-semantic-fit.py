...
def main(argv: list[str]) -> int:
    ...
    parser.add_argument("--docs", nargs="+", metavar="PATH",
                        help="scope to matrix rows / source docs whose path is in this set; each a .md file, relative "
                             "to the repo root (run from it) or absolute (per-batch cadence)")
    ...
    if args.self_test and args.docs:
        ...
        parser.error("--self-test takes no --docs; run the self-test and the scoped audit separately.")
    if args.self_test:
        return _self_test()
    if args.docs:
        ...
        not_md = [d for d in args.docs if not (d.endswith(".md") and (lint_common.REPO_ROOT / d).is_file())]
        if not_md:
            for d in not_md:
                print(f"ERROR: --docs {d}: not a .md document (a directory or other file selects no "
                      f"matrix row or framework table).", file=sys.stderr)
            return 2
