...
def build(ref: Path) -> dict:
    ...
    if len(reqs) < MIN_ASVS_REQS or len(sections) < MIN_ASVS_SECTIONS or len(chapters) < MIN_ASVS_CHAPTERS:
        raise ValueError(f"ASVS source looks truncated ({len(reqs)} requirements, {len(sections)} "
                         f"sections, {len(chapters)} chapters)")
    ...
    return {
        ...
        "asvs": {
            ...
            "sha256": digest(["OWASP ASVS", "5.0.0", ASVS_SRC], asvs_ids),
        },
        "cwe": {
            "name": "MITRE CWE",
            "edition": "4.20",
...
def main(argv: list[str]) -> int:
    ...
    print(f"build-alignment-citation-registry: wrote {OUT.name}. Pins for "
          ...
          f"{data['asvs']['sha256']}; CWE {data['cwe']['counts']} {data['cwe']['sha256']}.")
    return 0
