...
# public grc_library repo (reaching no grc_library_ref / grc_library_private
# sibling) can still run every audit gate green AND run the §1.19.2-scoped
# advisory tools (ref-holds and the reference-reaching family) without a
# spurious error. The corpus' QA toolchain is
...
for tool in ref-holds.py build-reference-manifest.py \
