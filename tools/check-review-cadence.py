...
Parses each active document's metadata block, computes the
...
Usage:
    ...
    python3 tools/check-review-cadence.py --all

The ``--all`` flag lists every document with its computed schedule
(used for diagnostics). The ``--as-of`` flag overrides today's date
...
Exit codes:
    ...
    2   the ``--as-of`` argument could not be parsed as YYYY-MM-DD.
"""
...
# is restricted to the DOMAINS list and never reaches those directories.
...
FREQUENCY_MAP = [
    ...
    ("semi annual", 6),
    ("biennial", 24),
    # 'biannual' = twice a year (6 months). Distinct from 'biennial'
    # which is every two years. The order here keeps 'biennial' first
    # so prefix-style 'in' matches don't collide.
    ("biannual", 6),
    ("annual", 12),
    ("yearly", 12),
    ("12 months", 12),
