...
# grc_library_scratch (_scratch) is RETIRED (maintainer-directed 2026-09-23): never used, cloned, or synced, and
# deliberately NOT an expected sibling, so detect-env neither probes it nor reports it "missing" (which would nudge
# a re-clone). RETIRED_SIBLINGS documents the retirement; nothing here resolves or exports it.
RETIRED_SIBLINGS = ("grc_library_scratch",)
SIBLINGS = ("grc_library_ref", "grc_library_private")
    ...
    class T(unittest.TestCase):
        ...
        def test_repo_env_resolves_siblings(self):
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                # a retired sibling that is still PRESENT on disk is ignored: no env var, no structure entry
                self.assertNotIn("GRC_SCRATCH_PATH", info["env"])
                self.assertNotIn("grc_library_scratch", info["structure"])
        ...
        def test_repo_env_omits_absent_sibling(self):
            ...
            with tempfile.TemporaryDirectory() as d:
                ...
                # a retired sibling is never reported missing (no re-clone nudge)
                self.assertNotIn("grc_library_scratch", info["missing"])
...
def repo_env_map(root: "Path") -> dict:
    ...
    for name, p in (("grc_library_ref", ref), ("grc_library_private", private)):
        structure[name] = {"path": str(p), "present": _usable_sibling(p)}
    missing = [n for n in ("grc_library_ref", "grc_library_private")
...
def repo_env_export_block(env: dict) -> str:
    ...
    for k in ("GRC_REPO", "GRC_REF_PATH", "GRC_PRIVATE_PATH"):
        if k in env:
