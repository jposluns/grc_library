#!/usr/bin/env python3
"""Enforce required section presence by document type.

Library documents follow a canonical structure: a Purpose section is
universal, Scope is near-universal, and most controls-related documents
also declare Framework alignment. This linter verifies that documents
include the sections expected for their doctype.

The linter is deliberately conservative: it requires only sections that
are near-universal in current library practice, accepts multiple
acceptable heading names per requirement, and exempts edge cases by
filename pattern.
...
Exit codes:
    0   no findings
    1   one or more documents missing a required section
"""
