...
Monday-dated files. Three kinds of record are swept:
  ...
  3. (TODO section 1.19.9) the aged table ROWS of the ROLLUP_REGISTERS
     ...
     dynamic cutoff prepared). See ROLLUP_REGISTERS.
...
Modes (mutually exclusive; default --dry-run):
    ...
    --emit-archive DIR  write the weekly detailed-CHANGELOG archive files, copy
                        the dated record files, and write the per-register aged
                        roll-up rows into DIR (the grc_library_private archive).
                        Does NOT modify grc_library.
    --prune             rewrite the in-repo detailed mirror to current-week-only,
                        rewrite each roll-up register to its kept (current-week)
                        rows, and delete the swept dated record files from
                        grc_library, then delink any now-absent history.md
                        Detail-column links. Requires --verify-archived DIR.
...
def main(argv: list[str]) -> int:
    ...
    if args.prune:
        ...
        print(f"Pruned: mirror rewritten to {len(keep)} current-week entries "
              ...
              f"{len(rollups)} register(s). Archive verified present. Delinked "
              f"{delinked} now-absent history.md Detail-column link(s).")
        return 0
